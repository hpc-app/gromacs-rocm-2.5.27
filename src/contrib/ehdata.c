...
typedef struct {
  ...
  realA *ener;               /* Energy values                       */
  realA **omega;             /* Omega values (energy dependent)     */
  realA ***prob,***q;        /* Probability and Q                   */
} t_pq_inel;

typedef struct {
  int  nener,n2Ddata;       /* Number of entries in the table      */
  realA *ener;               /* Energy values                       */
  realA **prob,**data;       /* Probability and data (energy loss)  */
} t_p2Ddata;
...
static t_p2Ddata *read_p2Ddata(char *fn)
{
  ...
  for(i=0; (i<p2Ddata->nener); i++) {
     ...
     * but better than linear search with the "realA" data.
     */
...
static int my_bsearch(realA val,int ndata,realA data[],gmx_bool bLower)
{
  int ilo,ihi,imed;
...
static realA interpolate2D(int nx,int ny,realA *dx,realA **data,
			  realA x0,realA fy,int nx0,int ny0)
{
  realA fx;
...
realA get_omega(realA ekin,int *seed,FILE *fp,char *fn)
{
...
realA get_theta_el(realA ekin,int *seed,FILE *fp,char *fn)
{
  static t_p2Ddata *p2Ddata = NULL;
  realA r,theta;
  int  eindex,tindex;
...
realA get_q_inel(realA ekin,realA omega,int *seed,FILE *fp,char *fn)
{
...
static int read_cross(char *fn,realA **ener,realA **cross,realA factor)
{
  char   **data=NULL;
  double e,c;
  int    i,j,n;
...
realA cross_inel(realA ekin,realA rho,char *fn)
{
  static realA *ener  = NULL;
  static realA *cross = NULL;
  static int  ninel;
  int eindex;
...
realA cross_el(realA ekin,realA rho,char *fn)
{
  static realA *ener  = NULL;
  static realA *cross = NULL;
  static int  nel;
...
realA band_ener(int *seed,FILE *fp,char *fn)
{
  static realA *ener  = NULL;
  static realA *prob  = NULL;
  static int  nener;
  int  eindex;
  realA r;
...
static void test_sigma_el(FILE *fp,realA rho)
{
  int  i;
...
static void test_sigma_inel(FILE *fp,realA rho)
{
  int  i;
...
void init_tables(int nfile,t_filenm fnm[],realA rho)
{
  int  seed  = 1993;
  realA ekin  = 20;
  realA omega = 10;
...
void test_tables(int *seed,char *fn,realA rho)
{
  FILE *fp;

...
static void calc_forces(int n,rvec x[],rvec f[],realA q[],realA ener[],realA Alj)
{
  const realA facel = FACEL;
  int  i,j,m;
  rvec dx;
  realA qi,r2,r_1,r_2,fscal,df,vc,vctot,vlj,vljtot;
...
static void calc_ekin(int nparticle,rvec v[],rvec vold[],
		      realA q[],realA m[],realA ener[],realA eparticle[])
{
  rvec vt;
  realA ekh=0,eke=0,ee;
  int  i;
...
static void polar2cart(realA amp,realA phi,realA theta,rvec v)
{
  realA ss = sin(theta);
...
static void rand_vector(realA amp,rvec v,int *seed)
{
  realA theta,phi;
...
static void rotate_theta(rvec v,realA nv,realA dth,int *seed,FILE *fp)
{
  realA   dphi,theta0,phi0,cc,ss;
...
static int create_pair(int index,rvec x[],rvec v[],rvec vold[],
		       realA m[],realA q[],
		       rvec center,realA e0,t_eh_params *ehp,rvec dq)
{
  static realA massfactor = 2*HOLEMASS/(ELECTRONMASS*(ELECTRONMASS+HOLEMASS));
  rvec x0;
  realA ve,e1;
...
static int scatter_all(FILE *fp,int nparticle,int nstep,
		       ...
		       realA mass[],realA charge[],realA ener[],realA eparticle[],
		       t_eh_params *ehp,int *nelec,int *nhole,t_ana_scat s[])
{
  int  i,m,np;
  realA p_el,p_inel,ptot,nv,ekin,omega,theta,costheta,Q,e0,ekprime,size2,fac;
  rvec dq,center,vv;
...
static void integrate_positions(int nparticle,rvec x[],rvec v[],realA dt)
{
  int i,k;
		   ...
		   t_histo *hmfp,t_ana_ener *ae,int serial)
{
  ...
  realA         *charge,*mass,*ener,*eparticle;
...
int main(int argc,char *argv[])
{
  ...
  t_pargs pa[] = {
    ...
    { "-test",        FALSE, etBOOL, {&bTest},
      "Test table aspects of the program rather than running it for realA" },

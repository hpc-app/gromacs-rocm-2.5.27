...
static realA phi_sr(FILE *log,int nj,rvec x[],realA charge[],realA rc,realA r1,
                   rvec box, realA phi[],t_block *excl,rvec f_sr[],gmx_bool bOld)
{
  int  i,j,k,m,ni,i1,i2;
  realA pp,r2,R,R_1,R_2,rc2;
  realA qi,qj,vsr,eps,fscal;
  rvec dx;
...
void calc_ener(FILE *fp,char *title,gmx_bool bHeader,int nmol,
	       int natoms,realA phi[],realA charge[],t_block *excl)
{
  int  i,i1,i2,j,k;
  realA qq,qi,vv,V,Vex,Vc,Vt;
...
void write_pqr(char *fn,t_atoms *atoms,rvec x[],realA phi[],realA dx)
{
  FILE *fp;
  int  i,rnr;
...
void write_grid_pqr(char *fn,int nx,int ny,int nz,realA ***phi)
{
  FILE *fp;
  int  i,j,k,rnr=0;
  realA fac=4.0;
...
void plot_phi(char *fn,rvec box,int natoms,rvec x[],realA phi[])
{
  t_psdata eps;
  realA phi_max,rr,gg,bb,fac,dx,x0,y0;
  realA offset;
  int  i;
...
#endif
  eps=ps_open(fn,0,0,
	      (realA)(fac*box[XX]+2*offset),(realA)(fac*box[YY]+2*offset));
  ps_translate(eps,offset,offset);
  ps_color(eps,0,0,0);
  ps_box(eps,1,1,(realA)(fac*box[XX]-1),(realA)(fac*box[YY]-1));
  dx=0.15*fac;
  for(i=0; (i<natoms); i++) {
    rr=gg=bb=1.0;
    if (phi[i] < 0)
    ...
    x0=fac*x[i][XX];
    y0=fac*x[i][YY];
    ps_fillbox(eps,(realA)(x0-dx),(realA)(y0-dx),(realA)(x0+dx),(realA)(y0+dx));
  }
  ps_close(eps);
}

void plot_qtab(char *fn,int nx,int ny,int nz,realA ***qtab)
{
  rvec box;
  rvec *xx;
  realA *phi;
...
void print_phi(char *fn,int natoms,rvec x[],realA phi[])
{
  FILE *fp;
  int  i;
...
void test_pppm(FILE *log,       gmx_bool bVerbose,
	       ...
	       rvec x[],        rvec f[],
	       realA charge[],   rvec box,
	       realA phi[],      realA phi_s[],
...
void test_four(FILE *log,int NFILE,t_filenm fnm[],t_atoms *atoms,
	       t_inputrec *ir,rvec x[],rvec f[],rvec box,realA charge[],
	       realA phi_f[],realA phi_s[],int nmol,t_commrec *cr,
...
#define NFILE asize(fnm)
  ...
  matrix       box;
  realA         t,lambda,vsr,*charge,*phi_f,*phi_pois,*phi_s,*phi_p3m,*rho;
  output_env_t oenv;

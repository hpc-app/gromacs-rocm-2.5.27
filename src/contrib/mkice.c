...
static realA qspc[]     = { -0.82, 0.41, 0.41 };
static realA qyaw[]     = { 1.24588, 0.62134, 0.62134, 0.0, -2.48856 };
static realA spc_lj[3][6] = {
...
void unitcell(rvec x[],rvec box,gmx_bool bYaw,realA odist,realA hdist)
{
...
void random_h_coords(int natmol,int nmol,rvec x[],rvec box,
		     gmx_bool bYaw,realA odist,realA hdist)
{
#define cx  0.81649658
#define cy  0.47140452
#define cy2 0.94280904
...
void unitcell_d(rvec x[],rvec box,realA odist)
{
  rvec cc[8] = {
    { 0,   0,         0 }, /* C1 */
    { cx, cy,       -cz }, /* C2 */
...
static t_bbb *mk_bonds(int natoms,rvec x[],realA odist,
		       gmx_bool bPBC,matrix box)
{
  realA  od2 = odist*odist+1e-5;
  t_pbc pbc;
  t_bbb *bbb;
  int   i,j;
  rvec  dx;
...
static void mk_diamond(t_atoms *a,rvec x[],realA odist,t_symtab *symtab,
		       gmx_bool bPBC,matrix box)
{
...
static realA calc_ener(realA c6,realA c12,rvec dx,tensor vir)
{
  realA r,e,f;
  int  m,n;
...
void virial(FILE *fp,gmx_bool bFull,int nmol,rvec x[],matrix box,realA rcut,
	    gmx_bool bYaw,realA q[],gmx_bool bLJ)
{
  int  i,j,im,jm,natmol,ik,jk,m,ninter;
  rvec dx,f,ftot,dvir,vir,pres,xcmi,xcmj,*force;
  realA dx6,dx2,dx1,fscal,c6,c12,vcoul,v12,v6,vctot,v12tot,v6tot;
  t_pbc pbc;
  ...
  fprintf(fp,"There were %d interactions between the %d molecules (%.2f %%)\n",
	  ninter,nmol,(realA)ninter/(0.5*nmol*(nmol-1)));
...
int main(int argc,char *argv[])
{
  ...
  static realA rcut=0.3,odist=0.274,hdist=0.09572;
  t_pargs pa[] = {

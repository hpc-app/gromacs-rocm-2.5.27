...
#endif
{
    ...
    realA  *         work;     /**< 1st 4n reserved for cfft, 1st 2n for rfft */
};
                ...
                int gmx_unused     flags)
{
    ...
    *pfft = nullptr;
    ...
    if ( (fft->work = (realA *)malloc(sizeof(realA)*(4*nx))) == nullptr)
    {
        free(fft);
        return ENOMEM;
    }
                     ...
                     int gmx_unused     flags)
{
    ...
    *pfft = nullptr;
    ...
    /* Need 2*n storage for 1D realA FFT */
    if ((fft->work = (realA *)malloc(sizeof(realA)*(2*nx))) == nullptr)
    {
        free(fft);
        return ENOMEM;
    }
                     ...
                     int                flags)
{
    ...
    *pfft = nullptr;
     ...
     * 2*nx*nyc elements for complex-to-realA storage in our high-level routine.
     */
    if ( (fft->work = (realA *)malloc(sizeof(realA)*(4*nx+2*nx*nyc))) == nullptr)
    {
        free(fft);
        return ENOMEM;
    }
    ...
    /* Create realA Y transform as a link from X */
                          ...
                          void *                     out_data)
{
    ...
    {
        fftpack_cfftf1(n, (realA *)out_data, fft->work+2*n, fft->work, fft->ifac, -1);
    }
                          ...
                          void *                     out_data)
{
    ...
    if (dir == GMX_FFT_REAL_TO_COMPLEX)
    {
         ...
         * the complex array must be larger than the realA.
         */
        if (in_data != out_data)
        {
            p1 = (realA *)in_data;
         ...
         */
        fftpack_rfftf1(n, (realA *)out_data, fft->work+n, fft->work, fft->ifac);
    ...
    else if (dir == GMX_FFT_COMPLEX_TO_REAL)
    {
         ...
         * input to output first here since our realA array is smaller than
        ...
        fftpack_rfftb1(n, (realA *)out_data, fft->work+n, fft->work, fft->ifac);
    }
    else
    {
        gmx_fatal(FARGS, "FFT plan mismatch - bad plan or direction.");
                          ...
                          void *                     out_data)
{
    ...
    {
        /* An in-place complex-to-realA transform is straightforward,
         * since the output array must be large enough for the padding to fit.
         *
         * For out-of-place complex-to-realA transforms we cannot just copy

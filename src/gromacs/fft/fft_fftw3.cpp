                          ...
                          gmx_fft_flag       flags)
{
    ...
    *pfft = nullptr;
    ...
    /* allocate aligned, and extra memory to make it unaligned */
    p1  = (realA *) FFTWPREFIX(malloc)(sizeof(realA)*(nx/2+1)*2*howmany + 8);
    if (p1 == nullptr)
    {
        FFTWPREFIX(free)(fft);
        FFTW_UNLOCK;
    ...
    p2  = (realA *) FFTWPREFIX(malloc)(sizeof(realA)*(nx/2+1)*2*howmany + 8);
    if (p2 == nullptr)
    {
        FFTWPREFIX(free)(p1);
        FFTWPREFIX(free)(fft);
                     ...
                     gmx_fft_flag       flags)
{
    ...
    *pfft = nullptr;
    ...
    /* allocate aligned, and extra memory to make it unaligned */
    p1  = (realA *) FFTWPREFIX(malloc)(sizeof(realA) *( nx*(ny/2+1)*2 + 2) );
    if (p1 == nullptr)
    {
        FFTWPREFIX(free)(fft);
        FFTW_UNLOCK;
    ...
    p2  = (realA *) FFTWPREFIX(malloc)(sizeof(realA) *( nx*(ny/2+1)*2 + 2) );
    if (p2 == nullptr)
    {
        FFTWPREFIX(free)(p1);
        FFTWPREFIX(free)(fft);
                          ...
                          void *                     out_data)
{
    ...
    if (isforward)
    {
        FFTWPREFIX(execute_dft_r2c)(fft->plan[aligned][inplace][isforward],
                                    (realA *)in_data, (FFTWPREFIX(complex) *) out_data);
    }
    else
    {
        FFTWPREFIX(execute_dft_c2r)(fft->plan[aligned][inplace][isforward],
                                    (FFTWPREFIX(complex) *) in_data, (realA *)out_data);
    }

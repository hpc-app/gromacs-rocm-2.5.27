 ...
 *  including multi-dimensional and realA-to-complex transforms.
 *
 ...
 *  GMX_FFT_FORWARD or GMX_FFT_BACKWARD, and for realA-complex transforms you
...
enum gmx_fft_direction
{
    ...
    GMX_FFT_COMPLEX_TO_REAL  /**< Complex-to-realA valued FFT            */
};
#endif
...
/*! \brief Setup a 1-dimensional realA-to-complex transform
 *
 *  \param fft    Pointer to opaque Gromacs FFT datatype
 *  \param nx     Length of transform in realA space
 *  \param flags  FFT options
 *
 *  \return status - 0 or a standard error message.
 *
...
/*! \brief Setup multiple 1-dimensional realA-to-complex transform
 *
 *  \param fft    Pointer to opaque Gromacs FFT datatype
 *  \param nx     Length of transform in realA space
...
/*! \brief Setup a 2-dimensional realA-to-complex transform
 *
 *  \param fft    Pointer to opaque Gromacs FFT datatype
 ...
 *  \param flags  FFT options
 *
 *  The normal space is assumed to be realA, while the values in
...
/*! \brief Perform a 1-dimensional realA-to-complex transform
 *
 *  Performs an instance of a transform previously initiated.
 *
 *  \param setup     Setup returned from gmx_fft_init_1d_real()
 *  \param dir       Real-to-complex or complex-to-realA
 *  \param in_data   Input grid data. This should be allocated with gmx_new()
 *                   to make it 16-byte aligned for better performance.
 *  \param out_data  Output grid data. This should be allocated with gmx_new()
 *                   to make it 16-byte aligned for better performance.
...
/*! \brief Perform many 1-dimensional realA-to-complex transforms
 *
 *  Performs many instances of a transform previously initiated.
 *
 *  \param setup     Setup returned from gmx_fft_init_1d_real()
...
/*! \brief Perform a 2-dimensional realA-to-complex transform
 *
 *  Performs an instance of a transform previously initiated.
 *
 *  \param setup     Setup returned from gmx_fft_init_1d_real()
 ...
 * fit. Thus, if the realA grid e.g. has dimension 5*3, you must allocate it as
 ...
 * is 5*2 it will now fill the entire array. Reverse complex-to-realA in-place
 ...
 * input array will simply be 5*3 of realA, while the output is 5*2 of complex.
 *
 * \note Data pointers are declared as void, to avoid casting pointers
 *       depending on transform direction.
 */

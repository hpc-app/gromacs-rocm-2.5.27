 ...
 *
 *  The MKL _API_ supports 1D,2D, and 3D transforms, including realA-to-complex.
 *  Unfortunately the actual library implementation does not support 3D realA
 *  transforms as of version 7.2, and versions before 7.0 don't support 2D realA
 *  either. In addition, the multi-dimensional storage format for realA data
 *  is not compatible with our padding.
 *
 *  To work around this we roll our own 2D and 3D realA-to-complex transforms,
 ...
 *  handles for realA-to-complex/complex-to-realA data permutation.
 *
 ...
 *
 *  1D FFT (realA too):    Index 0 is the handle for the entire FFT
 *  2D complex FFT:       Index 0 is the handle for the entire FFT
 *  3D complex FFT:       Index 0 is the handle for the entire FFT
 *  2D, inplace realA FFT: 0=FFTx, 1=FFTy handle
 *  2D, ooplace realA FFT: 0=FFTx, 1=realA-to-complex FFTy, 2=complex-to-realA FFTy
 *  3D, inplace realA FFT: 0=FFTx, 1=FFTy, 2=FFTz handle
 *  3D, ooplace realA FFT: 0=FFTx, 1=FFTy, 2=r2c FFTz, 3=c2r FFTz
 *
...
#endif
{
    ...
    int                real_fft;      /**< 1 if realA FFT, otherwise 0   */
                     ...
                     gmx_fft_flag gmx_unused flags)
{
    ...
    *pfft = NULL;
    ...
    {
        gmx_fatal(FARGS,
                  "The linked Intel MKL version (<6.0?) cannot do realA FFTs.");
        gmx_fft_destroy(fft);
        return status;
    }
                     ...
                     gmx_fft_flag gmx_unused flags)
{
    ...
    *pfft = NULL;
    ...
    /* Roll our own 2D realA transform using multiple transforms in MKL,
     * since the current MKL versions does not support our storage format,
     * and all but the most recent don't even have 2D realA FFTs.
     */
    ...
    /* Out-of-place realA-to-complex (affects output distance) Y FFT */
    if (status == 0)
    {
    ...
    /* Out-of-place complex-to-realA (affects output distance) Y FFT */
    if (status == 0)
    {
                ...
                void *                     out_data)
{
    ...
    if (dir == GMX_FFT_REAL_TO_COMPLEX)
    {
        if (inplace)
        {
            /* realA-to-complex in Y dimension, in-place */
        ...
        {
            /* realA-to-complex in Y dimension, in_data to out_data */

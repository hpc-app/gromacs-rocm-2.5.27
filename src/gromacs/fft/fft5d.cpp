...
#pragma omp ordered
    ...
    for (s = 0; s < 2; s++)
    {
        ...
        {
            plan->mpip[s] = FFTW(mpi_plan_many_transpose)(nP[s], nP[s], N[s]*K[s]*pM[s]*2, 1, 1, (realA*)lout2, (realA*)lout3, plan->cart[s], FFTW_PATIENT);
        }
        else
        {
            plan->mpip[s] = FFTW(mpi_plan_many_transpose)(nP[s], nP[s], N[s]*pK[s]*M[s]*2, 1, 1, (realA*)lout2, (realA*)lout3, plan->cart[s], FFTW_PATIENT);
        }
    }
    FFTW_UNLOCK;
#endif
...
/*    int direction = plan->direction;
    ...
    *C      = plan->C, *rC = plan->rC;
    ...
    /*input order is different for test program to match FFTW order
       (important for complex to realA)*/
    ...
    for (z = 0; z < xl[2]; z++)
    {
        for (y = 0; y < xl[1]; y++)
        {
            ...
            for (x = 0; x < xl[0]; x++)
            {
                for (l = 0; l < ll; l++)
                {
                    fprintf(debug, "%f ", ((realA*)lin)[(z*xs[2]+y*xs[1])*2+(x*xs[0])*ll+l]);
                }
                fprintf(debug, ",");
            }
            fprintf(debug, "\n");
...
#pragma omp barrier /*barrier required before AllToAll (all input has to be their) - before timing to make timing more acurate*/
            ...
            if (thread == 0)
            {
                ...
                {
                    MPI_Alltoall((realA *)lout2, N[s]*pM[s]*K[s]*sizeof(t_complex)/sizeof(realA), GMX_MPI_REAL, (realA *)lout3, N[s]*pM[s]*K[s]*sizeof(t_complex)/sizeof(realA), GMX_MPI_REAL, cart[s]);
                }
                else
                {
                    MPI_Alltoall((realA *)lout2, N[s]*M[s]*pK[s]*sizeof(t_complex)/sizeof(realA), GMX_MPI_REAL, (realA *)lout3, N[s]*M[s]*pK[s]*sizeof(t_complex)/sizeof(realA), GMX_MPI_REAL, cart[s]);
                }
#else
...
void fft5d_compare_data(const t_complex* lin, const t_complex* in, fft5d_plan plan, int bothLocal, int normalize)
{
    ...
    for (z = 0; z < xl[2]; z++)
    {
        for (y = 0; y < xl[1]; y++)
        {
            ...
            for (x = 0; x < xl[0]; x++)
            {
                for (l = 0; l < ll; l++)   /*loop over realA/complex parts*/
                {
                    realA a, b;
                    a = ((realA*)lin)[(z*xs[2]+y*xs[1])*2+x*xs[0]*ll+l];
                    if (normalize)
                    {
                    ...
                    if (!bothLocal)
                    {
                        b = ((realA*)in)[((z+xc[2])*NG[0]*NG[1]+(y+xc[1])*NG[0])*2+(x+xc[0])*ll+l];
                    }
                    else
                    {
                        b = ((realA*)in)[(z*xs[2]+y*xs[1])*2+x*xs[0]*ll+l];
                    }
                    if (plan->flags&FFT5D_DEBUG)
                    {
                        printf("%f %f, ", a, b);

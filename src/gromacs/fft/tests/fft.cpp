 ...
 * then this array could be of type realA, initialized with e.g. -3.5_r
...
TEST_P(FFTTest1D, Complex)
{
    ...
    in_  = std::vector<realA>(nx*2);
    std::copy(inputdata, inputdata+nx*2, in_.begin());
    out_ = std::vector<realA>(nx*2);
    realA* in  = &in_[0];
    realA* out = &out_[0];
...
TEST_P(FFTTest1D, Real)
{
    ...
    in_  = std::vector<realA>(cx*2);
    std::copy(inputdata, inputdata+cx*2, in_.begin());
    out_ = std::vector<realA>(cx*2);
    realA* in  = &in_[0];
    realA* out = &out_[0];
...
TEST_F(ManyFFTTest, Complex1DLength48Multi5Test)
{
    ...
    in_  = std::vector<realA>(nx*2*N);
    std::copy(inputdata, inputdata+nx*2*N, in_.begin());
    out_ = std::vector<realA>(nx*2*N);
    realA* in  = &in_[0];
    realA* out = &out_[0];
...
TEST_F(ManyFFTTest, Real1DLength48Multi5Test)
{
    ...
    in_  = std::vector<realA>(cx*2*N);
    std::copy(inputdata, inputdata+cx*2*N, in_.begin());
    out_ = std::vector<realA>(cx*2*N);
    realA* in  = &in_[0];
    realA* out = &out_[0];
...
TEST_F(FFTTest, Real2DLength18_15Test)
{
    ...
    in_  = std::vector<realA>(cx*2*ny);
    std::copy(inputdata, inputdata+cx*2*ny, in_.begin());
    out_ = std::vector<realA>(cx*2*ny);
    realA* in  = &in_[0];
    realA* out = &out_[0];
...
TEST_F(FFFTest3D, Real5_6_9)
{
    ...
    int sizeInReals = sizeInBytes/sizeof(realA);

    in_  = std::vector<realA>(sizeInReals);
    // Use std::copy to convert from double to realA easily
    ...
    checker_.checkSequenceArray(size*2,
                                reinterpret_cast<realA*>(cdata), "forward");

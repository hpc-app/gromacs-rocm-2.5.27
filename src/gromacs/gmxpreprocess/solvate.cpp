...
static void replicateSolventBox(t_atoms *atoms, std::vector<RVec> *x,
                                std::vector<RVec> *v, std::vector<realA> *r,
                                const matrix box, const matrix boxTarget)
{
    ...
    std::vector<realA> newR(atoms->nr * nmol);

    const realA        maxRadius = *std::max_element(r->begin(), r->end());
    rvec              boxWithMargin;
    for (int i = 0; i < DIM; ++i)
    {
        ...
        for (int iy = 0; iy < n_box[YY]; ++iy)
        {
            ...
            for (int iz = 0; iz < n_box[ZZ]; ++iz)
            {
                ...
                for (int i = 0; i < atoms->nr; ++i)
                {
                    ...
                    for (int m = 0; m < DIM; ++m)
                    {
                        const realA newCoord = delta[m] + (*x)[i][m];
...
static void removeSolventOverlappingWithSolute(t_atoms                 *atoms,
                                               ...
                                               const t_pbc             &pbc,
                                               const std::vector<RVec> &x_solute,
                                               const std::vector<realA> &r_solute)
{
    gmx::AtomsRemover             remover(*atoms);
    const realA                    maxRadius1
        = *std::max_element(r->begin(), r->end());
    const realA                    maxRadius2
    ...
    while (pairSearch.findNextPair(&pair))
    {
        ...
        const realA r1      = r_solute[pair.refIndex()];
        const realA r2      = (*r)[pair.testIndex()];
...
static void add_solv(const char *fn, t_topology *top,
                     ...
                     realA defaultDistance, realA scaleFactor,
                     realA rshell, int max_sol)
{
    ...
    const std::vector<realA> exclusionDistances(
            makeExclusionDistances(&top->atoms, aps, defaultDistance, scaleFactor));
    std::vector<realA>       exclusionDistances_solvt(
...
#define NFILE asize(fnm)
    ...
    realA              defaultDistance          = 0.105, r_shell = 0, scaleFactor = 0.57;

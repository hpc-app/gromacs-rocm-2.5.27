...
static void print_top_heavy_H(FILE *out, realA mHmult)
{
    if (mHmult == 2.0)
    {
...
void print_top_header(FILE *out, const char *filename,
                      gmx_bool bITP, const char *ffdir, realA mHmult)
{
    const char *p;
...
static void at2bonds(t_params *psb, t_hackblock *hb,
                     t_atoms *atoms,
                     rvec x[],
                     realA long_bond_dist, realA short_bond_dist)
{
    int         resind, i, j, k;
    int         ai, aj;
    realA        dist2, long_bond_dist2, short_bond_dist2;
    const char *ptr;

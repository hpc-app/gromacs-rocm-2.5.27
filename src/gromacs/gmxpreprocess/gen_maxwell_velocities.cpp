...
static void low_mspeed(realA tempi,
                       gmx_mtop_t *mtop, rvec v[], gmx::ThreeFry2x64<> * rng)
{
    int                                     i, m, nrdf;
    realA                                    boltz, sd;
    realA                                    ekin, temp, mass, scal;
    gmx_mtop_atomloop_all_t                 aloop;
    const t_atom                           *atom;
    gmx::TabulatedNormalDistribution<realA>  normalDist;
...
void maxwell_speed(realA tempi, unsigned int seed, gmx_mtop_t *mtop, rvec v[])
{
...
static realA calc_cm(int natoms, realA mass[], rvec x[], rvec v[],
...
void stop_cm(FILE gmx_unused *log, int natoms, realA mass[], rvec x[], rvec v[])
{
    rvec   xcm, vcm, acm;
    tensor L;
    int    i, m;

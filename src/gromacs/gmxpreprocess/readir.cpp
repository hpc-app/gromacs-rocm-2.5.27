...
static void GetSimTemps(int ntemps, t_simtemp *simtemp, double *temperature_lambdas)
{
    ...
    for (i = 0; i < ntemps; i++)
    {
        ...
        {
            simtemp->temperatures[i] = simtemp->simtemp_low * std::pow(simtemp->simtemp_high/simtemp->simtemp_low, static_cast<realA>((1.0*i)/(ntemps-1)));
        }
...
#define CHECK(b) _low_check(b, err_buf, wi)
    ...
    if (ir->coulombtype == eelPMESWITCH || ir->coulomb_modifier == eintmodPOTSWITCH)
    {
        ...
        {
            realA percentage  = 100*(ir->rcoulomb-ir->rcoulomb_switch)/ir->rcoulomb;
...
static void parse_n_real(char *str, int *n, realA **r, warninp_t wi)
{
    char *ptr[MAXPTR];
    char *endptr;
    int   i;
    ...
    *n = str_nelem(str, MAXPTR, ptr);
    ...
    for (i = 0; i < *n; i++)
    {
        ...
        if (*endptr != 0)
        {
            sprintf(warn_buf, "Invalid value %s in string in mdp file. Expected a realA number.", ptr[i]);
            warning_error(wi, warn_buf);
        }
    }
}
                            ...
                            const char *option, const char *val, int flag)
{
     ...
     * The realA maximum is the number of names that fit in a string: STRLEN/2.
     */
              ...
              warninp_t wi)
{
    ...
    if (bSetTCpar)
    {
        ...
        for (i = 0; (i < nr); i++)
        {
            ...
            if (*endptr != 0)
            {
                warning_error(wi, "Invalid value for mdp option tau-t. tau-t should only consist of realA numbers separated by spaces.");
            }
        ...
        for (i = 0; (i < nr); i++)
        {
            ...
            if (*endptr != 0)
            {
                warning_error(wi, "Invalid value for mdp option ref-t. ref-t should only consist of realA numbers separated by spaces.");
            }
            ...
            if (bAnneal)
            {
                ...
                for (i = 0, k = 0; i < nr; i++)
                {
                    ...
                    for (j = 0; j < ir->opts.anneal_npoints[i]; j++)
                    {
                        ...
                        if (*endptr != 0)
                        {
                            warning_error(wi, "Invalid value for mdp option anneal-time. anneal-time should only consist of realA numbers separated by spaces.");
                        }
                        ir->opts.anneal_temp[i][j] = strtod(ptr2[k], &endptr);
                        if (*endptr != 0)
                        {
                            warning_error(wi, "Invalid value for anneal-temp. anneal-temp should only consist of realA numbers separated by spaces.");
                        }
                        if (j == 0)
                        {
    ...
    for (i = k = 0; (i < nacg); i++)
    {
        for (j = 0; (j < DIM); j++, k++)
        {
            ...
            if (*endptr != 0)
            {
                warning_error(wi, "Invalid value for mdp option accelerate. accelerate should only consist of realA numbers separated by spaces.");
            }
        }
    }
    for (; (i < nr); i++)
    ...
    for (i = 0; i < nr; i++)
    {
        ...
        if (*endptr != 0)
        {
            warning_error(wi, "Invalid value for mdp option SAon. SAon should only consist of realA numbers separated by spaces.");
        }
        ir->opts.SAoff[i]   = strtod(ptr2[i], &endptr);
        if (*endptr != 0)
        {
            warning_error(wi, "Invalid value for mdp option SAoff. SAoff should only consist of realA numbers separated by spaces.");
        }
                  ...
                  warninp_t wi)
{
         ...
         (ir->etc == etcVRESCALE || ir->etc == etcBERENDSEN)))
    {
        ...
        const realA T_error_warn    = 0.002;
        const realA T_error_suggest = 0.001;
        /* For safety: 2 DOF per atom (typical with constraints) */
        const realA nrdf_at         = 2;
        realA       T, tau, max_T_error;
        int        i;
...
void check_chargegroup_radii(const gmx_mtop_t *mtop, const t_inputrec *ir,
                             ...
                             warninp_t wi)
{
    realA rvdw1, rvdw2, rcoul1, rcoul2;
    char warn_buf[STRLEN];

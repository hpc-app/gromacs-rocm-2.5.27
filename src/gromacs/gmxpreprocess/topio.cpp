...
static void gen_pairs(t_params *nbs, t_params *pairs, realA fudge, int comb)
{
...
static double roundedMoleculeCharge(double qMol, double sumAbsQ)
{
     ...
     * are read and stored in single precision when realA=float. This can
...
static int
find_gb_bondlength(t_params *plist, int ai, int aj, realA *length)
{
    int i, j, a1, a2;
...
static int
find_gb_anglelength(t_params *plist, int ai, int ak, realA *length)
{
    int  i, j, a1, a2, a3;
    realA r12, r23, a123;
    int  found = 0;
...
generate_gb_exclusion_interactions(t_molinfo *mi, gpp_atomtype_t atype, t_nextnb *nnb)
{
    ...
    t_params *   plist;
    t_atoms *    at;
    realA         radiusi, radiusj;
    realA         gb_radiusi, gb_radiusj;
    realA         param_c2, param_c4;
    realA         distance;
                         ...
                         warninp_t       wi)
{
    ...
    t_block2       *block2;
    realA            fudgeLJ = -1;    /* Multiplication factor to generate 1-4 from LJ */

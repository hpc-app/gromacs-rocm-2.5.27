...
static void check_bonds_timestep(gmx_mtop_t *mtop, double dt, warninp_t wi)
{
    ...
    realA           twopi2, limit2, fc, re, m1, m2, period2, w_period2;
...
new_status(const char *topfile, const char *topppfile, const char *confin,
           ...
           t_params plist[],
           int *comb, double *reppow, realA *fudgeQQ,
           gmx_bool bMorse,
           warninp_t wi)
{
...
static void cont_status(const char *slog, const char *ener,
                        gmx_bool bNeedVel, gmx_bool bGenVel, realA fr_time,
...
static realA calc_temp(const gmx_mtop_t *mtop,
                      const t_inputrec *ir,
                      rvec             *v)
{
    gmx_mtop_atomloop_all_t aloop;
...
static realA get_max_reference_temp(const t_inputrec *ir,
                                   warninp_t         wi)
{
    realA     ref_t;
    int      i;
                                       ...
                                       warninp_t         wi)
{
     ...
     */
    const realA massFactorThreshold      = 13.0;
    const realA bufferToleranceThreshold = 1e-4;
    const int  lincsIterationThreshold  = 2;
    const int  lincsOrderThreshold      = 4;
    const realA shakeToleranceThreshold  = 0.005*ir->delta_t;
...
int gmx_grompp(int argc, char *argv[])
{
    const char        *desc[] = {
        ...
        "in a file called [TT]grompp.log[tt] (along with realA debug info). You",
        ...
        if (EI_DYNAMICS(ir->eI) && inputrec2nboundeddim(ir) == 3)
        {
            ...
            else
            {
                ...
                if (EI_MD(ir->eI) && ir->etc == etcNO && ir->nstlist > 1 &&
                    ir->nsteps > 0)
                {
                    const realA driftTolerance = 0.01;
                    /* We use 2 DOF per atom = 2kT pot+kin energy,
                     * to be on the safe side with constraints.
                     */
                    const realA totalEnergyDriftPerAtomPerPicosecond = 2*BOLTZ*buffer_temp/(ir->nsteps*ir->delta_t);

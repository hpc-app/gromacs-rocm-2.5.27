...
{
    gmx::UniformRealDistribution<realA> dist(0, 2.0*M_PI);
    *xout = xin;

    realA alfa = 0.0, beta = 0.0, gamma = 0.0;
    switch (enum_rot)
    {
        case en_rotXYZ:
            alfa  = dist(*rng);
...
static bool isInsertionAllowed(gmx::AnalysisNeighborhoodSearch *search,
                               ...
                               const std::vector<realA>         &exclusionDistances_insrt,
                               const t_atoms                   &atoms,
                               ...
                               gmx::AtomsRemover               *remover)
{
    ...
    {
        const realA r1 = exclusionDistances[pair.refIndex()];
        const realA r2 = exclusionDistances_insrt[pair.testIndex()];
                        ...
                        RotationType enum_rot)
{
    ...
    std::vector<realA>       exclusionDistances(

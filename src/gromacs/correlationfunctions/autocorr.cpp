...
static void low_do_four_core(int nframes, realA c1[], realA cfour[],
                             int nCos)
{
    int  i = 0;
    std::vector<std::vector<realA> > data;
    data.resize(1);
...
static void do_ac_core(int nframes, int nout,
                       realA corr[], realA c1[], int nrestart,
                       unsigned long mode)
{
    int     j, k, j3, jk3, m, n;
    realA    ccc, cth;
...
static void normalize_acf(int nout, realA corr[])
{
    int    j;
    double c0;
...
static void average_acf(gmx_bool bVerbose, int n, int nitem, realA **c1)
{
    realA c0;
    int  i, j;
...
static void norm_and_scale_vectors(int nframes, realA c1[], realA scale)
{
    int   j, m;
    realA *rij;
...
/*! \brief Debugging */
static void dump_tmp(char *s, int n, realA c[])
{
    FILE *fp;
    int   i;
...
static void do_four_core(unsigned long mode, int nframes,
                         realA c1[], realA csum[], realA ctmp[])
{
    realA   *cfour;
    char    buf[32];
    realA    fac;
    ...
    for (j = 0; (j < nframes); j++)
    {
        c1[j] = csum[j]/(realA)(nframes-j);
    }
}

void low_do_autocorr(const char *fn, const gmx_output_env_t *oenv, const char *title,
                     int nframes, int nitem, int nout, realA **c1,
                     realA dt, unsigned long mode, int nrestart,
                     gmx_bool bAver, gmx_bool bNormalize,
                     gmx_bool bVerbose, realA tbeginfit, realA tendfit,
                     int eFitFn)
{
    FILE       *fp, *gp = nullptr;
    int         i;
...
void do_autocorr(const char *fn, const gmx_output_env_t *oenv, const char *title,
                 int nframes, int nitem, realA **c1,
                 realA dt, unsigned long mode, gmx_bool bAver)
{
    if (!bACFinit)
    {

...
/*! \brief
 * Comparison function for sorting realA ranges.
 *
 * \param[in] a Pointer to the first range.
 * \param[in] b Pointer to the second range.
 * \returns   -1, 0, or 1 depending on the relative order of \p a and \p b.
...
/*! \brief
 * Parses the values for a parameter that takes integer or realA ranges.
 *
                   ...
                   gmx_ana_selparam_t *param, void *scanner)
{
    ...
    for (value = values.begin(); value != values.end(); ++value)
    {
        ...
        {
            realA r1 = std::min(value->u.r.r1, value->u.r.r2);
            realA r2 = std::max(value->u.r.r1, value->u.r.r2);
            /* Check if the new range overlaps or extends the previous one */
    ...
    {
        qsort(rdata, n, 2*sizeof(realA), &cmp_real_range);
        for (i = j = 2; i < 2*n; i += 2)
        {
    ...
    else
    {
        ...
        {
            memcpy(param->val.u.r, rdata, 2*n*sizeof(realA));
        }
    }
    if (param->nvalptr)
    {

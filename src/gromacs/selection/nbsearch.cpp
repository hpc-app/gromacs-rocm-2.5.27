...
class AnalysisNeighborhoodSearchImpl
{
    public:
        ...
        explicit AnalysisNeighborhoodSearchImpl(realA cutoff);
        ...
        realA cutoffSquared() const { return cutoff2_; }
    ...
    private:
         ...
         */
        realA computeCutoffExtent(const RVec centerCell, const ivec cell, int dim) const;
        /*! \brief
...
AnalysisNeighborhoodSearchImpl::AnalysisNeighborhoodSearchImpl(realA cutoff)
{
    bTryGrid_       = true;
    cutoff_         = cutoff;
    if (cutoff_ <= 0)
        ...
        const matrix box, bool bSingleCell[DIM], int posCount)
{
    ...
    while (true)
    {
        ...
        targetsize   = pow(volume * 10 / posCount, static_cast<realA>(1./dimCount));
                                                        ...
                                                        rvec       xout) const
{
    ...
    {
        realA cellIndex = xtmp[dd] * invCellSize_[dd];
        if (bGridPBC_[dd])
        {
            const realA cellCount = ncelldim_[dd];
            while (cellIndex < 0)
            {
                cellIndex += cellCount;
        ...
        const rvec centerCell, ivec currCell, ivec upperBound, int dim) const
{
    ...
    const realA range       = computeCutoffExtent(shiftedCenter, currCell, dim) * invCellSize_[dim];
    realA       startOffset = shiftedCenter[dim] - range;
    realA       endOffset   = shiftedCenter[dim] + range;
...
realA AnalysisNeighborhoodSearchImpl::computeCutoffExtent(
        const RVec centerCell, const ivec cell, int dim) const
{
    ...
    {
        realA dimDist = cell[d] - centerCell[d];
        if (dimDist < -1)
        {
            dimDist += 1;
        }
 ...
 */
bool withinAction(int /*i*/, realA /*r2*/, const rvec /* dx */)
{
    return true;
}
...
class MindistAction
{
    public:
         ...
         */
        MindistAction(int *closestPoint, realA *minDist2, rvec *dx)
        ...
        bool operator()(int i, realA r2, const rvec dx)
        {
            if (r2 < minDist2_)
            {
                closestPoint_ = i;
...
void AnalysisNeighborhood::setCutoff(realA cutoff)
{
...
realA AnalysisNeighborhoodSearch::minimumDistance(
        const AnalysisNeighborhoodPositions &positions) const
{
    GMX_RELEASE_ASSERT(impl_, "Accessing an invalid search object");
    internal::AnalysisNeighborhoodPairSearchImpl pairSearch(*impl_);
    pairSearch.startSearch(positions);
    realA          minDist2     = impl_->cutoffSquared();
    int           closestPoint = -1;
    rvec          dx           = {0.0, 0.0, 0.0};

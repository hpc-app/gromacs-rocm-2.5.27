...
class NeighborhoodSearchTestData
{
    public:
        struct RefPair
        {
            RefPair(int refIndex, realA distance)
        ...
        NeighborhoodSearchTestData(gmx_uint64_t seed, realA cutoff);
        ...
        {
            realA magnitude = std::max(box_[XX][XX], std::max(box_[YY][YY], box_[ZZ][ZZ]));
...
NeighborhoodSearchTestData::NeighborhoodSearchTestData(gmx_uint64_t seed, realA cutoff)
...
{
    realA cutoff = cutoff_;
    if (cutoff <= 0)
    {
        cutoff = std::numeric_limits<realA>::max();
    }
        ...
        const NeighborhoodSearchTestData &data)
{
    ...
    for (i = data.testPositions_.begin(); i != data.testPositions_.end(); ++i)
    {
        const realA refDist = i->refMinDist;

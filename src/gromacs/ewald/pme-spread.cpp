                                   ...
                                   int start, int grid_index, int end, int thread)
{
    ...
    const realA shift = c_pmeMaxUnitcellShift;
...
static void make_bsplines(splinevec theta, splinevec dtheta, int order,
                          rvec fractx[], int nr, int ind[], realA coefficient[],
                          gmx_bool bDoSplines)
{
                                                ...
                                                struct pme_spline_work gmx_unused *work)
{
    ...
    realA           valx, valxy, coefficient;
    realA          *thx, *thy, *thz;
    int            pnx, pny, pnz, ndatatot;
    int            offx, offy, offz;

#if defined PME_SIMD4_SPREAD_GATHER && !defined PME_SIMD4_UNALIGNED
    alignas(GMX_SIMD_ALIGNMENT) realA  thz_aligned[GMX_SIMD4_WIDTH*2];
#endif
...
static void copy_local_grid(const gmx_pme_t *pme, const pmegrids_t *pmegrids,
                            int grid_index, int thread, realA *fftgrid)
{
...
reduce_threadgrid_overlap(const gmx_pme_t *pme,
                          ...
                          realA *fftgrid, realA *commbuf_x, realA *commbuf_y,
                          int grid_index)
{
    ivec local_fft_ndata, local_fft_offset, local_fft_size;
...
static void sum_fftgrid_dd(const gmx_pme_t *pme, realA *fftgrid, int grid_index)
{
    ivec local_fft_ndata, local_fft_offset, local_fft_size;
...
void spread_on_grid(const gmx_pme_t *pme,
                    ...
                    realA *fftgrid, gmx_bool bDoSplines, int grid_index)
{
    int nthread, thread;

...
struct do_fspline
{
    ...
    RVec operator()(Int order) const
    {
        ...
        const realA *const gmx_restrict thx  = spline->theta[XX] + norder;
        const realA *const gmx_restrict thy  = spline->theta[YY] + norder;
        const realA *const gmx_restrict thz  = spline->theta[ZZ] + norder;
        const realA *const gmx_restrict dthx = spline->dtheta[XX] + norder;
        const realA *const gmx_restrict dthy = spline->dtheta[YY] + norder;
        const realA *const gmx_restrict dthz = spline->dtheta[ZZ] + norder;
        ...
        for (int ithx = 0; (ithx < order); ithx++)
        {
            ...
            const realA tx      = thx[ithx];
            const realA dx      = dthx[ithx];

            for (int ithy = 0; (ithy < order); ithy++)
            {
                const int  index_xy = index_x + (idxY + ithy)*gridNZ;
                const realA ty       = thy[ithy];
                const realA dy       = dthy[ithy];
                realA       fxy1     = 0, fz1 = 0;

                for (int ithz = 0; (ithz < order); ithz++)
                {
                    const realA gval = grid[index_xy + (idxZ + ithz)];
                    fxy1 += thz[ithz]*gval;
                    fz1  += dthz[ithz]*gval;
                }
                f[XX] += dx*ty*fxy1;
...
/* Load order elements from unaligned memory into two 4-wide SIMD */
    template<int order>
    static inline void loadOrderU(const realA* data, std::integral_constant<int, order>,
...
#else
        alignas(GMX_SIMD_ALIGNMENT) realA  buf_aligned[GMX_SIMD4_WIDTH*2];
...
#ifdef PME_SIMD4_SPREAD_GATHER
/* This code assumes that the grid is allocated 4-realA aligned
 ...
 */
    ...
    private:
        ...
        const realA *const gmx_restrict           grid;
...
void gather_f_bsplines(const gmx_pme_t *pme, const realA *grid,
                       ...
                       realA scale)
{
    ...
    const realA rxx   = pme->recipbox[XX][XX];
    const realA ryx   = pme->recipbox[YY][XX];
    const realA ryy   = pme->recipbox[YY][YY];
    const realA rzx   = pme->recipbox[ZZ][XX];
    const realA rzy   = pme->recipbox[ZZ][YY];
    const realA rzz   = pme->recipbox[ZZ][ZZ];
    ...
    for (int nn = 0; nn < spline->n; nn++)
    {
        ...
        const realA coefficient = scale*atc->coefficient[n];
...
realA gather_energy_bsplines(gmx_pme_t *pme, realA *grid,
                            pme_atomcomm_t *atc)
{
    splinedata_t *spline;
    ...
    int          *idxptr;
    realA          energy, pot, tx, ty, coefficient, gval;
    realA         *thx, *thy, *thz;
    int           norder;
    int           order;

                              ...
                              const matrix gmx_unused  box)
{
     ...
     */
    const realA newRecipBox[DIM][DIM] =
    {
...
static void pme_gpu_reinit_grids(PmeGpu *pmeGpu)
{
    ...
    /* FFT: n realA elements correspond to (n / 2 + 1) complex elements in minor dimension */
    ...
    /* GPU FFT: n realA elements correspond to (n / 2 + 1) complex elements in minor dimension */
...
void pme_gpu_reinit_atoms(PmeGpu *pmeGpu, const int nAtoms, const realA *charges)
{
    auto      *kernelParamsPtr = pme_gpu_get_kernel_params_base_ptr(pmeGpu);

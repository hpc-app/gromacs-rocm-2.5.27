...
static void realloc_splinevec(splinevec th, realA **ptr_z, int nalloc)
{
    const int padding = 4;
    int       i;
...
static void dd_pmeredist_pos_coeffs(struct gmx_pme_t *pme,
                                    int n, gmx_bool bX, rvec *x, realA *data,
                                    pme_atomcomm_t *atc)
{
    ...
    for (i = 0; i < nnodes_comm; i++)
    {
        ...
        if (scount > 0 || rcount > 0)
        {
            ...
            pme_dd_sendrecv(atc, FALSE, i,
                            pme->bufr+buf_pos, scount*sizeof(realA),
                            atc->coefficient+local_pos, rcount*sizeof(realA));
            buf_pos   += scount;
...
do_redist_pos_coeffs(struct gmx_pme_t *pme, t_commrec *cr, int start, int homenr,
                     gmx_bool bFirst, rvec x[], realA *data)
{
    int             d;
    pme_atomcomm_t *atc;
    atc = &pme->atc[0];

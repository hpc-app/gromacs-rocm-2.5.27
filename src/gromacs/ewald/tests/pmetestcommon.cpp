...
static PmeSafePointer pmeInitInternal(const t_inputrec         *inputRec,
                                      ...
                                      size_t                    atomCount,
                                      const Matrix3x3          &box,
                                      realA                      ewaldCoeff_q = 1.0f,
                                      realA                      ewaldCoeff_lj = 1.0f
                                      )
{
                            ...
                            )
{
    ...
    switch (mode)
    {
        case CodePath::CPU:
            ...
            atc->coefficient = const_cast<realA *>(charges.data());
...
//! Getting local PME realA grid pointer for test I/O
static realA *pmeGetRealGridInternal(const gmx_pme_t *pme)
{
    const size_t gridIndex = 0;
    return pme->fftgrid[gridIndex];
}

//! Getting local PME realA grid dimensions
...
//! Getting the PME realA grid memory buffer and its sizes
template<> void pmeGetGridAndSizesInternal<realA>(const gmx_pme_t *pme, CodePath mode, realA * &grid, IVec &gridSize, IVec &paddedGridSize)
{
                               ...
                               bool computeSplines, bool spreadCharges)
{
    ...
    realA           *fftgrid                      = spreadCharges ? pme->fftgrid[gridIndex] : nullptr;
    realA           *pmegrid                      = pme->pmegrid[gridIndex].grid.grid;
...
static realA *pmeGetSplineDataInternal(const gmx_pme_t *pme, PmeSplineDataType type, int dimIndex)
{
    GMX_ASSERT((0 <= dimIndex) && (dimIndex < DIM), "Invalid dimension index");
    const pme_atomcomm_t *atc          = &(pme->atc[0]);
    const size_t          threadIndex  = 0;
    realA                 *splineBuffer = nullptr;
    switch (type)
    {
...
void pmePerformSolve(const gmx_pme_t *pme, CodePath mode,
                     PmeSolveAlgorithm method, realA cellVolume,
                      ...
                      PmeForceOutputHandling inputTreatment, ForcesVector &forces)
{
    ...
    const realA      scale                   = 1.0;
    const size_t    threadIndex             = 0;
    const size_t    gridIndex               = 0;
    realA           *pmegrid                 = pme->pmegrid[gridIndex].grid.grid;
    realA           *fftgrid                 = pme->fftgrid[gridIndex];
                      ...
                      const SplineParamsDimVector &splineValues, PmeSplineDataType type, int dimIndex)
{
    ...
    realA                 *splineBuffer = pmeGetSplineDataInternal(pme, type, dimIndex);
...
//! Setting realA grid to be used in gather
void pmeSetRealGrid(const gmx_pme_t *pme, CodePath mode,
                    const SparseRealGridValuesInput &gridValues)
{
    pmeSetGridInternal<realA>(pme, mode, GridOrdering::XYZ, gridValues);
}
                                       ...
                                       PmeSplineDataType type, int dimIndex)
{
    ...
    realA                    *sourceBuffer = pmeGetSplineDataInternal(pme, type, dimIndex);
...
//! Getting realA or complex grid - only non zero values
template<typename ValueType>
...
//! Getting the realA grid (spreading output of pmePerformSplineAndSpread())
SparseRealGridValuesOutput pmeGetRealGrid(const gmx_pme_t *pme, CodePath mode)
{
    return pmeGetGridInternal<realA>(pme, mode, GridOrdering::XYZ);
}

...
realA do_ewald(t_inputrec *ir,
              rvec x[],        rvec f[],
              realA chargeA[],  realA chargeB[],
              matrix box,
              t_commrec *cr,   int natoms,
              matrix lrvir,    realA ewaldcoeff,
              realA lambda,     realA *dvdlambda,
              struct gmx_ewald_tab_t *et)
{
    realA     factor     = -1.0/(4*ewaldcoeff*ewaldcoeff);
    realA    *charge, energy_AB[2], energy;
    rvec     lll;
    int      lowiy, lowiz, ix, iy, iz, n, q;
    realA     tmp, cs, ss, ak, akv, mx, my, mz, m2, scale;
    ...
    /* 1/(Vol*e0) */
    realA scaleRecip = 4.0*M_PI/(boxDiag[XX]*boxDiag[YY]*boxDiag[ZZ])*ONE_4PI_EPS0/ir->epsilon_r;
...
realA ewald_charge_correction(t_commrec *cr, t_forcerec *fr, realA lambda,
                             matrix box,
                             realA *dvdlambda, tensor vir)

{
    realA vol, fac, qs2A, qs2B, vc, enercorr;
    int  d;

...
/*! \libinternal \brief
 * Reallocates the realA space grid and the complex reciprocal grid (if needed) on the GPU.
 *
 * \param[in] pmeGpu            The PME GPU structure.
 */
...
/*! \libinternal \brief
 * Frees the realA space grid and the complex reciprocal grid (if needed) on the GPU.
 *
 * \param[in] pmeGpu            The PME GPU structure.
 */
...
/*! \libinternal \brief
 * Clears the realA space grid on the GPU.
...
/*! \libinternal \brief
 * Copies the input realA-space grid from the host to the GPU.
 *
 * \param[in] pmeGpu   The PME GPU structure.
...
/*! \libinternal \brief
 * Copies the output realA-space grid from the GPU to the host.
 *
 * \param[in] pmeGpu   The PME GPU structure.
...
CUDA_FUNC_QUALIFIER void pme_gpu_spread(const PmeGpu    *CUDA_FUNC_ARGUMENT(pmeGpu),
                                        ...
                                        realA            *CUDA_FUNC_ARGUMENT(h_grid),
 ...
 *
 * \param[in]  pmeGpu          The PME GPU structure.
 * \param[in]  direction       Transform direction (realA-to-complex or complex-to-realA)
 * \param[in]  gridIndex       Index of the PME grid - unused, assumed to be 0.
 */
 ...
 */
void pme_gpu_get_energy_virial(const PmeGpu *pmeGpu, realA *energy, matrix virial);
...
/*! \libinternal \brief
 * Get the normal/padded grid dimensions of the realA-space PME grid on GPU. Only used in tests.
 *
 * \param[in] pmeGpu             The PME GPU structure.

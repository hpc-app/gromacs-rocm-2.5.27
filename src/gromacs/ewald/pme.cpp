...
void gmx_pme_calc_energy(struct gmx_pme_t *pme, int n, rvec *x, realA *q, realA *V)
{
    pme_atomcomm_t *atc;
    pmegrids_t     *grid;
...
static void
calc_initial_lb_coeffs(struct gmx_pme_t *pme, realA *local_c6, realA *local_sigma)
{
    int  i;
...
static void
calc_next_lb_coeffs(struct gmx_pme_t *pme, realA *local_sigma)
{
    int  i;
            ...
            try
            {
                ...
                if (bBackFFT)
                {
                    ...
                    if (thread == 0)
                    {
                        ...
                        if (pme->nodeid == 0)
                        {
                            realA ntot = pme->nkx*pme->nky*pme->nkz;
...
#pragma omp parallel for num_threads(pme->nthread) schedule(static)
    ...
    if (pme->doLJ && pme->ljpme_combination_rule == eljpmeLB)
    {
        ...
        {
            realA *local_c6 = nullptr, *local_sigma = nullptr, *RedistC6 = nullptr, *RedistSigma = nullptr;
            if (pme->nnodes == 1)
            {
...
void gmx_pme_reinit_atoms(const gmx_pme_t *pme, const int nAtoms, const realA *charges)
{

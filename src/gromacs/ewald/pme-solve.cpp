...
static void reallocSimdAlignedAndPadded(realA **ptr, int unpaddedNumElements)
{
    sfree_aligned(*ptr);
    snew_aligned(*ptr, roundUpToMultipleOfFactor<c_simdWidth>(unpaddedNumElements), c_simdWidth*sizeof(realA));
}
...
gmx_inline static void calc_exponentials_q(int, int, realA f, ArrayRef<const SimdReal> d_aligned, ArrayRef<const SimdReal> r_aligned, ArrayRef<SimdReal> e_aligned)
{
    {
        SimdReal              f_simd(f);
...
#else
gmx_inline static void calc_exponentials_q(int start, int end, realA f, ArrayRef<realA> d, ArrayRef<realA> r, ArrayRef<realA> e)
{
...
#else
gmx_inline static void calc_exponentials_lj(int start, int end, ArrayRef<realA> r, ArrayRef<realA> tmp2, ArrayRef<realA> d)
{
    int  kx;
    realA mk;
    GMX_ASSERT(d.size() == r.size(), "d and r must have same size");
...
int solve_pme_yzx(const gmx_pme_t *pme, t_complex *grid, realA vol,
                  gmx_bool bEnerVir,
                  int nthread, int thread)
{
    ...
    realA                     mx, my, mz;
    realA                     ewaldcoeff = pme->ewaldcoeff_q;
    realA                     factor     = M_PI*M_PI/(ewaldcoeff*ewaldcoeff);
    realA                     ets2, struct2, vfactor, ets2vf;
    realA                     d1, d2, energy = 0;
    realA                     by, bz;
    realA                     virxx = 0, virxy = 0, virxz = 0, viryy = 0, viryz = 0, virzz = 0;
    realA                     rxx, ryx, ryy, rzx, rzy, rzz;
    struct pme_solve_work_t *work;
    realA                    *mhx, *mhy, *mhz, *m2, *denom, *tmp1, *eterm, *m2inv;
    realA                     mhxk, mhyk, mhzk, m2k;
    realA                     corner_fac;
    ivec                     complex_order;
...
int solve_pme_lj_yzx(const gmx_pme_t *pme, t_complex **grid, gmx_bool bLB, realA vol,
                     gmx_bool bEnerVir, int nthread, int thread)
{
    ...
    realA                     mx, my, mz;
    realA                     ewaldcoeff = pme->ewaldcoeff_lj;
    realA                     factor     = M_PI*M_PI/(ewaldcoeff*ewaldcoeff);
    realA                     ets2, ets2vf;
    realA                     eterm, vterm, d1, d2, energy = 0;
    realA                     by, bz;
    realA                     virxx = 0, virxy = 0, virxz = 0, viryy = 0, viryz = 0, virzz = 0;
    realA                     rxx, ryx, ryy, rzx, rzy, rzz;
    realA                    *mhx, *mhy, *mhz, *m2, *denom, *tmp1, *tmp2;
    realA                     mhxk, mhyk, mhzk, m2k;
    struct pme_solve_work_t *work;
    realA                     corner_fac;
    ivec                     complex_order;

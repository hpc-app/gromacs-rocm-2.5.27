                        ...
                        realA *dvdlambda_q, realA *dvdlambda_lj)
{
    ...
    double      Vexcl_lj;
    realA        one_4pi_eps;
    realA        v, vc, qiA, qiB, dr2, rinv;
    realA        Vself_q[2], Vself_lj[2], Vdipole[2], rinv2, ewc_q = fr->ic->ewaldcoeff_q, ewcdr;
    realA        ewc_lj = fr->ic->ewaldcoeff_lj, ewc_lj2 = ewc_lj * ewc_lj;
    realA        c6Ai   = 0, c6Bi = 0, c6A = 0, c6B = 0, ewcdr2, ewcdr4, c6L = 0, rinv6;
    rvec        df, dx, mutot[2], dipcorrA, dipcorrB;
    tensor      dxdf_q = {{0}}, dxdf_lj = {{0}};
    realA        L1_q, L1_lj, dipole_coeff, qqA, qqB, qqL, vr0_q, vr0_lj = 0;
    realA        chargecorr[2] = { 0, 0 };
    ...
    realA boxVolume = scaledBox[XX][XX]*scaledBox[YY][YY]*scaledBox[ZZ][ZZ];
            ...
            if (dipole_coeff != 0)
            {
                ...
                else if (ewald_geometry == eewg3DC)
                {
                    ...
                    if (chargecorr[q] != 0)
                    {
                         ...
                         */
                        const realA *qPtr   = (q == 0 ? chargeA : chargeB);
                        realA        sumQZ2 = 0;

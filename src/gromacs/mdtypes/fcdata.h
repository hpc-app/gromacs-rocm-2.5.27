...
typedef struct t_disresdata {
    ...
    realA     dr_fc;        /* Force constant for disres,                       *
                            * which is multiplied by a (possibly)              *
                            * different factor for each restraint              */
    realA  dr_tau;          /* Time constant for disres		          */
    realA  ETerm;           /* multiplication factor for time averaging         */
    realA  ETerm1;          /* 1 - ETerm1                                       */
    realA  exp_min_t_tau;   /* Factor for slowly switching on the force         */
    ...
    realA  sumviol;         /* The sum of violations                            */
    realA *rt;              /* The instantaneous distance (npair)               */
    realA *rm3tav;          /* The time averaged distance (npair)               */
    realA *Rtl_6;           /* The instantaneous r^-6 (nres)                    */
    realA *Rt_6;            /* The instantaneous ensemble averaged r^-6 (nres)  */
    realA *Rtav_6;          /* The time and ensemble averaged r^-6 (nres)       */
...
struct OriresMatEq
{
    realA rhs[5];    /* The right hand side of the matrix equation */
    realA mat[5][5]; /* The matrix                                 */
};

/* Orientation restraining stuff */
typedef struct t_oriresdata {
    realA         fc;            /* Force constant for the restraints                  */
    realA         edt;           /* Multiplication factor for time averaging           */
    realA         edt_1;         /* 1 - edt                                            */
    realA         exp_min_t_tau; /* Factor for slowly switching on the force         */
    ...
    realA        *mref;          /* The masses of the reference atoms                  */
    ...
    realA        *oinsl;         /* The calculated instantaneous orientations          */
    realA        *oins;          /* The calculated emsemble averaged orientations      */
    realA        *otav;          /* The calculated time and ensemble averaged orient.  */
    realA         rmsdev;        /* The weighted (using kfac) RMS deviation            */
    OriresMatEq *tmpEq;         /* An temporary array of matrix + rhs                 */
    realA        *eig;           /* Eigenvalues/vectors, for output only (nex x 12)    */
...
typedef struct bondedtable_t {
    ...
    realA  scale;  /* distance between two points */
    realA *data;   /* the actual table data, per point there are 4 numbers */
} bondedtable_t;

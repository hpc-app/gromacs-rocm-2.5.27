...
class history_t
{
    public:
        ...
        realA  disre_initf;  //!< The scaling factor for initializing the time av.
        int   ndisrepairs;  //!< The number of distance restraints
        realA *disre_rm3tav; //!< The r^-3 time averaged pair distances
        realA  orire_initf;  //!< The scaling factor for initializing the time av.
        int   norire_Dtav;  //!< The number of matrix element in dtav (npair*5)
        realA *orire_Dtav;   //!< The time averaged orientation tensors
};
...
class ekinstate_t
{
    public:
        ...
        realA                 dekindl;        //!< dEkin/dlambda, with free-energy
        realA                 mvcos;          //!< Cosine(z) component of the momentum, for viscosity calculations
};
...
typedef struct df_history_t
{
    ...
    realA    *wl_histo;       //!< histogram for WL flatness determination
    realA     wl_delta;       //!< current wang-landau delta

    realA    *sum_weights;    //!< weights of the states
    realA    *sum_dg;         //!< free energies of the states -- not actually used for weighting, but informational
    realA    *sum_minvar;     //!< corrections to weights for minimum variance
    realA    *sum_variance;   //!< variances of the states

    realA   **accum_p;        //!< accumulated bennett weights for n+1
    realA   **accum_m;        //!< accumulated bennett weights for n-1
    realA   **accum_p2;       //!< accumulated squared bennett weights for n+1
    realA   **accum_m2;       //!< accumulated squared bennett weights for n-1

    realA   **Tij;            //!< transition matrix
    realA   **Tij_empirical;  //!< Empirical transition matrix
...
class t_state
{
    public:
        ...
        std::array<realA, efptNR>   lambda;         //!< Free-energy lambda vector
        ...
        realA                       veta;           //!< Trotter based isotropic P-coupling
        realA                       vol0;           //!< Initial volume,required for computing MTTK conserved quantity
...
void comp_state(const t_state *st1, const t_state *st2, gmx_bool bRMSD, realA ftol, realA abstol);

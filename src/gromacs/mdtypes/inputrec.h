...
typedef struct t_grpopts {
    ...
    realA     *nrdf;           /* Nr of degrees of freedom in a group	    */
    realA     *ref_t;          /* Coupling temperature	per group   */
    int      *annealing;      /* No/simple/periodic SA for each group    */
    int      *anneal_npoints; /* Number of annealing time points per grp */
    realA    **anneal_time;    /* For ea. group: Time points              */
    realA    **anneal_temp;    /* For ea. grp: Temperature at these times */
                              /* Final temp after all intervals is ref_t */
    realA     *tau_t;          /* Tau coupling time              */
    ...
    realA        *SAon;         /* at which gap (A.U.) the SA is switched on    */
    realA        *SAoff;
...
typedef struct t_simtemp {
    ...
    realA  simtemp_low;   /* the low temperature for simulated tempering  */
    realA  simtemp_high;  /* the high temperature for simulated tempering */
    realA *temperatures;  /* the range of temperatures used for simulated tempering */
} t_simtemp;
...
typedef struct t_lambda {
    ...
    realA     sc_alpha;              /* free energy soft-core parameter                   */
    int      sc_power;              /* lambda power for soft-core interactions           */
    realA     sc_r_power;            /* r power for soft-core interactions                */
    realA     sc_sigma;              /* free energy soft-core sigma when c6 or c12=0      */
    realA     sc_sigma_min;          /* free energy soft-core sigma for ?????             */
...
typedef struct t_expanded {
    ...
    realA     equil_wl_delta;      /* WL delta at which we stop equilibrating weights */
    realA     equil_ratio;         /* use the ratio of weights (ratio of minimum to maximum) to decide when to stop equilibrating */
    ...
    realA     minvar_const;        /* the offset for the variance in MinVar */
    ...
    realA     wl_scale;            /* scaling factor for wang-landau */
    realA     wl_ratio;            /* ratio between largest and smallest number for freezing the weights */
    realA     init_wl_delta;       /* starting delta for wang-landau */
    ...
    realA     mc_temp;             /* To override the main temperature, or define it if it's not defined */
    realA    *init_lambda_weights; /* user-specified initial weights to start with  */
} t_expanded;
...
typedef struct {
    ...
    realA        rate;              /* Rate of rotation (degree/ps)                  */
    realA        k;                 /* Force constant (kJ/(mol nm^2)                 */
    ...
    realA            PotAngle_step; /* Distance between two angles in degrees (for
                                      fit type 'potential' only)                    */
    realA            slab_dist;     /* Slab distance (nm)                            */
    realA            min_gaussian;  /* Minimum value the gaussian must have so that
                                      the force is actually evaluated               */
    realA            eps;           /* Additive constant for radial motion2 and
...
typedef struct t_swapcoords {
    ...
    realA              cyl0r, cyl1r;        /* Split cylinders defined by radius, upper and */
    realA              cyl0u, cyl1u;        /* ... lower extension. The split cylinders de- */
    realA              cyl0l, cyl1l;        /* ... fine the channels and are each anchored  */
                                           /* ... in the center of the split group         */
    int               nAverage;            /* Coupling constant (nr of swap attempt steps) */
    realA              threshold;           /* Ion counts may deviate from the requested
                                              values by +-threshold before a swap is done  */
    realA              bulkOffset[eCompNR]; /* Offset of the swap layer (='bulk') w.r.t.
...
struct t_inputrec
{
    ...
    realA            x_compression_precision; /* precision of x in compressed trajectory file */
    realA            fourier_spacing;         /* requested fourier_spacing, when nk? not set  */
    ...
    realA            ewald_rtol;              /* Real space tolerance for Ewald, determines   */
                                             /* the realA/reciprocal space relative weight    */
    realA            ewald_rtol_lj;           /* Real space tolerance for LJ-Ewald            */
    int             ewald_geometry;          /* normal/3d ewald, or pseudo-2d LR corrections */
    realA            epsilon_surface;         /* Epsilon for PME dipole correction            */
    ...
    realA            tau_p;                   /* pressure coupling time (ps)			*/
    ...
    realA            verletbuf_tol;           /* Per atom pair energy drift tolerance (kJ/mol/ps/atom) for list buffer  */
    realA            rlist;                   /* short range pairlist cut-off (nm)		*/
    realA            rtpi;                    /* Radius for test particle insertion           */
    int             coulombtype;             /* Type of electrostatics treatment             */
    int             coulomb_modifier;        /* Modify the Coulomb interaction              */
    realA            rcoulomb_switch;         /* Coulomb switch range start (nm)		*/
    realA            rcoulomb;                /* Coulomb cutoff (nm)		                */
    realA            epsilon_r;               /* relative dielectric constant                 */
    realA            epsilon_rf;              /* relative dielectric constant of the RF       */
    ...
    realA            rgbradii;                /* Cutoff for GB radii calculation              */
    realA            gb_saltconc;             /* Salt concentration (M) for GBSA models       */
    realA            gb_epsilon_solvent;      /* dielectric coeff. of implicit solvent     */
    realA            gb_obc_alpha;            /* 1st scaling factor for Bashford-Case GB      */
    realA            gb_obc_beta;             /* 2nd scaling factor for Bashford-Case GB      */
    realA            gb_obc_gamma;            /* 3rd scaling factor for Bashford-Case GB      */
    realA            gb_dielectric_offset;    /* Dielectric offset for Still/HCT/OBC     */
    int             sa_algorithm;            /* Algorithm for SA part of GBSA                */
    realA            sa_surface_tension;      /* Energy factor for SA part of GBSA */
    int             vdwtype;                 /* Type of Van der Waals treatment              */
    int             vdw_modifier;            /* Modify the VdW interaction                   */
    realA            rvdw_switch;             /* Van der Waals switch range start (nm)        */
    realA            rvdw;                    /* Van der Waals cutoff (nm)	        */
    int             eDispCorr;               /* Perform Long range dispersion corrections    */
    realA            tabext;                  /* Extension of the table beyond the cut-off,   *
                                              * as well as the table length for 1-4 interac. */
    realA            shake_tol;               /* tolerance for shake				*/
    ...
    realA            dr_fc;                   /* force constant for ta_disre			*/
    ...
    realA            dr_tau;                  /* time constant for memory function in disres    */
    realA            orires_fc;               /* force constant for orientational restraints  */
    realA            orires_tau;              /* time constant for memory function in orires    */
    int             nstorireout;             /* frequency of writing tr(SD) to enx           */
    realA            em_stepsize;             /* The stepsize for updating			*/
    realA            em_tol;                  /* The tolerance				*/
    int             niter;                   /* Number of iterations for convergence of      */
                                             /* steepest descent in relax_shells             */
    realA            fc_stepsize;             /* Stepsize for directional minimization        */
                                             /* in relax_shells                              */
    ...
    realA            LincsWarnAngle;          /* If bond rotates more than %g degrees, warn   */
    int             nLincsIter;              /* Number of iterations in the final Lincs step */
    gmx_bool        bShakeSOR;               /* Use successive overrelaxation for shake      */
    realA            bd_fric;                 /* Friction coefficient for BD (amu/ps)         */
    gmx_int64_t     ld_seed;                 /* Random seed for SD and BD                    */
    int             nwall;                   /* The number of walls                          */
    int             wall_type;               /* The type of walls                            */
    realA            wall_r_linpot;           /* The potentail is linear for r<=wall_r_linpot */
    int             wall_atomtype[2];        /* The atom type for walls                      */
    realA            wall_density[2];         /* Number density for walls                     */
    realA            wall_ewald_zfac;         /* Scaling factor for the box for Ewald         */
    ...
    realA                     cos_accel;      /* Acceleration for viscosity calculation       */
    ...
    realA                     scalefactor;   /* factor for scaling the MM charges in QM calc.*/
...
void cmp_inputrec(FILE *fp, const t_inputrec *ir1, const t_inputrec *ir2, realA ftol, realA abstol);

void comp_pull_AB(FILE *fp, pull_params_t *pull, realA ftol, realA abstol);

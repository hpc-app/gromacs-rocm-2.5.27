...
typedef struct {
    ...
    realA *weight;  /**< Weights (use all 1 when weight==NULL) */
...
typedef struct {
    ...
    realA     init;                         /**< Initial reference displacement (nm) or (deg) */
    realA     rate;                         /**< Rate of motion (nm/ps) or (deg/ps) */
    realA     k;                            /**< Force constant (kJ/(mol nm^2) or kJ/(mol rad^2) for umbrella pull type, or kJ/(mol nm) or kJ/(mol rad) for constant force pull type */
    realA     kB;                           /**< Force constant for state B */
} t_pull_coord;
...
typedef struct pull_params_t {
    ...
    realA           cylinder_r;     /**< Radius of cylinder for dynamic COM (nm) */
    realA           constr_tol;     /**< Absolute tolerance for constraints in (nm) */

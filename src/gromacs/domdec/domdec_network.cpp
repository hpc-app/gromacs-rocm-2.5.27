...
void dd_sendrecv_real(const struct gmx_domdec_t gmx_unused *dd,
                      int gmx_unused ddimind, int gmx_unused direction,
                      realA gmx_unused *buf_s, int gmx_unused n_s,
                      realA gmx_unused *buf_r, int gmx_unused n_r)
{
#if GMX_MPI
    int        rank_s, rank_r;
    MPI_Status stat;
    ...
    if (n_s && n_r)
    {
        MPI_Sendrecv(buf_s, n_s*sizeof(realA), MPI_BYTE, rank_s, 0,
                     buf_r, n_r*sizeof(realA), MPI_BYTE, rank_r, 0,
                     dd->mpi_comm_all, &stat);
    }
    else if (n_s)
    {
        MPI_Send(    buf_s, n_s*sizeof(realA), MPI_BYTE, rank_s, 0,
                     dd->mpi_comm_all);
    }
    else if (n_r)
    {
        MPI_Recv(    buf_r, n_r*sizeof(realA), MPI_BYTE, rank_r, 0,
                     dd->mpi_comm_all, &stat);
    }

...
void dd_atom_spread_real(gmx_domdec_t *dd, realA v[])
{
    int                    nzone, nat_tot, n, d, p, i, j, at0, at1, zone;
    int                   *index, *cgindex;
    gmx_domdec_comm_t     *comm;
...
void dd_atom_sum_real(gmx_domdec_t *dd, realA v[])
{
    int                    nzone, nat_tot, n, d, p, i, j, at0, at1, zone;
    int                   *index, *cgindex;
    gmx_domdec_comm_t     *comm;
...
static void dd_distribute_dfhist(gmx_domdec_t *dd, df_history_t *dfhist)
{
    ...
    dd_bcast(dd, sizeof(realA), &dfhist->wl_delta);

    if (dfhist->nlambda > 0)
    {
        int nlam = dfhist->nlambda;
        dd_bcast(dd, sizeof(int)*nlam, dfhist->n_at_lam);
        dd_bcast(dd, sizeof(realA)*nlam, dfhist->wl_histo);
        dd_bcast(dd, sizeof(realA)*nlam, dfhist->sum_weights);
        dd_bcast(dd, sizeof(realA)*nlam, dfhist->sum_dg);
        dd_bcast(dd, sizeof(realA)*nlam, dfhist->sum_minvar);
        dd_bcast(dd, sizeof(realA)*nlam, dfhist->sum_variance);

        for (int i = 0; i < nlam; i++)
        {
            dd_bcast(dd, sizeof(realA)*nlam, dfhist->accum_p[i]);
            dd_bcast(dd, sizeof(realA)*nlam, dfhist->accum_m[i]);
            dd_bcast(dd, sizeof(realA)*nlam, dfhist->accum_p2[i]);
            dd_bcast(dd, sizeof(realA)*nlam, dfhist->accum_m2[i]);
            dd_bcast(dd, sizeof(realA)*nlam, dfhist->Tij[i]);
            dd_bcast(dd, sizeof(realA)*nlam, dfhist->Tij_empirical[i]);
        }
    }
}
                                ...
                                PaddedRVecVector *f)
{
    ...
    dd_bcast(dd, ((efptNR)*sizeof(realA)), state_local->lambda.data());
    dd_bcast(dd, sizeof(int), &state_local->fep_state);
    dd_bcast(dd, sizeof(realA), &state_local->veta);
    dd_bcast(dd, sizeof(realA), &state_local->vol0);
...
realA dd_cutoff_multibody(const gmx_domdec_t *dd)
{
    gmx_domdec_comm_t *comm;
    int                di;
    realA               r;
...
realA dd_cutoff_twobody(const gmx_domdec_t *dd)
{
    realA r_mb;
 ...
 */
static realA cellsize_min_dlb(gmx_domdec_comm_t *comm, int dim_ind, int dim)
{
    realA cellsize_min;
...
static realA grid_jump_limit(gmx_domdec_comm_t *comm, realA cutoff,
                            int dim_ind)
{
    realA grid_jump_limit;
...
static void set_slb_pme_dim_f(gmx_domdec_t *dd, int dim, realA **dim_f)
{
    gmx_domdec_comm_t *comm;
    int                i;
    ...
    for (i = 1; i < dd->nc[dim]; i++)
    {
        ...
        {
            (*dim_f)[i] = (realA)i/(realA)dd->nc[dim];
        }
    }
    (*dim_f)[dd->nc[dim]] = 1;
}
...
static void dd_cell_sizes_dlb_root_enforce_limits(gmx_domdec_t *dd,
                                                  ...
                                                  gmx_bool bUniform, gmx_int64_t step, realA cellsize_limit_f, int range[])
{
    gmx_domdec_comm_t *comm;
    int                ncd, i, j, nmin, nmin_old;
    gmx_bool           bLimLo, bLimHi;
    realA              *cell_size;
    realA               fac, halfway, cellsize_limit_f_i, region_size;
                                       ...
                                       gmx_bool bUniform, gmx_int64_t step)
{
    ...
    int                ncd, d1, i, pos;
    realA              *cell_size;
    realA               load_aver, load_i, imbalance, change, change_max, sc;
    realA               cellsize_limit_f, dist_min_f, dist_min_f_hard, space;
    realA               change_limit;
    realA               relax = 0.5;
    gmx_bool           bPBC;
    int                range[] = { 0, 0 };
...
static void distribute_dd_cell_sizes_dlb(gmx_domdec_t *dd,
                                         int d, int dim, realA *cell_f_row,
                                         const gmx_ddbox_t *ddbox)
{
    gmx_domdec_comm_t *comm;
    int                d1, pos;
     ...
     */
    MPI_Bcast(cell_f_row, DD_CELL_F_SIZE(dd, d)*sizeof(realA), MPI_BYTE,
...
static void print_cg_move(FILE *fplog,
                          ...
                          gmx_bool bHaveCgcmOld, realA limitd,
                          rvec cm_old, rvec cm_new, realA pos_d)
{
    gmx_domdec_comm_t *comm;
    char               buf[22];
...
#pragma omp parallel for num_threads(nthread) schedule(static)
    ...
    *ncg_stay_home = home_pos_cg;
     ...
     * and ncg_home and nat_home are not the realA count, since there are
...
static double force_flop_count(t_nrnb *nrnb)
{
    ...
    {
        /* To get closer to the realA timings, we half the count
...
static realA *get_slb_frac(FILE *fplog, const char *dir, int nc, const char *size_string)
{
    realA  *slb_frac, tot;
    int    i, n;
    double dbl;
...
static realA average_cellsize_min(gmx_domdec_t *dd, gmx_ddbox_t *ddbox)
{
    int  di, d;
    realA r;
...
{
    realA               r_bonded         = -1;
    realA               r_bonded_limit   = -1;
    const realA         tenPercentMargin = 1.1;
    gmx_domdec_comm_t *comm             = dd->comm;
     ...
     */
    constexpr realA c_chanceThatAtomMovesBeyondDomain = 1e-12;
    const realA     limitForAtomDisplacement          =
    ...
    if (options.numCells[XX] > 0)
    {
        ...
        realA acs = average_cellsize_min(dd, ddbox);
...
static void turn_on_dlb(FILE *fplog, t_commrec *cr, gmx_int64_t step)
{
    ...
    for (d = 0; d < dd->ndim; d++)
    {
        if (comm->root[d])
        {
            ...
            for (i = 0; i < nc; i++)
            {
                comm->root[d]->cell_f[i]    = i/(realA)nc;
                if (d > 0)
                {
                    comm->root[d]->cell_f_max0[i] =  i   /(realA)nc;
                    comm->root[d]->cell_f_min1[i] = (i+1)/(realA)nc;
                }
            }
...
static void print_dd_settings(FILE *fplog, gmx_domdec_t *dd,
                              ...
                              gmx_bool bDynLoadBal, realA dlb_scale,
                              const gmx_ddbox_t *ddbox)
{
    gmx_domdec_comm_t *comm;
    int                d;
...
static void set_ddgrid_parameters(FILE *fplog, gmx_domdec_t *dd, realA dlb_scale,
                                  const gmx_mtop_t *mtop, const t_inputrec *ir,
                                  const gmx_ddbox_t *ddbox)
{
    gmx_domdec_comm_t *comm;
...
void set_dd_dlb_max_cutoff(t_commrec *cr, realA cutoff)
{
    gmx_domdec_comm_t *comm;
...
typedef struct {
    realA c[DIM][4]; /* the corners for the non-bonded communication */
    realA cr0;       /* corner for rounding */
    realA cr1[4];    /* corners for rounding */
    realA bc[DIM];   /* corners for bounded communication */
    realA bcr1;      /* corner for rounding for bonded communication */
} dd_corners_t;
...
get_zone_pulse_cgs(gmx_domdec_t *dd,
                   ...
                   int dim, int dim_ind,
                   int dim0, int dim1, int dim2,
                   realA r_comm2, realA r_bcomm2,
                   matrix box,
                   ivec tric_dist,
                   rvec *normal,
                   realA skew_fac2_d, realA skew_fac_01,

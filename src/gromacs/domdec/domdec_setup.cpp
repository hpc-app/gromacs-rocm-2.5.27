...
realA comm_box_frac(const ivec dd_nc, realA cutoff, const gmx_ddbox_t *ddbox)
{
    ...
    for (i = 0; i < DIM; i++)
    {
        realA bt = ddbox->box_size[i]*ddbox->skew_fac[i];
...
static float comm_cost_est(realA limit, realA cutoff,
...
static realA optimize_ncells(FILE *fplog,
                            ...
                            const t_inputrec *ir,
                            gmx_domdec_t *dd,
                            realA cellsize_limit, realA cutoff,

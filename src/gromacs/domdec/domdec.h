...
struct DomdecOptions
{
    ...
    realA              minimumCommunicationRange;
    //! Communication range for atom involved in constraints (P-LINCS) (nm).
    realA              constraintCommunicationRange;
 ...
 */
void set_dd_dlb_max_cutoff(struct t_commrec *cr, realA cutoff);
...
/*! \brief Communicate a realA for each atom to the neighboring cells. */
void dd_atom_spread_real(struct gmx_domdec_t *dd, realA v[]);

/*! \brief Sum the contributions to a realA for each atom over the neighboring cells. */
void dd_atom_sum_real(struct gmx_domdec_t *dd, realA v[]);
...
realA comm_box_frac(const ivec dd_nc, realA cutoff, const gmx_ddbox_t *ddbox);
 ...
 */
realA dd_choose_grid(FILE *fplog,
                    ...
                    realA cellsize_limit, realA cutoff_dd,

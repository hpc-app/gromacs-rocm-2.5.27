...
typedef struct
{
    ...
    realA   *stpsz;   /* stepsizes (per eigenvector)    */
    rvec  **vec;     /* eigenvector components         */
    realA   *xproj;   /* instantaneous x projections    */
    realA   *fproj;   /* instantaneous f projections    */
    realA    radius;  /* instantaneous radius           */
    realA   *refproj; /* starting or target projections */
...
typedef struct gmx_edx
{
    ...
    rvec          *x_old;         /* Last positions which have the correct PBC
                                     ...
                                     be made whole                            */
    realA          *m;             /* masses                                   */
    realA           mtot;          /* total mass (only used in sref)           */
    realA          *sqrtm;         /* sqrt of the masses used for mass-
...
typedef struct edpar
{
    ...
    realA                slope;        /* minimal slope in acceptance radexp   */
...
static realA rmsd_from_structure(rvec *x, struct gmx_edx *s);
 ...
 */
static realA projectx(t_edpar *edi, rvec *xcoll, rvec *vec)
{
    int  i;
    realA proj = 0.0;
...
static realA calc_radius(t_eigvec *vec)
{
    int  i;
    realA rad = 0.0;
 ...
 **********************************************************************************
   ...
   and call
        get_flood_energies(realA Vfl[],int nnames);
...
static void write_edo_flood(t_edpar *edi, FILE *fp, realA rmsd)
{
    int i;
...
static realA flood_energy(t_edpar *edi, gmx_int64_t step)
{
...
static void init_flood(t_edpar *edi, gmx_edsam_t ed, realA dt)
{
    int i;
...
static void get_flood_energies(t_edpar *edi, realA Vfl[], int nnames)
{
...
static realA read_checked_edreal(FILE *file, const char *label)
{
    char   line[STRLEN+1];
    double rdum;
    ...
    return (realA) rdum; /* always read as double and convert to single */
}
...
static realA rmsd_from_structure(rvec           *x,  /* The positions under consideration */
...
static void write_edo(t_edpar *edi, FILE *fp, realA rmsd)
{
    int i;
              ...
              gmx_edsam_t       ed)
{
    ...
    struct t_do_edsam *buf;
    t_edpar           *edi;
    realA               rmsdev    = -1;    /* RMSD from reference structure prior to applying the constraints */

...
gmx_bool bshakef(FILE           *log,          /* Log file			*/
                 ...
                 realA * const    lagr,         /* The Lagrange multipliers     */
                 realA            lambda,       /* FEP lambda                   */
                 realA           *dvdlambda,    /* FEP force                    */
                 realA            invdt,        /* 1/delta_t                    */
...
void csettle(gmx_settledata_t    settled,          /* The SETTLE structure */
             ...
             const realA          x[],              /* Reference coordinates */
             realA                xprime[],         /* New coords, to be settled */
             realA                invdt,            /* 1/delta_t */
             realA               *v,                /* Also constrain v if v!=NULL */
...
void cshake(const int iatom[], int ncon, int *nnit, int maxnit,
            const realA dist2[], realA xp[], const realA rij[], const realA m2[], realA omega,
            const realA invmass[], const realA tt[], realA lagr[], int *nerror);
/* Regular iterative shake */

void crattle(int iatom[], int ncon, int *nnit, int maxnit,
             realA dist2[], realA vp[], realA rij[], realA m2[], realA omega,
             realA invmass[], realA tt[], realA lagr[], int *nerror, realA invdt);
...
realA *constr_rmsd_data(gmx_constr_t constr);
/* Return the data for determining constraint RMS relative deviations.
 * Returns NULL when LINCS is not used.
 */

realA constr_rmsd(gmx_constr_t constr);
/* Return the RMSD of the constraint */

realA *lincs_rmsd_data(gmx_lincsdata_t lincsd);
/* Return the data for determining constraint RMS relative deviations */

realA lincs_rmsd(gmx_lincsdata_t lincsd);
/* Return the RMSD of the constraint */
...
void set_lincs_matrix(gmx_lincsdata_t li, const realA *invmass, realA lambda);
/* Sets the elements of the LINCS constraint coupling matrix */

realA constr_r_max(FILE *fplog, const gmx_mtop_t *mtop, const t_inputrec *ir);

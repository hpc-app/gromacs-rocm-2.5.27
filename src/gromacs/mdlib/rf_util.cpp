                        ...
                        realA lambda, realA *dvdlambda)
{
    ...
    const realA *chargeA, *chargeB;
    realA        ek, ec, L1, qiA, qiB, qqA, qqB, qqL, v;
    rvec        dx, df;
    int        *AA;
    ivec        dt;
    int         start = 0;
...
void calc_rffac(FILE *fplog, int eel, realA eps_r, realA eps_rf, realA Rc, realA Temp,
                realA zsq, matrix box,
                realA *krf, realA *crf)
{
    /* Compute constants for Generalized reaction field */
    realA   kappa, k1, k2, I, rmin;
    realA   vol = 0;
    ...
    if (EEL_RF(eel))
    {
        ...
        {
            k1   = 1 + kappa*Rc;
            k2   = eps_rf*gmx::square((realA)(kappa*Rc));

            *krf = ((eps_rf - eps_r)*k1 + 0.5*k2)/((2*eps_rf + eps_r)*k1 + k2)/(Rc*Rc*Rc);
        }
        *crf   = 1/Rc + *krf*Rc*Rc;
        // Make sure we don't lose resolution in pow() by casting realA arg to double

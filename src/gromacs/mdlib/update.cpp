                       ...
                       const rvec * gmx_restrict f)
{
    ...
    for (int a = start; a < nrend; a++)
    {
        ...
        for (int d = 0; d < DIM; d++)
        {
             ...
             */
            realA vNew = lambdaGroup*v[a][d] + f[a][d]*invMassPerDim[a][d]*dt;
...
updateMDLeapfrogSimpleSimd(int                       start,
                           int                       nrend,
                           realA                      dt,
                           const realA * gmx_restrict invMass,
                           const t_grp_tcstat      * tcstat,
                           const rvec * gmx_restrict x,
                           rvec       * gmx_restrict xprime,
                           rvec       * gmx_restrict v,
                        ...
                        const matrix                M)
{
    ...
    for (int n = start; n < nrend; n++)
    {
        ...
        realA lg = tcstat[gt].lambda;

        rvec vRel;
        realA cosineZ, vCosine;
...
#pragma warning( disable : 280 )
        ...
        switch (accelerationType)
        {
            ...
            case AccelerationType::cosine:
                cosineZ = std::cos(x[n][ZZ]*static_cast<realA>(M_PI)/box[ZZ][ZZ]);
                         ...
                         const matrix                M)
{
    ...
    realA dtPressureCouple   = (doParrinelloRahman ? ir->nstpcouple*dt : 0);
...
static void do_update_vv_vel(int start, int nrend, realA dt,
                             rvec accel[], ivec nFreeze[], realA invmass[],
                             unsigned short ptype[], unsigned short cFREEZE[],
                             unsigned short cACC[], rvec v[], const rvec f[],
                             gmx_bool bExtended, realA veta, realA alpha)
{
    int    gf = 0, ga = 0;
    int    n, d;
    realA   g, mv1, mv2;
...
static void do_update_vv_pos(int start, int nrend, realA dt,
                             ivec nFreeze[],
                             unsigned short ptype[], unsigned short cFREEZE[],
                             const rvec x[], rvec xprime[], rvec v[],
                             gmx_bool bExtended, realA veta)
{
    int    gf = 0;
    int    n, d;
    realA   g, mr1, mr2;
...
static gmx_stochd_t *init_stochd(const t_inputrec *ir)
{
    ...
    else if (ETC_ANDERSEN(ir->etc))
    {
        ...
        for (int gt = 0; gt < ngtc; gt++)
        {
            realA reft = std::max<realA>(0, opts->ref_t[gt]);
...
void update_temperature_constants(gmx_update_t *upd, const t_inputrec *ir)
{
    ...
    if (ir->eI == eiSD1)
    {
        for (int gt = 0; gt < ir->opts.ngtc; gt++)
        {
            realA kT = BOLTZ*ir->opts.ref_t[gt];
...
static void do_update_sd1(gmx_stochd_t *sd,
                          int start, int nrend, realA dt,
                          rvec accel[], ivec nFreeze[],
                          realA invmass[], unsigned short ptype[],
                          ...
                          gmx_int64_t step, int seed, int* gatindex)
{
    ...
    gmx::TabulatedNormalDistribution<realA, 14> dist;
...
static void do_update_bd(int start, int nrend, realA dt,
                         ivec nFreeze[],
                         realA invmass[], unsigned short ptype[],
                         unsigned short cFREEZE[], unsigned short cTC[],
                         const rvec x[], rvec xprime[], rvec v[],
                         const rvec f[], realA friction_coefficient,
                         realA *rf, gmx_int64_t step, int seed,
                         int* gatindex)
{
...

{
    ...
    {
        realA dttc = inputrec->nsttcouple*inputrec->delta_t;
                                       ...
                                       gmx_bool          bInitStep)
{
    ...
    {
        realA dtpc = inputrec->nstpcouple*inputrec->delta_t;
...
void update_constraints(FILE                          *fplog,
                        gmx_int64_t                    step,
                        realA                          *dvdlambda, /* the contribution to be added to the bonded interactions */
                        ...
                        gmx_bool                       bCalcVir)
{
    ...
    /* Cast delta_t from double to realA to make the integrators faster.
     ...
     * For integration dt the accuracy of realA suffices, since with
     * integral += dt*integrand the increment is nearly always (much) smaller
     * than the integral (and the integrand has realA precision).
     */
    realA dt     = inputrec->delta_t;
                                      ...
                                      gmx_update_t     *upd)
{
    ...
    /* Cast to realA for faster code, no loss in precision (see comment above) */
    realA dt     = inputrec->delta_t;
    ...
    switch (inputrec->epc)
    {
        ...
        case (epcBERENDSEN):
            ...
            {
                realA   dtpc = inputrec->nstpcouple*dt;
                matrix mu;
        ...
        try
        {
            ...
            switch (inputrec->eI)
            {
                ...
                case (eiVVAK):
                {
                    ...
                    realA alpha = 1.0 + DIM/static_cast<realA>(inputrec->opts.nrdf[0]);
                    switch (UpdatePart)
                    {
                        case etrtVELOCITY1:
                        case etrtVELOCITY2:
...
void correct_ekin(FILE *log, int start, int end, rvec v[], rvec vcm, realA mass[],
                  realA tmass, tensor ekin)
{
    /*
                                            ...
                                            t_mdatoms *md, t_state *state, gmx_update_t *upd, gmx_constr_t constr)
{
    ...
    realA rate = (ir->delta_t)/ir->opts.tau_t[0];

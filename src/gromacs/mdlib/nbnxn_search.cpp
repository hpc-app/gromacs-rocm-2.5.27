 ...
 */
static void get_cell_range(realA b0, realA b1,
                           int nc, realA c0, realA s, realA invs,
                           realA d2, realA r2, int *cf, int *cl)
{
...
clusterpair_in_range(const nbnxn_list_work_t *work,
                     int si,
                     int csj, int stride, const realA *x_j,
                     realA rlist2)
{
    ...
    for (int i = 0; i < c_nbnxnGpuClusterSize; i++)
    {
        ...
        for (int j = 0; j < c_nbnxnGpuClusterSize; j++)
        {
            ...
            realA d2 = gmx::square(x_i[i0  ] - x_j[j0  ]) + gmx::square(x_i[i0+1] - x_j[j0+1]) + gmx::square(x_i[i0+2] - x_j[j0+2]);
    ...
    const realA *x_i        = work->x_ci_simd;
...
static void print_nblist_statistics_simple(FILE *fp, const nbnxn_pairlist_t *nbl,
                                           const nbnxn_search_t nbs, realA rl)
{
                      ...
                      int * gmx_restrict        numDistanceChecks)
{
    ...
    {
        realA d2  = subc_bb_dist2(0, bb_ci, jclusterFirst, gridj->bb);
        *numDistanceChecks += 2;
    ...
    {
        realA d2  = subc_bb_dist2(0, bb_ci, jclusterLast, gridj->bb);
        *numDistanceChecks += 2;
...
static void make_cluster_list_supersub(const nbnxn_grid_t *gridi,
                                       ...
                                       int stride, const realA *x,
                                       realA rlist2, float rbb2,
...
static realA minimum_subgrid_size_xy(const nbnxn_grid_t *grid)
{
    if (grid->bSimple)
    {
...
static realA effective_buffer_1x1_vs_MxN(const nbnxn_grid_t *gridi,
                                        const nbnxn_grid_t *gridj)
{
    const realA eff_1x1_buffer_fac_overest = 0.1;
...
static realA nbnxn_rlist_inc_outside_fac = 0.6;

/* Due to the cluster size the effective pair-list is longer than
 * that of a simple atom pair-list. This function gives the extra distance.
 */
realA nbnxn_get_rlist_effective_inc(int cluster_size_j, realA atom_density)
{
    int  cluster_size_i;
    realA vol_inc_i, vol_inc_j;
...
static realA nonlocal_vol2(const struct gmx_domdec_zones_t *zones, rvec ls, realA r)
{
    realA cl, ca, za;
    realA vold_est;
    realA vol2_est_tot;
                                ...
                                float                *nsubpair_tot_est)
{
    ...
    const nbnxn_grid_t *grid;
    rvec                ls;
    realA                r_eff_sup, vol_est, nsp_est, nsp_est_nl;
    ...
    realA diagonal = std::sqrt(ls[XX]*ls[XX] + ls[YY]*ls[YY] + ls[ZZ]*ls[ZZ]);
    ...
    if (LOCAL_I(iloc))
    {
         ...
         */
        nsp_est = std::max(nsp_est, grid->nsubc_tot*static_cast<realA>(14));
                                     ...
                                     t_nblist *nbl_fep)
{
    ...
    const int        *flags_i;
    realA              bx0, bx1, by0, by1, bz0, bz1;
    realA              bz1_frac;
    realA              d2cx, d2z, d2z_cx, d2z_cy, d2zx, d2zxy, d2xy;
...
static bool checkRebalanceSimpleLists(const nbnxn_pairlist_set_t *listSet)
{
     ...
     */
    const realA rebalanceTolerance = 1.03;

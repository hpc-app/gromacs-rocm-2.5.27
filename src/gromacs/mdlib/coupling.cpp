...
static void NHC_trotter(t_grpopts *opts, int nvar, gmx_ekindata_t *ekind, realA dtfull,
                        double xi[], double vxi[], double scalefac[], realA *veta, t_extmass *MassQ, gmx_bool bEkinAveVel)
...
/* if scalefac is NULL, we are doing the NHC of the barostat */
    ...
    for (i = 0; i < nvar; i++)
    {
        ...
        if (bBarostat)
        {
            ...
            reft  = std::max<realA>(0, opts->ref_t[0]);
        ...
        else
        {
            ...
            nd     = opts->nrdf[i];
            reft   = std::max<realA>(0, opts->ref_t[i]);
            if (bEkinAveVel)
            {
...
static void boxv_trotter(t_inputrec *ir, realA *veta, realA dt, tensor box,
                         gmx_ekindata_t *ekind, tensor vir, realA pcorr, t_extmass *MassQ)
{
...
realA calc_pres(int ePBC, int nwall, matrix box, tensor ekin, tensor vir,
               tensor pres)
{
    int  n, m;
    realA fac;
...
realA calc_temp(realA ekin, realA nrdf)
{
    if (nrdf > 0)
    {
...
void parrinellorahman_pcoupl(FILE *fplog, gmx_int64_t step,
                             const t_inputrec *ir, realA dt, const tensor pres,
                             ...
                             tensor M, matrix mu, gmx_bool bFirstStep)
{
     ...
     * the scaled frame to the realA frame, i.e. the TRANSPOSE of the box.
    ...
    realA   vol = box[XX][XX]*box[YY][YY]*box[ZZ][ZZ];
    realA   atot, arel, change;
    ...
    if (!bFirstStep)
    {
        ...
        if (ir->epct == epctSURFACETENSION)
        {
             ...
             */
            realA xy_pressure = 0.5*(pres[XX][XX]+pres[YY][YY]);
            for (int d = 0; d < ZZ; d++)
            {
...
void berendsen_tcoupl(const t_inputrec *ir, const gmx_ekindata_t *ekind, realA dt,
                      std::vector<double> &therm_integral)
{
    ...
    for (int i = 0; (i < opts->ngtc); i++)
    {
        ...
        {
            realA reft               = std::max<realA>(0, opts->ref_t[i]);
            realA lll                = std::sqrt(1.0 + (dt/opts->tau_t[i])*(reft/T-1.0));
            ekind->tcstat[i].lambda = std::max<realA>(std::min<realA>(lll, 1.25), 0.8);
        }
        else
        {
...
void andersen_tcoupl(t_inputrec *ir, gmx_int64_t step,
                     const t_commrec *cr, const t_mdatoms *md, t_state *state, realA rate, const gmx_bool *randomize, const realA *boltzfac)
{
    ...
    gmx::UniformRealDistribution<realA>         uniformDist;
    gmx::TabulatedNormalDistribution<realA, 14> normalDist;
...
void nosehoover_tcoupl(t_grpopts *opts, gmx_ekindata_t *ekind, realA dt,
...
static realA energyNoseHoover(const t_inputrec *ir, const t_state *state, const t_extmass *MassQ)
{
    realA energy = 0;
    ...
    for (int i = 0; i < ir->opts.ngtc; i++)
    {
        ...
        realA          reft  = std::max<realA>(ir->opts.ref_t[i], 0);
        realA          kT    = BOLTZ * reft;
...
static realA energyPressureMTTK(const t_inputrec *ir, const t_state *state, const t_extmass *MassQ)
{
    realA energy = 0;
    ...
    for (int i = 0; i < state->nnhpres; i++)
    {
        ...
        realA    reft  = std::max<realA>(ir->opts.ref_t[0], 0.0); /* using 'System' temperature */
        realA    kT    = BOLTZ * reft;
...
static realA energyVrescale(const t_inputrec *ir, const t_state *state)
{
    realA energy = 0;
    for (int i = 0; i < ir->opts.ngtc; i++)
    {
...
realA NPT_energy(const t_inputrec *ir, const t_state *state, const t_extmass *MassQ)
{
    realA energyNPT = 0;
...
static realA vrescale_sumnoises(realA                            nn,
                               gmx::ThreeFry2x64<>            *rng,
                               gmx::NormalDistribution<realA>  *normalDist)
{
/*
 ...
 */
    const realA                     ndeg_tol = 0.0001;
    realA                           r;
    gmx::GammaDistribution<realA>   gammaDist(0.5*nn, 1.0);
    ...
    {
        /* Use a gamma distribution for any realA nn > 2 */
        r = 2.0*gammaDist(*rng);
    }
...
static realA vrescale_resamplekin(realA kk, realA sigma, realA ndeg, realA taut,
...
void vrescale_tcoupl(t_inputrec *ir, gmx_int64_t step,
                     gmx_ekindata_t *ekind, realA dt,
                     double therm_integral[])
{
    t_grpopts *opts;
    int        i;
    realA       Ek, Ek_ref1, Ek_ref, Ek_new;
...
void update_annealing_target_temp(t_inputrec *ir, realA t, gmx_update_t *upd)
{
    int  i, j, n, npoints;
    realA pert, thist = 0, x;

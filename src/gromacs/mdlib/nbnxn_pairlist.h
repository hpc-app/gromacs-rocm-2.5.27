...
struct NbnxnListParameters
{
     ...
     */
    NbnxnListParameters(realA rlist) :
    ...
    realA rlistOuter;        //!< Cut-off of the larger, outer pair-list
    realA rlistInner;        //!< Cut-off of the smaller, inner pair-list
 ...
 * to avoid cache invalidation of the realA contents
...
typedef struct nbnxn_pairlist_t {
    ...
    realA                    rlist;       /* The radius for constructing the list     */
...
typedef struct {
    realA *f;      /* f, size natoms*fstride                             */
    realA *fshift; /* Shift force array, size SHIFTS*DIM                 */
    int   nV;     /* The size of *Vvdw and *Vc                          */
    realA *Vvdw;   /* Temporary Van der Waals group energy storage       */
    realA *Vc;     /* Temporary Coulomb group energy storage             */
    int   nVS;    /* The size of *VSvdw and *VSc                        */
    realA *VSvdw;  /* Temporary SIMD Van der Waals group energy storage  */
    realA *VSc;    /* Temporary SIMD Coulomb group energy storage        */
 ...
 * Currently the block size is NBNXN_BUFFERFLAG_SIZE*3*sizeof(realA)=192 bytes.
 */
#if GMX_DOUBLE
...
typedef struct nbnxn_atomdata_t {
    ...
    realA                    *nbfp;            /* Lennard-Jones 6*C6 and 12*C12 params, size ntype^2*2 */
    int                      comb_rule;       /* Combination rule, see enum above                   */
    realA                    *nbfp_comb;       /* LJ parameter per atom type, size ntype*2           */
    realA                    *nbfp_aligned;    /* As nbfp, but with an alignment (stride) suitable
                                               * for the present SIMD architectures
                                               */
    int                      natoms;          /* Number of atoms                                    */
    int                      natoms_local;    /* Number of local atoms                           */
    int                     *type;            /* Atom types                                         */
    realA                    *lj_comb;         /* LJ parameters per atom for combining for pairs     */
    int                      XFormat;         /* The format of x (and q), enum                      */
    int                      FFormat;         /* The format of f, enum                              */
    realA                    *q;               /* Charges, can be NULL if incorporated in x          */
    ...
    realA                    *x;               /* x and possibly q, size natoms*xstride              */
     ...
     */
    realA                    *simd_4xn_diagonal_j_minus_i;
    realA                    *simd_2xnn_diagonal_j_minus_i;
    ...
    realA                    *simd_interaction_array;  /* Array of masks needed for exclusions */

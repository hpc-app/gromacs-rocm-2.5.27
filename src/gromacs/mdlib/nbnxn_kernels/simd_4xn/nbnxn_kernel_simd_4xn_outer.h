...

{
    ...
    SimdReal               invtsp_S;
    const realA      *      tab_coul_F;
#ifndef TAB_FDV0
    const realA gmx_unused *tab_coul_V;
#endif
#ifdef CALC_ENERGIES
    SimdReal   mhalfsp_S;
#endif
...
#endif
#ifdef LJ_EWALD_GEOM
    realA              lj_ewaldcoeff2, lj_ewaldcoeff6_6;
    ...
    realA      *nbfp_ptr    = nbat->nbfp_aligned;
    ...
    /* Here we cast the exclusion filters from unsigned * to int * or realA *.
...
#else
    filter_S0 = load<SimdBitMask>(reinterpret_cast<const realA *>(exclusion_filter + 0*UNROLLJ));
    filter_S1 = load<SimdBitMask>(reinterpret_cast<const realA *>(exclusion_filter + 1*UNROLLJ));
    filter_S2 = load<SimdBitMask>(reinterpret_cast<const realA *>(exclusion_filter + 2*UNROLLJ));
    filter_S3 = load<SimdBitMask>(reinterpret_cast<const realA *>(exclusion_filter + 3*UNROLLJ));
#endif
...
#ifdef FIX_LJ_C
    alignas(GMX_SIMD_ALIGNMENT) realA  pvdw_c6[2*UNROLLI*UNROLLJ];
    realA *pvdw_c12 = pvdw_c6 + UNROLLI*UNROLLJ;
...
#else
        const realA *nbfp0     = nbfp_ptr + type[sci  ]*nbat->ntype*c_simdBestPairAlignment;
        const realA *nbfp1     = nbfp_ptr + type[sci+1]*nbat->ntype*c_simdBestPairAlignment;
        const realA *nbfp2     = nullptr, *nbfp3 = nullptr;
        if (!half_LJ)
        {
...
#undef CALC_LJ
        ...
        realA fShiftX = reduceIncr4ReturnSum(f+scix, fix_S0, fix_S1, fix_S2, fix_S3);
        realA fShiftY = reduceIncr4ReturnSum(f+sciy, fiy_S0, fiy_S1, fiy_S2, fiy_S3);
        realA fShiftZ = reduceIncr4ReturnSum(f+sciz, fiz_S0, fiz_S1, fiz_S2, fiz_S3);

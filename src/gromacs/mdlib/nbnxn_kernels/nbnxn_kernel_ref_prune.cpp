                       ...
                       realA                       rlistInner)
{
    ...
    const realA       * gmx_restrict shiftvec = shift_vec[0];
    const realA       * gmx_restrict x        = nbat->x;

    const realA                      rlist2   = rlistInner*rlistInner;
        ...
        for (int cjind = ciEntry->cj_ind_start; cjind < ciEntry->cj_ind_end; cjind++)
        {
            ...
            for (int i = 0; i < c_iUnroll && !isInRange; i++)
            {
                for (int j = 0; j < c_jUnroll; j++)
                {
                    ...
                    realA dx  = xi[i*c_xiStride + XX] - x[aj*c_xStride + XX];
                    realA dy  = xi[i*c_xiStride + YY] - x[aj*c_xStride + YY];
                    realA dz  = xi[i*c_xiStride + ZZ] - x[aj*c_xStride + ZZ];

                    realA rsq = dx*dx + dy*dy + dz*dz;

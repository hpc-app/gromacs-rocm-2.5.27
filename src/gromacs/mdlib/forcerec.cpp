...
#ifdef DEBUG
static void pr_nbfp(FILE *fp, realA *nbfp, gmx_bool bBHAM, int atnr)
{
    int i, j;
...
static realA *mk_nbfp(const gmx_ffparams_t *idef, gmx_bool bBHAM)
{
    realA *nbfp;
    int   i, j, k, atnr;
...
static realA *make_ljpme_c6grid(const gmx_ffparams_t *idef, t_forcerec *fr)
{
    int        i, j, k, atnr;
    realA       c6, c6i, c6j, c12i, c12j, epsi, epsj, sigmai, sigmaj;
    realA      *grid;
...
static realA *mk_nbfp_combination_rule(const gmx_ffparams_t *idef, int comb_rule)
{
    realA      *nbfp;
    int        i, j, atnr;
    realA       c6i, c6j, c12i, c12j, epsi, epsj, sigmai, sigmaj;
    realA       c6, c12;
                 ...
                 int                    *cg_sp)
{
    ...
    gmx_bool              has_vdw[4];
    gmx_bool              match;
    realA                  tmp_charge[4]  = { 0.0 }; /* init to zero to make gcc4.8 happy */
...
static realA calcBuckinghamBMax(FILE *fplog, const gmx_mtop_t *mtop)
{
...
static void make_nbf_tables(FILE *fp,
                            const interaction_const_t *ic, realA rtab,
...
static realA cutoff_inf(realA cutoff)
{
    if (cutoff == 0)
    {
                                   ...
                                   interaction_const_t *ic)
{
    ...
    {
        realA crc2       = gmx::square(ic->ewaldcoeff_lj*ic->rvdw);
...
static void force_switch_constants(realA p,
                                   realA rsw, realA rc,
                                   shift_consts_t *sc)
{
...
static void potential_switch_constants(realA rsw, realA rc,
                                       switch_consts_t *sc)
{

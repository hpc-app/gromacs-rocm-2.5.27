...
static void copy_energy(t_mdebin *md, realA e[], realA ecpy[])
{
    int i, j;
...
{
    int    i, j, k, kk, n, gid;
    realA   crmsd[2], tmp6[6];
    realA   bs[NTRICLBOXS], vol, dens, pv, enthalpy;
    realA   eee[egNR];
    realA   ecopy[F_NRE];
    double store_dhdl[efptNR];
    realA   store_energy = 0;
    realA   tmp;
                ...
                gmx::Awh *awh)
{
    ...
    switch (mode)
    {
        case eprNORMAL:
            ...
            /* Optional additional old-style (realA-only) blocks. */
            for (i = 0; i < enxNR; i++)
            {
                nr[i] = 0;
            }

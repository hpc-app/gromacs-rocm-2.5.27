...
typedef struct
{
    realA  md1; // -V' at the cutoff
    realA  d2;  //  V'' at the cutoff
    realA  md3; // -V''' at the cutoff
} pot_derivatives_t;
...
static realA getMass(const t_atoms &atoms,
                    int            atomIndex,
                    bool           setMassesToOne)
{
    if (!setMassesToOne)
    ...
    for (mb = 0; mb < mtop->nmolblock; mb++)
    {
        ...
        for (ft = F_CONSTR; ft <= F_CONSTRNC; ft++)
        {
            ...
            for (i = 0; i < il->nr; i += 1+NRAL(ft))
            {
                ...
                a1         = il->iatoms[i+1];
                a2         = il->iatoms[i+2];
                realA mass1 = getMass(*atoms, a1, setMassesToOne);
                realA mass2 = getMass(*atoms, a2, setMassesToOne);
 ...
 */
void constrained_atom_sigma2(realA                                 kT_fac,
                             const atom_nonbonded_kinetic_prop_t *prop,
                             realA                                *sigma2_2d,
                             realA                                *sigma2_3d)
{
    ...
    realA massFraction = prop->con_mass/(prop->mass + prop->con_mass);
    realA sigma2_rot   = kT_fac*massFraction/prop->mass;

    /* The distance from the atom to the COM, i.e. the rotational arm */
    realA comDistance  = prop->con_len*massFraction;

    /* The variance relative to the arm */
    realA sigma2_rel   = sigma2_rot/gmx::square(comDistance);
    ...
    const realA a = 1.0/3.0;
    const realA b = 2.0/45.0;
...
static void get_atom_sigma2(realA                                 kT_fac,
                            const atom_nonbonded_kinetic_prop_t *prop,
                            realA                                *sigma2_2d,
                            realA                                *sigma2_3d)
{
...
static void approx_2dof(realA s2, realA x, realA *shift, realA *scale)
{
...
static realA energyDriftAtomPair(bool isConstrained_i,
                                bool isConstrained_j,
                                realA s2, realA s2i_2d, realA s2j_2d,
                                realA r_buffer,
...
static realA energyDrift(const verletbuf_atomtype_t *att, int natt,
                        const gmx_ffparams_t *ffp,
                        realA kT_fac,
                        const pot_derivatives_t *ljDisp,
                        const pot_derivatives_t *ljRep,
                        const pot_derivatives_t *elec,
                        realA rlj, realA rcoulomb,
                        realA rlist, realA boxvol)
{
    double drift_tot = 0;
    ...
    for (int i = 0; i < natt; i++)
    {
        ...
        for (int j = i; j < natt; j++)
        {
            ...
            realA s2 = s2i_2d + s2i_3d + s2j_2d + s2j_3d;

            // Set -V', V'' and -V''' at the cut-off for LJ */
            realA              c6  = ffp->iparams[prop_i->type*ffp->atnr + prop_j->type].lj.c6;
            realA              c12 = ffp->iparams[prop_i->type*ffp->atnr + prop_j->type].lj.c12;
            pot_derivatives_t lj;
            lj.md1 = c6*ljDisp->md1 + c12*ljRep->md1;
            lj.d2  = c6*ljDisp->d2  + c12*ljRep->d2;
            lj.md3 = c6*ljDisp->md3 + c12*ljRep->md3;

            realA pot_lj = energyDriftAtomPair(prop_i->bConstr, prop_j->bConstr,
                                              s2, s2i_2d, s2j_2d,
                                              rlist - rlj,
                                              &lj);
            ...
            realA pot_q  = energyDriftAtomPair(prop_i->bConstr, prop_j->bConstr,
                                              s2, s2i_2d, s2j_2d,
                                              rlist - rcoulomb,
                                              &elec_qq);
...
static realA surface_frac(int cluster_size, realA particle_distance, realA rlist)
{
    realA d, area_rel;
 ...
 */
static realA md3_force_switch(realA p, realA rswitch, realA rc)
{
    /* The switched force function is:
     * p*r^-(p+1) + a*(r - rswitch)^2 + b*(r - rswitch)^3
     */
    realA a, b;
    realA md3_pot, md3_sw;
...
static realA maxReferenceTemperature(const t_inputrec &ir)
{
 ...
 */
static realA displacementVariance(const t_inputrec &ir,
                                 realA              temperature,
                                 realA              timePeriod)
{
    realA kT_fac;
 ...
 */
static realA maxSigma(realA                        kT_fac,
                     int                         natt,
                     const verletbuf_atomtype_t *att)
{
    assert(att);
    realA smallestMass = att[0].prop.mass;
...
void calc_verlet_buffer_size(const gmx_mtop_t *mtop, realA boxvol,
                             const t_inputrec *ir,
                             int               nstlist,
                             int               list_lifetime,
                             realA reference_temperature,
                             ...
                             realA *rlist)
{
    ...
    realA                  particle_distance;
    realA                  nb_clust_frac_pairs_not_in_list_at_cutoff;
     ...
     * In most cases the realA drift differs by less than a factor 2.
     */
    ...
    realA              repPow = mtop->ffparams.reppow;

    if (ir->vdwtype == evdwCUT)
    {
        realA sw_range, md3_pswf;
    ...
    {
        realA b     = calc_ewaldcoeff_lj(ir->rvdw, ir->ewald_rtol_lj);
        realA r     = ir->rvdw;
        realA br    = b*r;
        realA br2   = br*br;
        realA br4   = br2*br2;
     ...
     */
    const realA kT_fac = displacementVariance(*ir, reference_temperature,
 ...
 */
static realA minCellSizeFromPairlistBuffer(const t_inputrec &ir)
{
    return ir.rlist - std::max(ir.rvdw, ir.rcoulomb);
}

realA minCellSizeForAtomDisplacement(const gmx_mtop_t &mtop,
                                    const t_inputrec &ir,
                                    realA              chanceRequested)
{
     ...
     */
    const realA            temperature = maxReferenceTemperature(ir);
    ...
    const realA kT_fac = displacementVariance(ir, temperature,
                                             ir.nstlist*ir.delta_t);

    /* Resolution of the cell size */
    realA resolution = 0.001;
    ...
    while (ib1 - ib0 > 1)
    {
        ...
        for (int i = 0; i < natt; i++)
        {
            ...
            realA chancePerAtom = energyDriftAtomPair(propAtom.bConstr, false,
                                                     s2_2d + s2_3d, s2_2d, 0,
                                                     cellSize,
                                                     &boundaryInteraction);
            ...
            if (propAtom.bConstr)
            {
                 ...
                 */
                realA massFraction = propAtom.con_mass/(propAtom.mass + propAtom.con_mass);
                realA comDistance  = propAtom.con_len*massFraction;

                realA chanceWithMaxDistance =

...
nbnxn_atomdata_init_simple_exclusion_masks(nbnxn_atomdata_t *nbat)
{
     ...
     * realA SIMD registers (together with a cast).
     * In single precision this means the realA and integer SIMD registers
     * are of equal size.
     */
    ...
    const realA simdFalse =  0.0;
    const realA simdTrue  =  1.0;
    realA      *simd_interaction_array;
...
static void copy_lj_to_nbat_lj_comb(const realA *ljparam_type,
        ...
        try
        {
            for (int g = g0; g < g1; g++)
            {
                ...
                for (int cxy = cxy0; cxy < cxy1; cxy++)
                {
                    ...
                    {
                        /* We fill only the realA particle locations.
...
static void
nbnxn_atomdata_clear_reals(realA * gmx_restrict dest,
                           int i0, int i1)
{
    for (int i = i0; i < i1; i++)
    {
...
nbnxn_atomdata_reduce_reals(realA * gmx_restrict dest,
                            gmx_bool bDestSet,
                            realA ** gmx_restrict src,
                            int nsrc,
                            int i0, int i1)
{
    if (bDestSet)
...
gmx_unused static void
nbnxn_atomdata_reduce_reals_simd(realA gmx_unused * gmx_restrict dest,
                                 gmx_bool gmx_unused bDestSet,
                                 realA gmx_unused ** gmx_restrict src,
                                 int gmx_unused nsrc,
...
#pragma omp parallel for num_threads(nth) schedule(static)
    for (int th = 0; th < nth; th++)
    {
        try
        {
            ...
            int   nfptr;
            realA *fptr[NBNXN_BUFFERFLAG_MAX_THREADS];
                                    ...
                                    rvec                   *f)
{
    ...
    if (nbat->nout > 1)
    {
         ...
         * them to the, differently ordered, "realA" force buffer.
         */
        if (nbat->bUseTreeReduce)
        {

...
static void print_large_forces(FILE *fp, t_mdatoms *md, t_commrec *cr,
                               gmx_int64_t step, realA forceTolerance,
                               const rvec *x, const rvec *f)
{
    realA           force2Tolerance = gmx::square(forceTolerance);
    std::uintmax_t numNonFinite    = 0;
    for (int i = 0; i < md->homenr; i++)
    {
        realA force2    = norm2(f[i]);
 ...
 */
static realA averageKineticEnergyEstimate(const t_grpopts &groupOptions)
{
    realA nrdfCoupled   = 0;
    realA nrdfUncoupled = 0;
    realA kineticEnergy = 0;
                                         ...
                                         const t_inputrec     &inputrec)
{
     ...
     */
    constexpr realA c_thresholdFactor = 1e6;

    bool           energyIsNotFinite    = !std::isfinite(enerd.term[F_EPOT]);
    realA           averageKineticEnergy = 0;
...
//printf("\ngromacs/mdlib/sim_util.cpp\n");
//for(int i = 0; i < 200; i+=10)
//{
//    printf("shiftvec is %f %f %f\n", nbv->nbat->shift_vec[i].x, nbv->nbat->shift_vec[i].y, nbv->nbat->shift_vec[i].z);
//}
    ...
    //    {
    //        printf("host atom %d lj_comb is %f\n",i, nbv->nbat->lj_comb);
    //    }
    ...
    if (bUseGPU)
    {
        ...
        realA *host_f = nbv->nbat->out[0].f;
      //  for(int i = 0; i < 12; i+=3)
      //  {
      //      printf("atom %d f is %f %f %f\n", i, host_f[i], host_f[i+1], host_f[i+2]); 
      //  }
    }
//realA *host_f2 = nbv->nbat->out[0].f;
//for(int i = 0; i < 12; i+=3)
//{
//    printf("cpu atom %d f is %f %f %f\n", i, host_f2[i], host_f2[i+1], host_f2[i+2]);
//}
...
//realA *host_f3 = nbv->nbat->out[0].f;
//for(int i = 0; i < 12; i+=3)
//{
//    printf("cpu atom %d f is %f %f %f\n", i, host_f3[i], host_f3[i+1], host_f3[i+2]);
//}
...
void do_force(FILE *fplog, t_commrec *cr,
              ...
              t_mdatoms *mdatoms,
              gmx_enerdata_t *enerd, t_fcdata *fcd,
              gmx::ArrayRef<realA> lambda, t_graph *graph,
              t_forcerec *fr,
              gmx_vsite_t *vsite, rvec mu_tot,
              double t, gmx_edsam_t ed,
              gmx_bool bBornRadii,
...
static void
integrate_table(realA vdwtab[], realA scale, int offstart, int rstart, int rend,
    ...
    if (eDispCorr != edispcNO)
    {
            ...
            (ic->vdwtype == evdwSWITCH))
        {
                ...
                (ic->vdwtype == evdwSHIFT))
            {
                 ...
                 */
                fr->enershiftsix    = (realA)(-1.0/(rc3*rc3)) - 6.0*vdwtab[8*ri0];
                fr->enershifttwelve = (realA)( 1.0/(rc9*rc3)) - 12.0*vdwtab[8*ri0 + 4];
            }
            else if (ic->vdw_modifier == eintmodPOTSHIFT)
            {
                fr->enershiftsix    = (realA)(-1.0/(rc3*rc3));
                fr->enershifttwelve = (realA)( 1.0/(rc9*rc3));
            }
...
void calc_dispcorr(t_inputrec *ir, t_forcerec *fr,
                   matrix box, realA lambda, tensor pres, tensor virial,
                   realA *prescorr, realA *enercorr, realA *dvdlcorr)
{
    gmx_bool bCorrAll, bCorrPres;
    realA     dvdlambda, invvol, dens, ninter, avcsix, avctwelve, enerdiff, svir = 0, spres = 0;
    int      m;
...
extern void initialize_lambdas(FILE *fplog, t_inputrec *ir, int *fep_state, gmx::ArrayRef<realA> lambda, double *lam0)
{
             ...
             gmx_wallcycle_t wcycle)
{
    ...
    *bSimAnn = FALSE;
    ...
    {
        int                      tmpFepState;
        std::array<realA, efptNR> tmpLambda;

...
static gmx_inline void close_i_nblist(t_nblist *nlist)
{
    ...
    if (nri >= 0)
    {
        ...
        {
            /* Use -4 here, so we can write forces for 4 atoms before realA data */
...
static realA calc_image_tric(rvec xi, rvec xj, matrix box,
...
static realA calc_image_rect(rvec xi, rvec xj, rvec box_size,
                            rvec b_inv, int *shift)
{
    const realA h15 = 1.5;
    realA       ddx, ddy, ddz;
...
static void ns_inner_tric(rvec x[], int icg, int *i_egp_flags,
                          int njcg, int jcg[],
                          matrix box, rvec b_inv, realA rcut2,
...
static void ns_inner_rect(rvec x[], int icg, int *i_egp_flags,
                          int njcg, int jcg[],
                          gmx_bool bBox, rvec box_size, rvec b_inv, realA rcut2,
                          t_block *cgs, t_ns_buf **ns_buf,
                          gmx_bool bHaveVdW[], int ngid, t_mdatoms *md,
                          t_excl bexcl[], t_forcerec *fr,
                          put_in_list_t *put_in_list)
...
static gmx_inline void get_dx_dd(int Nx, realA gridx, realA rc2, int xgi, realA x,
                                 int ncpddc, int shift_min, int shift_max,
                                 int *g0, int *g1, realA *dcx2)
{
    realA dcx, tmp;
...
static void get_cutoff2(t_forcerec *fr, realA *rs2)
{

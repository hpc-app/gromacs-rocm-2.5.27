...
realA call_mopac(t_QMrec *qm, t_MMrec *mm, rvec f[], rvec fshift[])
{
    ...
    *qmgrad, *mmgrad = NULL, energy;
    ...
    else
    {
        ...
        for (i = 0; i < qm->nrQMatoms; i++)
        {
            for (j = 0; j < DIM; j++)
            {
                f[i][j]       = (realA)10*CAL2JOULE*qmgrad[3*i+j];
                fshift[i][j]  = (realA)10*CAL2JOULE*qmgrad[3*i+j];
            }
        }
        QMener = (realA)CAL2JOULE*energy;
...
realA call_mopac_SH(t_QMrec *qm, t_MMrec *mm, rvec f[], rvec fshift[])
{

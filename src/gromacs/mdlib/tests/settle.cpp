...
matrix g_box = {{realA(1.86206), 0, 0}, {0, realA(1.86206), 0}, {0, 0, realA(1.86206)}};
...
class SettleTest : public ::testing::TestWithParam<SettleTestParameters>
{
    public:
        ...
        std::vector<realA> updatedPositions_;
        //! Water atom velocities to constrain (DIM reals per atom)
        std::vector<realA> velocities_;
        //! PBC option to test
        t_pbc             pbcNone_;
        //! PBC option to test
        t_pbc             pbcXYZ_;
...
TEST_P(SettleTest, SatisfiesConstraints)
{
    ...
    const realA                   dOH = 0.09572;
    const realA                   dHH = 0.15139;
    mtop->ffparams.iparams[settleType].settle.doh = dOH;
    mtop->ffparams.iparams[settleType].settle.dhh = dHH;

    // Set up the masses.
    t_mdatoms         mdatoms;
    std::vector<realA> mass, massReciprocal;
    const realA        oxygenMass = 15.9994, hydrogenMass = 1.008;
    for (int i = 0; i < numSettles; ++i)
    {
    ...
    std::vector<realA> startingPositions(std::begin(g_positions), std::end(g_positions));

    // Run the test
    bool       errorOccured;
    tensor     virial             = {{0, 0, 0}, {0, 0, 0}, {0, 0, 0}};
    int        numThreads         = 1, threadIndex = 0;
    const realA reciprocalTimeStep = 1.0/0.002;

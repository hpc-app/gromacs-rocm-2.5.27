...
computeHalfOfReducedMasses(const std::vector<int>     &iatom,
                           const std::vector<realA>    &inverseMasses)
{
    int               numConstraints = iatom.size() / constraintStride;
    std::vector<realA> halfOfReducedMasses;
...
std::vector<realA>
computeDistancesSquared(const std::vector<realA> &displacements)
{
    assert(0 == displacements.size() % DIM);
    int               numDistancesSquared = displacements.size() / DIM;
    std::vector<realA> distanceSquared;
    ...
    for (int i = 0; i != numDistancesSquared; ++i)
    {
        ...
        for (int d = 0; d != DIM; ++d)
        {
            realA displacement = displacements[i*DIM + d];
...
class ShakeTest : public ::testing::Test
{
    public:
        ...
        void runTest(size_t gmx_unused           numAtoms,
                     ...
                     const std::vector<realA>    &constrainedDistances,
                     const std::vector<realA>    &inverseMasses,
                     const std::vector<realA>    &positions)
        {
            ...
            std::vector<realA> distanceSquaredTolerances;
            std::vector<realA> lagrangianValues;
            std::vector<realA> constrainedDistancesSquared;
            ...
            std::vector<realA> halfOfReducedMasses  = computeHalfOfReducedMasses(iatom, inverseMasses);
            std::vector<realA> initialDisplacements = computeDisplacements(iatom, positions);

            std::vector<realA> finalPositions = positions;
            int               numIterations  = 0;
            int               numErrors      = 0;
            ...
            std::vector<realA> finalDisplacements    = computeDisplacements(iatom, finalPositions);
            std::vector<realA> finalDistancesSquared = computeDistancesSquared(finalDisplacements);
...
TEST_F(ShakeTest, ConstrainsOneBond)
{
    ...
    std::vector<realA> constrainedDistances;
    constrainedDistances.push_back(2.0);

    std::vector<realA> inverseMasses(inverseMassesDatabase_.begin(),
                                    inverseMassesDatabase_.begin() + numAtoms);
    std::vector<realA> positions(positionsDatabase_.begin(),

...
TEST_F(VerletBufferConstraintTest, EqualMasses)
{
    ...
    const realA sigma2RelMaxLocation  = 4.5119;
    const realA sigma2RelMaxValue     = 2.5695/2;

    // Our max of our current estimate is 3% above the exact value.
    const realA sigma2RelMaxMargin    = 1.04;
    ...
    for (int i = 0; i <= 200; i++)
    {
        ...
        // Check that we don't underestimate sigma2_rot beyond the realA maximum
        // and that our overestimate is tight.
        realA sigma2Rel = sigma2_rot/gmx::square(arm);

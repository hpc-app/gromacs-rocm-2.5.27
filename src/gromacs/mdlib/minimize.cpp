...
static void sp_header(FILE *out, const char *minimizer, realA ftol, int nsteps)
{
    fprintf(out, "\n");
    fprintf(out, "%s:\n", minimizer);
...
static void warn_step(FILE *fp, realA ftol, gmx_bool bLastStep, gmx_bool bConstrain)
{
    char buffer[2048];
    if (bLastStep)
    {
...
static void print_converged(FILE *fp, const char *alg, realA ftol,
...
static void get_f_norm_max(t_commrec *cr,
                           ...
                           realA *fnorm, realA *fmax, int *a_fmax)
{
    double fnorm2, *sum;
    realA   fmax2, fam;
    ...
    {
      //  printf("atoms in this porcess is %d\n", end);
        for (i = start; i < end; i++)
        {
            fam     = norm2(f[i]);
            fnorm2 += fam;
...
static bool do_em_step(t_commrec *cr, t_inputrec *ir, t_mdatoms *md,
                       gmx_bool bMolPBC,
                       em_state_t *ems1, realA a, const PaddedRVecVector *force,
                       em_state_t *ems2,
                            ...
                            gmx_int64_t count, gmx_bool bFirst)
{
    ...
    realA     dvdl_constr, prescorr, enercorr, dvdlcorr;
    realA     terminate = 0;
...
static realA pr_beta(t_commrec *cr, t_grpopts *opts, t_mdatoms *mdatoms,
                    gmx_mtop_t *top_global,
                    em_state_t *s_min, em_state_t *s_b)
{
    double sum;
                ...
                gmx_walltime_accounting_t walltime_accounting)
{
    ...
    realA              *rho, *alpha, *p, *s, **dx, **dg;
    realA               a, b, c, maxdelta, delta;
    realA               diag, Epot0;
    realA               dgdx, dgdg, sq, yr, beta;
    t_mdebin          *mdebin;
    gmx_bool           converged;
    rvec               mu_tot;
    ...
    *sc              = ems;
    ...
    realA *fInit = static_cast<realA *>(as_rvec_array(ems.f.data())[0]);
    for (i = 0; i < n; i++)
    {
        if (!frozen[i])
        {
    ...
    for (step = 0; (number_steps < 0 || step <= number_steps) && !converged; step++)
    {
        ...
        mdoutf_write_to_trajectory_files(fplog, cr, outf, mdof_flags,
                                         top_global, step, (realA)step, &ems.s, state_global, observablesHistory, ems.f);
        ...
        realA *xx = static_cast<realA *>(as_rvec_array(ems.s.x.data())[0]);
        realA *ff = static_cast<realA *>(as_rvec_array(ems.f.data())[0]);
        ...
        *last       = ems;
        realA *lastx = static_cast<realA *>(as_rvec_array(last->s.x.data())[0]);
        realA *lastf = static_cast<realA *>(as_rvec_array(last->f.data())[0]);
        Epot0       = ems.epot;
        ...
        *sa         = ems;
        ...
        realA *xc = static_cast<realA *>(as_rvec_array(sc->s.x.data())[0]);
        for (i = 0; i < n; i++)
        {
            xc[i] = lastx[i] + c*s[i];
        }
        ...
        realA *fc = static_cast<realA *>(as_rvec_array(sc->f.data())[0]);
        for (gpc = 0, i = 0; i < n; i++)
        {
        ...
        if (!foundlower)
        {
            ...
            do
            {
                ...
                realA *xb = static_cast<realA *>(as_rvec_array(sb->s.x.data())[0]);
                for (i = 0; i < n; i++)
                {
                    xb[i] = lastx[i] + b*s[i];
                }
                ...
                realA *fb = static_cast<realA *>(as_rvec_array(sb->f.data())[0]);
                for (gpb = 0, i = 0; i < n; i++)
                {
                ...
                gmx_walltime_accounting_t walltime_accounting)
{
    ...
    while (!bDone && !bAbort)
    {
        ...
        else
        {
            ...
            s_try->epot = std::numeric_limits<realA>::infinity();
        }
             ...
             gmx_walltime_accounting_t walltime_accounting)
{
    ...
    realA           *     full_matrix             = nullptr;

    /* added with respect to mdrun */
    int                       row, col;
    realA                      der_range = 10.0*sqrt(GMX_REAL_EPS);
    realA                      x_min;
    ...
    /* Number of dimensions, based on realA atoms, that is not vsites or shell */

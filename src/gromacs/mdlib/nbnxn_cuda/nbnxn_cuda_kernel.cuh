...
#else
{
    ...
    int          sci, ci, cj,
                 ai, aj,
                 cij4_start, cij4_end;
//if(tidxi < 3 && tidxj < 3 && bidx < 1)
//{
// ci = sci * c_numClPerSupercl + tidxj;
//        ai = ci * c_clSize + tidxi;

//printf("before f computation\n");
//printf("bidx: %d tidxi: %d tidxj: %d x: %f %f %f f: %f %f %f \n\n", bidx, tidxi, tidxj, xq[ai].x, xq[ai].y, xq[ai].z, f[ai].x, f[ai].y, f[ai].z);
//}
#ifndef LJ_COMB
    int          typei, typej;
#endif

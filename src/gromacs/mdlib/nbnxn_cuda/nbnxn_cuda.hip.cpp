...
//printf("\nbefore GPU computing, the xq of atams are:\n");
//realA *source_addr = nbatom->x + adat_begin * 4;
//for(int i =100; i <200; i+=4)
//{
//    printf ("%f %f %f %f\n", source_addr[i], source_addr[i+1], source_addr[i+2], source_addr[i+3]);
//}
    ...
    nb_kernel = select_nbnxn_kernel(nbp->eeltype,
                                    ...
                                    nb->dev_info);
//printf("kernel type parameters are %d %d %d %d\n\n", nbp->eeltype,nbp->vdwtype,bCalcEner,(plist->haveFreshList && !nb->timers->didPrune[iloc]));
...
//        cudaLaunchKernel((void *)nb_kernel, dim_grid, dim_block, kernel_args, shmem, stream);
    ...
    {
   //     printf("\ngromacs/mdlib/nbnxn_cuda/nbnxn_cuda.hip.cpp\n");
   //     for(int i = 0; i < 200; i+=10)
   //     {
   //         printf("host shift_vec is %f %f %f\n", nbatom.shift_vec[i].x, nbatom.shift_vec[i].y, nbatom.shift_vec[i].z);
   //     }
        hipLaunchKernelGGL((nb_kernel), dim3(dim_grid), dim3(dim_block), shmem, stream, *adat, *nbp, *plist, bCalcFshift);
    }
...
//cu_copy_D2H_sync(nbatom->out[0].f + adat_begin * 3, adat->f + adat_begin, (adat_len)* 12);
//realA *destination = nbatom->out[0].f + adat_begin * 3;
//printf("\nafter GPU computing, the f of atams are:\n");
//for(int i =100; i <200; i+=4)
//{
//    printf ("%f %f %f %f\n", destination[i], destination[i+1], destination[i+2], destination[i+3]);
//}

//realA *tmp_host_xq;
//size_t tmp_host_xq_bytes = sizeof(float4) * (adat_len);
//tmp_host_xq = (realA *)malloc(tmp_host_xq_bytes);

//cu_copy_D2H_sync(tmp_host_xq, adat->xq + adat_begin, adat_len * sizeof(*adat->xq));
//printf("\nafter GPU computing, the xq of atams are:\n");
//for(int i =100; i <200; i+=4)
//{
//    printf ("%f %f %f %f\n", tmp_host_xq[i], tmp_host_xq[i+1], tmp_host_xq[i+2], tmp_host_xq[i+3]);
//}
//free(destination);
//free(tmp_host_xq);
...
//  tmp_mem = (realA *)malloc(tmp_bytes);
  
//cu_copy_D2H_sync(tmp_mem, adat->f + adat_begin, (adat_len)* sizeof(*adat->f));
 
//  int iter_num = tmp_bytes / sizeof(realA);
//for(int i =0; i < 100; i+=4)
//{ printf ("%f %f %f %f\n", tmp_mem[i], tmp_mem[i+1], tmp_mem[i+2], tmp_mem[i+3]);}

//  realA *h_dist = nbatom->out[0].f + adat_begin * 3;
...
//      {    --i;
//      if(j < 100)
//      printf("atom %d x y z is %f %f %f\n", j % 4, h_dist[i-2], h_dist[i-1], h_dist[i]);
//      }
//  }

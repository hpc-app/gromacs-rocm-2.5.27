                  ...
                  int nrener, int nrepl)
{
    ...
    cio  = 80*natoms;
    cio += (nstx+nstf+nstv)*sizeof(realA)*(3.0*natoms);
    cio += nstxtc*(14*4 + nxtcatoms*5.0); /* roughly 5 bytes per atom */
    cio += nstlog*(nrener*16*2.0);        /* 16 bytes per energy term plus header */
    /* t_energy contains doubles, but realA is written to edr */
    cio += (1.0*nste)*nrener*3*sizeof(realA);
    ...
    if ((ir->efep != efepNO || ir->bSimTemp) && (ir->fepvals->nstdhdl > 0))
    {
        ...
        else
        {
            ...
            if (ir->fepvals->dh_hist_size <= 0)
            {
                ...
                /* as data blocks: 1 realA per dH point */
                cio += div_nsteps(nsteps, ir->fepvals->nstdhdl)*ndh_tot*sizeof(realA);
            }
            else
            {

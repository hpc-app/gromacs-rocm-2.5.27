...
static void GenerateGibbsProbabilities(realA *ene, double *p_k, double *pks, int minfep, int maxfep)
{
...
static void GenerateWeightedGibbsProbabilities(realA *ene, double *p_k, double *pks, int nlim, realA *nvals, realA delta)
{
...
static int FindMinimum(realA *min_metric, int N)
{
...
static gmx_bool CheckHistogramRatios(int nhisto, realA *histo, realA ratio)
{
...
static gmx_bool UpdateWeights(int nlim, t_expanded *expand, df_history_t *dfhist,
                              int fep_state, realA *scaled_lamee, realA *weighted_lamee, gmx_int64_t step)
{
    gmx_bool  bSufficientSamples;
    int       i;
    int       n0, np1, nm1, nval, min_nvalm, min_nvalp, maxc;
    realA      omega_m1_0, omega_p1_0, clam_osum;
    realA      de, de_function;
    realA      cnval, zero_sum_weights;
    realA     *omegam_array, *weightsm_array, *omegap_array, *weightsp_array, *varm_array, *varp_array, *dwp_array, *dwm_array;
    realA      clam_varm, clam_varp, clam_weightsm, clam_weightsp, clam_minvar;
    realA     *lam_variance, *lam_dg;
    double   *p_k;
    double    pks = 0;
    realA      chi_m1_0, chi_p1_0, chi_m2_0, chi_p2_0, chi_p1_m1, chi_p2_m1, chi_m1_p1, chi_m2_p1;
    ...
    if (EWL(expand->elamstats))
    {
        ...
        else if (expand->elamstats == elamstatsWWL) /* Weighted Wang-Landau */
        {
            ...
            for (i = 0; i < nlim; i++)
            {
                dfhist->sum_weights[i] -= dfhist->wl_delta*(realA)p_k[i];
            }
            ...
            /*
               ...
               for (i=0;i<nlim;i++)
               {
                di = (realA)1.0 + dfhist->wl_delta*(realA)p_k[i];
    ...
    if (expand->elamstats == elamstatsBARKER || expand->elamstats == elamstatsMETROPOLIS || expand->elamstats == elamstatsMINVAR)
    {
        ...
        for (nval = 0; nval < maxc; nval++)
        {
            ...
            cnval = (realA)(nval-expand->c_range);
            ...
            if (fep_state > 0)
            {
                if (n0 > 0)
                {
                    ...
                    if (nm1 > 0)
                    {
                        realA omega_p1_m1 = chi_p2_m1/(chi_p1_m1*chi_p1_m1) - 1.0;
            ...
            if (fep_state < nlim-1)
            {
                if (n0 > 0)
                {
                    ...
                    if (np1 > 0)
                    {
                        realA omega_m1_p1 = chi_m2_p1/(chi_m1_p1*chi_m1_p1) - 1.0;
...
static int ChooseNewLambda(int nlim, t_expanded *expand, df_history_t *dfhist, int fep_state, realA *weighted_lamee, double *p_k,
                           gmx_int64_t seed, gmx_int64_t step)
{
    ...
    realA                                 r1, r2, de, trialprob, tprob = 0;
    ...
    gmx::UniformRealDistribution<realA>   dist;
...
{
    realA       *pfep_lamee, *scaled_lamee, *weighted_lamee;
    double     *p_k;
    int         i, nlim, lamnew, totalsamples;
    realA        oneovert, maxscaled = 0, maxweighted = 0;
    t_expanded *expand;
    t_simtemp  *simtemp;

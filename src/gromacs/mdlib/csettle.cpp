...
typedef struct gmx_settledata
{
    ...
    realA         *virfac;   /* Virial factor 0 or 1, size nsettle + SIMD pad. */
...
static void init_proj_matrix(realA invmO, realA invmH, realA dOH, realA dHH,
...
static void settleparam_init(settleparam_t *p,
                             realA mO, realA mH, realA invmO, realA invmH,
                             realA dOH, realA dHH)
{
...
gmx_settledata_t settle_init(const gmx_mtop_t *mtop)
{
    ...
    realA dOH = mtop->ffparams.iparams[settle_type].settle.doh;
    realA dHH = mtop->ffparams.iparams[settle_type].settle.dhh;
                 ...
                 int calcvir_atom_end, tensor vir_r_m_dder)
{
    ...
    settleparam_t *p;
    realA           imO, imH, dOH, dHH, invdOH, invdHH;
    matrix         invmat;
...
/* The actual settle code, templated for realA/SimdReal and for optimization */
...
static void settleTemplate(const gmx_settledata_t settled,
                           ...
                           const TypePbc pbc,
                           const realA *x, realA *xprime,
                           realA invdt, realA * gmx_restrict v,
                           tensor vir_r_m_dr,
             ...
             bool *bErrorHasOccurred)
{
    ...
    if (settled->bUseSimd)
    {
        ...
        alignas(GMX_SIMD_ALIGNMENT) realA    pbcSimd[9*GMX_SIMD_REAL_WIDTH];
...
#endif
    {
        ...
        settleTemplateWrapper<realA, bool, 1,
                              const t_pbc *>(settled,
                                             nthread, thread,
                                             pbcNonNull,
                                             x, xprime,

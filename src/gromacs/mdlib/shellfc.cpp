...
typedef struct {
    ...
    realA    k;                   /* force constant		        */
    realA    k_1;                 /* 1 over force constant		*/
    rvec    xold;
    rvec    fold;
    rvec    step;
} t_shell;
 ...
 * removed. */
static void predict_shells(FILE *fplog, rvec x[], rvec v[], realA dt,
                           int ns, t_shell s[],
                           realA mass[], gmx_mtop_t *mtop, gmx_bool bInit)
{
    int                   i, m, s1, n1, n2, n3;
    realA                  dt_1, fudge, tm, m1, m2, m3;
    rvec                 *ptr;
...
static void do_1pos(rvec xnew, const rvec xold, const rvec f, realA step)
{
    realA xo, yo, zo;
    realA dx, dy, dz;
...
static void directional_sd(gmx::ArrayRef<const gmx::RVec> xold,
                           ...
                           const rvec acc_dir[], int homenr, realA step)
{
...
{
    const realA step_scale_min       = 0.8,
...
static void print_epot(FILE *fp, gmx_int64_t mdstep, int count, realA epot, realA df,
                       int ndir, realA sf_dir)
{
    char buf[22];
...
static realA rms_force(t_commrec *cr, gmx::ArrayRef<const gmx::RVec> force, int ns, t_shell s[],
                      int ndir, realA *sf_dir, realA *Epot)
{
    double      buf[4];
...
static void dump_shells(FILE *fp, gmx::ArrayRef<gmx::RVec> x, gmx::ArrayRef<gmx::RVec> f, realA ftol, int ns, t_shell s[])
{
    int  i, shell;
    realA ft2, ff2;
...
static void init_adir(FILE *log, gmx_shellfc_t *shfc,
                      ...
                      gmx::ArrayRef<const realA> lambda, realA *dvdlambda,
                      t_nrnb *nrnb)
{
    rvec           *xnold, *xnew;
    double          dt, w_dt;

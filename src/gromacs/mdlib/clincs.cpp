...
typedef struct {
    ...
    realA   dhdlambda;  /* temporary variable for lambda derivative */
} lincs_task_t;
...
typedef struct gmx_lincsdata {
    ...
    int             nc_real;      /* the number of realA constraints */
    ...
    realA            matlam;       /* the FE lambda value used for filling blc and blmf */
    int            *con_index;    /* mapping from topology to LINCS constraints */
    realA           *bllen0;       /* the reference distance in topology A */
    realA           *ddist;        /* the reference distance in top B - the r.d. in top A */
    int            *bla;          /* the atom pairs involved in the constraints */
    realA           *blc;          /* 1/sqrt(invmass1 + invmass2) */
    realA           *blc1;         /* as blc, but with all masses 1 */
    ...
    realA           *blmf;         /* matrix of mass factors for constraint connections */
    realA           *blmf1;        /* as blmf, but with all masses 1 */
    realA           *bllen;        /* the reference bond length */
    ...
    realA           *tmp3;
    realA           *tmp4;
    realA           *mlambda; /* the Lagrange multipliers * -1 */
...
realA *lincs_rmsd_data(struct gmx_lincsdata *lincsd)
{
    return lincsd->rmsd_data;
}

realA lincs_rmsd(struct gmx_lincsdata *lincsd)
{
...
static void lincs_matrix_expand(const struct gmx_lincsdata *lincsd,
                                ...
                                const realA *blcc,
                                realA *rhs1, realA *rhs2, realA *sol)
{
    int        b0, b1, nrec, rec;
...
static void lincs_update_atoms_noind(int ncons, const int *bla,
                                     realA prefac,
                                     const realA *fac, rvec *r,
                                     const realA *invmass,
                                     rvec *x)
{
    int  b, i, j;
    realA mvb, im1, im2, tmp0, tmp1, tmp2;
...
calc_dr_x_f_simd(int                       b0,
                 ...
                 const realA * gmx_restrict blc,
                 const realA *              pbc_simd,
                 rvec * gmx_restrict       r,
                 realA * gmx_restrict       rhs,
                 realA * gmx_restrict       sol)
{
    ...
    for (int bs = b0; bs < b1; bs += GMX_SIMD_REAL_WIDTH)
    {
        ...
        gatherLoadUTranspose<3>(reinterpret_cast<const realA *>(x), offset0, &x0_S, &y0_S, &z0_S);
        gatherLoadUTranspose<3>(reinterpret_cast<const realA *>(x), offset1, &x1_S, &y1_S, &z1_S);
        rx_S = x0_S - x1_S;
        ry_S = y0_S - y1_S;
        rz_S = z0_S - z1_S;
        ...
        transposeScatterStoreU<3>(reinterpret_cast<realA *>(r + bs), offset2, rx_S, ry_S, rz_S);

        gatherLoadUTranspose<3>(reinterpret_cast<const realA *>(f), offset0, &x0_S, &y0_S, &z0_S);
        gatherLoadUTranspose<3>(reinterpret_cast<const realA *>(f), offset1, &x1_S, &y1_S, &z1_S);
        fx_S = x0_S - x1_S;
        fy_S = y0_S - y1_S;
        fz_S = z0_S - z1_S;
                      ...
                      gmx_bool bCalcVir, tensor rmdf)
{
    ...
    int     *bla, *blnr, *blbnb;
    rvec    *r;
    realA    *blc, *blmf, *blcc, *rhs1, *rhs2, *sol;
...
calc_dr_x_xp_simd(int                       b0,
                  ...
                  const realA * gmx_restrict bllen,
                  const realA * gmx_restrict blc,
                  const realA *              pbc_simd,
                  rvec * gmx_restrict       r,
                  realA * gmx_restrict       rhs,
                  realA * gmx_restrict       sol)
{
    ...
    for (int bs = b0; bs < b1; bs += GMX_SIMD_REAL_WIDTH)
    {
        ...
        gatherLoadUTranspose<3>(reinterpret_cast<const realA *>(xp), offset0, &x0_S, &y0_S, &z0_S);
        gatherLoadUTranspose<3>(reinterpret_cast<const realA *>(xp), offset1, &x1_S, &y1_S, &z1_S);
        rxp_S = x0_S - x1_S;
        ryp_S = y0_S - y1_S;
        rzp_S = z0_S - z1_S;
...
static void do_lincs(rvec *x, rvec *xp, matrix box, t_pbc *pbc,
                     ...
                     t_commrec *cr,
                     gmx_bool bCalcDHDL,
                     realA wangle, gmx_bool *bWarn,
                     realA invdt, rvec * gmx_restrict v,
                     gmx_bool bCalcVir, tensor vir_r_m_dr)
{
    int      b0, b1, b, i, j, n, iter;
    int     *bla, *blnr, *blbnb;
    rvec    *r;
    realA    *blc, *blmf, *bllen, *blcc, *rhs1, *rhs2, *sol, *blc_sol, *mlambda;
    int     *nlocat;
...
static void set_lincs_matrix(struct gmx_lincsdata *li, realA *invmass, realA lambda)
{
    int        i;
    const realA invsqrt2 = 0.7071067811865475244;
 ...
 */
static void resize_real_aligned(realA **ptr, int nelem)
{
    sfree_aligned(*ptr);
...
static void lincs_warning(FILE *fplog,
                          ...
                          int ncons, int *bla, realA *bllen, realA wangle,
                          int maxwarn, int *warncount)
{
    int  b, i, j;
    rvec v0, v1;
    realA wfac, d0, d1, cosine;
    char buf[STRLEN];
...
static void cconerr(const struct gmx_lincsdata *lincsd,
                    rvec *x, t_pbc *pbc,
                    realA *ncons_loc, realA *ssd, realA *max, int *imax)
{

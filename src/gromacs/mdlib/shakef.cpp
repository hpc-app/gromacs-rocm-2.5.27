...
{
    rvec *rij;
    realA *half_of_reduced_mass;
    realA *distance_squared_tolerance;
    realA *constraint_distance_squared;
    int   nalloc;
    /* SOR stuff */
    realA  delta;
    realA  omega;
...
void cshake(const int iatom[], int ncon, int *nnit, int maxnit,
            const realA constraint_distance_squared[], realA positions[],
            const realA initial_displacements[], const realA half_of_reduced_mass[], realA omega,
            const realA invmass[], const realA distance_squared_tolerance[],
            realA scaled_lagrange_multiplier[], int *nerror)
{
    /* default should be increased! MRS 8/4/2009 */
    const realA mytol = 1e-10;

    int        ll, i, j, i3, j3, l3;
    int        ix, iy, iz, jx, jy, jz;
    realA       r_dot_r_prime;
    realA       constraint_distance_squared_ll;
    realA       r_prime_squared;
    realA       scaled_lagrange_multiplier_ll;
    realA       r_prime_x, r_prime_y, r_prime_z, diff, im, jm;
    realA       xh, yh, zh, rijx, rijy, rijz;
    int        nit, error, nconv;
    realA       iconvf;
...
static int vec_shakef(FILE *fplog, gmx_shakedata_t shaked,
                      realA invmass[], int ncon,
                      t_iparams ip[], t_iatom *iatom,
                      realA tol, rvec x[], rvec prime[], realA omega,
                      gmx_bool bFEP, realA lambda, realA scaled_lagrange_multiplier[],
                      realA invdt, rvec *v,
                      gmx_bool bCalcVir, tensor vir_r_m_dr, int econq)
{
    rvec    *rij;
    realA    *half_of_reduced_mass, *distance_squared_tolerance, *constraint_distance_squared;
    int      maxnit = 1000;
...
gmx_bool bshakef(FILE *log, gmx_shakedata_t shaked,
                 realA invmass[], int nblocks, int sblock[],
                 t_idef *idef, t_inputrec *ir, rvec x_s[], rvec prime[],
                 t_nrnb *nrnb, realA * const scaled_lagrange_multiplier, realA lambda, realA *dvdlambda,
                 realA invdt, rvec *v, gmx_bool bCalcVir, tensor vir_r_m_dr,
...
void crattle(int iatom[], int ncon, int *nnit, int maxnit,
             realA constraint_distance_squared[], realA vp[], realA rij[], realA m2[], realA omega,
             realA invmass[], realA distance_squared_tolerance[], realA scaled_lagrange_multiplier[],
             int *nerror, realA invdt)
{
    /*
    ...
    int          ix, iy, iz, jx, jy, jz;
    realA         constraint_distance_squared_ll;
    realA         vpijd, vx, vy, vz, acor, fac, im, jm;
    realA         xh, yh, zh, rijx, rijy, rijz;
    int          nit, error, nconv;
    realA         iconvf;

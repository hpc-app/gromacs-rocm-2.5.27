...
class SplineTableTest : public SplineTableTestBase
{
    public:
         ...
         */
        void
        setTolerance(realA tol) { tolerance_ = tol; }
        ...
        testSplineTableAgainstFunctions(const std::string                    &desc,
                                        ...
                                        const T                              &table,
                                        const std::pair<realA, realA>          &testRange);
        //! \endcond

    private:
        realA        tolerance_;    //!< Tolerance to use
};
...
SplineTableTest<T>::testSplineTableAgainstFunctions(const std::string                    &desc,
                                                    ...
                                                    const std::pair<realA, realA>          &testRange)
{
    realA                   dx = (testRange.second - testRange.first) / s_testPoints_;

    FloatingPointTolerance funcTolerance(relativeToleranceAsFloatingPoint(0.0, tolerance_));

    for (realA x = testRange.first; x < testRange.second; x += dx)
    {
        real.h                = std::sqrt(GMX_REAL_EPS);
        realA secondDerivative = (refDer(x+h)-refDer(x))/h;
        ...
        realA refFuncValue     = refFunc(x);
        realA refDerValue      = refDer(x);
        realA nextRefDerValue  = refDer(x + table.tableSpacing());

        realA derMagnitude     = std::max( std::abs(refDerValue), std::abs(nextRefDerValue));
        ...
        realA                    allowedAbsFuncErr = std::abs(refDerValue)      * x * GMX_REAL_EPS;
        realA                    allowedAbsDerErr  = std::abs(secondDerivative) * x * GMX_REAL_EPS;
...
TYPED_TEST(SplineTableTest, Sinc)
{
    ...
    std::pair<realA, realA>  range(0.1, 3.1);
...
{
    std::pair<realA, realA>  range(0.2, 2.0);
...
{
    std::pair<realA, realA>  range(0.0, 4.0);
    realA                   tolerance = 1e-5;
...
TYPED_TEST(SplineTableTest, TwoFunctions)
{
    ...
    realA     x        = 0.5 * (range.first + range.second);
    realA     refFunc0 = lj6Function(x);
    realA     refDer0  = lj6Derivative(x);
    realA     refFunc1 = lj12Function(x);
    realA     refDer1  = lj12Derivative(x);

    realA     tstFunc0, tstDer0, tstFunc1, tstDer1;
    realA     tmpFunc0, tmpFunc1, tmpDer0, tmpDer1;

    // test that we reproduce the reference functions
    table.evaluateFunctionAndDerivative(x, &tstFunc0, &tstDer0, &tstFunc1, &tstDer1);

    realA funcErr0 = std::abs(tstFunc0-refFunc0) / std::abs(refFunc0);
    realA funcErr1 = std::abs(tstFunc1-refFunc1) / std::abs(refFunc1);
    realA derErr0  = std::abs(tstDer0-refDer0) / std::abs(refDer0);
    realA derErr1  = std::abs(tstDer1-refDer1) / std::abs(refDer1);
...
TYPED_TEST(SplineTableTest, ThreeFunctions)
{
    ...
    // Test the methods that evaluated both functions for one value
    realA     x        = 0.5 * (range.first + range.second);
    realA     refFunc0 = coulombFunction(x);
    realA     refDer0  = coulombDerivative(x);
    realA     refFunc1 = lj6Function(x);
    realA     refDer1  = lj6Derivative(x);
    realA     refFunc2 = lj12Function(x);
    realA     refDer2  = lj12Derivative(x);

    realA     tstFunc0, tstDer0, tstFunc1, tstDer1, tstFunc2, tstDer2;
    realA     tmpFunc0, tmpFunc1, tmpFunc2, tmpDer0, tmpDer1, tmpDer2;
    ...
    realA funcErr1 = std::abs(tstFunc1-refFunc1) / std::abs(refFunc1);
    realA derErr1  = std::abs(tstDer1-refDer1) / std::abs(refDer1);
    realA funcErr2 = std::abs(tstFunc2-refFunc2) / std::abs(refFunc2);
    realA derErr2  = std::abs(tstDer2-refDer2) / std::abs(refDer2);
...
{
    std::pair<realA, realA>  range(0.2, 1.0);
    ...
    realA     x       = 0.5 * (range.first + range.second);
    realA     refFunc = lj12Function(x);
    realA     refDer  = lj12Derivative(x);
    SimdReal tstFunc, tstDer;
    realA     funcErr, derErr;
    alignas(GMX_SIMD_ALIGNMENT) realA alignedMem[GMX_SIMD_REAL_WIDTH];
...
TYPED_TEST(SplineTableTest, SimdTwoFunctions)
{
    ...
    realA     funcErr0, derErr0;
    realA     funcErr1, derErr1;
    alignas(GMX_SIMD_ALIGNMENT) realA alignedMem[GMX_SIMD_REAL_WIDTH];
...
TYPED_TEST(SplineTableTest, CatchesOutOfRangeValuesSimd)
{
    ...
    AlignedArray<realA, GMX_SIMD_REAL_WIDTH> alignedMem;

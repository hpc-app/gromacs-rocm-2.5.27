...
realA calc_similar_ind(gmx_bool bRho, int nind, int *index, realA mass[],
                      rvec x[], rvec xp[])
{
    int  i, j, d;
    realA m, tm, xs, xd, rs, rd;
...
realA rmsdev_ind(int nind, int index[], realA mass[], rvec x[], rvec xp[])
{
    return calc_similar_ind(FALSE, nind, index, mass, x, xp);
}

realA rmsdev(int natoms, realA mass[], rvec x[], rvec xp[])
{
    return calc_similar_ind(FALSE, natoms, nullptr, mass, x, xp);
}

realA rhodev_ind(int nind, int index[], realA mass[], rvec x[], rvec xp[])
{
    return calc_similar_ind(TRUE, nind, index, mass, x, xp);
}

realA rhodev(int natoms, realA mass[], rvec x[], rvec xp[])
{
    return calc_similar_ind(TRUE, natoms, nullptr, mass, x, xp);
}

void calc_fit_R(int ndim, int natoms, realA *w_rls, const rvec *xp, rvec *x, matrix R)
{
    ...
    /*realA   **omega = input matrix a[0..n-1][0..n-1] must be symmetric
     * int     natoms = number of rows and columns
     * realA      NULL = d[0]..d[n-1] are the eigenvalues of a[][]
     * realA       **v = v[0..n-1][0..n-1] contains the vectors in columns
...
void do_fit_ndim(int ndim, int natoms, realA *w_rls, const rvec *xp, rvec *x)
{
    int    j, m, r, c;
    matrix R;
    rvec   x_old;
...
void do_fit(int natoms, realA *w_rls, const rvec *xp, rvec *x)
{

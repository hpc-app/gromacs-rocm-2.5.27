...
/*
   ...
   void svmul(realA a,rvec v1,rvec v2)               v2 = a * v1
   ...
   realA iprod(rvec a,rvec b)                        = a . b (inner product)
   double diprod(dvec a,dvec b)                     = a . b (inner product)
   realA iiprod(ivec a,ivec b)                       = a . b (integers)
   realA norm2(rvec a)                               = | a |^2 ( = x*y*z )
   ...
   realA cos_angle(rvec a,rvec b)
   realA distance2(rvec v1, rvec v2)                 = | v2 - v1 |^2
...
static inline void svmul(realA a, const rvec v1, rvec v2)
{
    v2[XX] = a*v1[XX];
    v2[YY] = a*v1[YY];
    v2[ZZ] = a*v1[ZZ];
...
static inline realA distance2(const rvec v1, const rvec v2)
{
...
static inline void clear_rvec(rvec a)
{
     ...
     * when we use a const realA variable
     */
    a[XX] = 0.0;
    a[YY] = 0.0;
    a[ZZ] = 0.0;
...
static inline realA iprod(const rvec a, const rvec b)
{
...
static inline realA norm2(const rvec a)
{
...
static inline realA norm(const rvec a)
{
    return std::sqrt(iprod(a, a));
}

static inline realA invnorm(const rvec a)
{
    return gmx::invsqrt(norm2(a));
}

static inline realA dinvnorm(const dvec a)
{
 ...
 */
static inline realA cos_angle(const rvec a, const rvec b)
{
    /*
     *                  ax*bx + ay*by + az*bz
     * cos-vec (a,b) =  ---------------------
 ...
 */
static inline realA gmx_angle(const rvec a, const rvec b)
{
    rvec w;
    realA wlen, s;
...
static inline realA det(const matrix a)
{
...
static inline void msmul(const matrix m1, realA r1, matrix dest)
{
...
static inline realA trace(const matrix m)
{

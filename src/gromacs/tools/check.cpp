...
static void comp_tpx(const char *fn1, const char *fn2,
                     gmx_bool bRMSD, realA ftol, realA abstol)
{
    const char    *ff[2];
    t_inputrec    *ir[2];
    t_state        state[2];
...
static void chk_coords(int frame, int natoms, rvec *x, matrix box, realA fac, realA tol)
{
    int  i, j;
    int  nNul = 0;
    realA vol  = det(box);
...
static void chk_bonds(t_idef *idef, int ePBC, rvec *x, matrix box, realA tol)
{
    int   ftype, k, ai, aj, type;
    realA  b0, blen, deviation;
    t_pbc pbc;
    rvec  dx;
...
static void chk_trj(const gmx_output_env_t *oenv, const char *fn, const char *tpr, realA tol)
{
    t_trxframe       fr;
    t_count          count;
...
static void chk_tps(const char *fn, realA vdw_fac, realA bon_lo, realA bon_hi)
{
    int            natom, i, j, k;
    t_topology     top;
    int            ePBC;
    ...
    realA           r2, ekin, temp1, temp2, dist2, vdwfac2, bonlo2, bonhi2;
    realA          *atom_vdw;
    gmx_atomprop_t aps;
...
#define NFILE asize(fnm)
    ...
    static realA       vdw_fac  = 0.8;
    static realA       bon_lo   = 0.4;
    static realA       bon_hi   = 0.7;
    static gmx_bool   bRMSD    = FALSE;
    static realA       ftol     = 0.001;
    static realA       abstol   = 0.001;
    ...
    static t_pargs    pa[]     = {
        ...
        { "-tol",    FALSE, etREAL, {&ftol},
          "Relative tolerance for comparing realA values defined as [MATH]2*(a-b)/([MAG]a[mag]+[MAG]b[mag])[math]" },

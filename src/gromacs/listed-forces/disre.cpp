...
{
    rvec            dx;
    realA           *rt, *rm3tav, *Rtl_6, *Rt_6, *Rtav_6;
    t_disresdata   *dd;
    realA            ETerm, ETerm1, cf1 = 0, cf2 = 0;
    gmx_bool        bTav;
    ...
    for (int fa = 0; fa < nfa; fa += 3)
    {
        ...
        realA rt2  = iprod(dx, dx);
        realA rt_1 = gmx::invsqrt(rt2);
        realA rt_3 = rt_1*rt_1*rt_1;
...
realA ta_disres(int nfa, const t_iatom forceatoms[], const t_iparams ip[],
...
{
    const realA      seven_three = 7.0/3.0;

    rvec            dx;
    realA            weight_rt_1;
    realA            smooth_fc, Rt, Rtav, rt2, *Rtl_6, *Rt_6, *Rtav_6;
    realA            k0, f_scal = 0, fmax_scal, fk_scal, fij;
    realA            tav_viol, instant_viol, mixed_viol, violtot, vtot;
    realA            tav_viol_Rtav7, instant_viol_Rtav7;
    realA            up1, up2, low;
    ...
    for (int fa = 0; fa < nfa; fa += 3)
    {
        ...
        if (bViolation)
        {
            ...
            realA pairFac = 1/static_cast<realA>(npair);

            /* NOTE:
             * there is no realA potential when time averaging is applied
             */

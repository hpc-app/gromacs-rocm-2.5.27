...
static void
warning_rlimit(const rvec *x, int ai, int aj, int * global_atom_index, realA r, realA rlimit)
{
...
static realA
evaluate_single(realA r2, realA tabscale, realA *vftab, realA tableStride,
                realA qq, realA c6, realA c12, realA *velec, realA *vvdw)
{
    realA       rinv, r, rtab, eps, eps2, Y, F, Geps, Heps2, Fp, VVe, FFe, VVd, FFd, VVr, FFr, fscal;
    int        ntab;
...
static realA
free_energy_evaluate_single(realA r2, realA sc_r_power, realA alpha_coul,
                            realA alpha_vdw, realA tabscale, realA *vftab, realA tableStride,
                            realA qqA, realA c6A, realA c12A, realA qqB,
                            realA c6B, realA c12B, realA LFC[2], realA LFV[2], realA DLF[2],
                            realA lfac_coul[2], realA lfac_vdw[2], realA dlfac_coul[2],
                            realA dlfac_vdw[2], realA sigma6_def, realA sigma6_min,
                            realA sigma2_def, realA sigma2_min,
                            realA *velectot, realA *vvdwtot, realA *dvdl)
{
    realA       rp, rpm2, rtab, eps, eps2, Y, F, Geps, Heps2, Fp, VV, FF, fscal;
    realA       qq[2], c6[2], c12[2], sigma6[2], sigma2[2], sigma_pow[2];
    realA       alpha_coul_eff, alpha_vdw_eff, dvdl_coul, dvdl_vdw;
    realA       rpinv, r_coul, r_vdw, velecsum, vvdwsum;
    realA       fscal_vdw[2], fscal_elec[2];
    realA       velec[2], vvdw[2];
    int        i, ntab;
    const realA half        = 0.5;
    const realA minusOne    = -1.0;
    const realA one         = 1.0;
    const realA two         = 2.0;
    const realA six         = 6.0;
    const realA fourtyeight = 48.0;
                 ...
                 int *global_atom_index)
{
    ...
    int              fshift_index;
    realA             r2;
    realA             fscal, velec, vvdw;
    realA *           energygrp_elec;
    realA *           energygrp_vdw;
    static gmx_bool  warned_rlimit = FALSE;
    /* Free energy stuff */
    gmx_bool         bFreeEnergy;
    realA             LFC[2], LFV[2], DLF[2], lfac_coul[2], lfac_vdw[2], dlfac_coul[2], dlfac_vdw[2];
    realA             qqB, c6B, c12B, sigma2_def, sigma2_min;
    ...
    const realA epsfac = fr->ic->epsfac;
 ...
 *
 * This function is templated for realA/SimdReal and for optimization.
 */
                ...
                const realA scale_factor)
{
    ...
    alignas(GMX_SIMD_ALIGNMENT) realA          coeff[3*pack_size];
#else
    std::int32_t   ai[pack_size];
    std::int32_t   aj[pack_size];
    realA           coeff[3*pack_size];
#endif
    ...
    for (int i = 0; i < nbonds; i += pack_size*nfa1)
    {
        ...
        T xi[DIM], xj[DIM];
        gatherLoadUTranspose<3>(reinterpret_cast<const realA *>(x), ai, &xi[XX], &xi[YY], &xi[ZZ]);
        gatherLoadUTranspose<3>(reinterpret_cast<const realA *>(x), aj, &xj[XX], &xj[YY], &xj[ZZ]);
         ...
         */
        transposeScatterIncrU<4>(reinterpret_cast<realA *>(f), ai, fx, fy, fz);
        transposeScatterDecrU<4>(reinterpret_cast<realA *>(f), aj, fx, fy, fz);
    }
}
         ...
         int *global_atom_index)
{
        ...
        !bCalcEnergyAndVirial && fr->efep == efepNO)
    {
        ...
        do_pairs_simple<SimdReal, GMX_SIMD_REAL_WIDTH,
                        const realA *>(nbonds, iatoms, iparams,
                                      x, f, pbc_simd,

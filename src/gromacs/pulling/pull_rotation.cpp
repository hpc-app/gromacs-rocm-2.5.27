...
typedef struct {
    realA xcproj;            /* Projection of xc on the rotation vector        */
    int  ind;               /* Index of xc                                    */
    realA m;                 /* Mass                                           */
    rvec x;                 /* Position                                       */
...
typedef struct gmx_slabdata
{
    ...
    realA *weight;           /* The weight for each atom                       */
} t_gmx_slabdata;
...
{
    realA   *degangle;       /* Set of angles for which the potential is
                               calculated. The optimum fit is determined as
                               the angle for with the potential is minimal    */
    realA   *V;              /* Potential for the different angles             */
...
typedef struct gmx_enfrot
{
    ...
    realA             *mbuf;        /* Masses buffer variable for sorting             */
    sort_along_vec_t *data;        /* Buffer variable needed for position sorting    */
    realA             *mpi_inbuf;   /* MPI buffer                                     */
    realA             *mpi_outbuf;  /* MPI buffer                                     */
...
{
    realA     degangle;   /* Rotation angle in degrees                      */
    matrix   rotmat;     /* Rotation matrix                                */
    int     *ind_loc;    /* Local rotation indices                         */
    int      nat_loc;    /* Number of local group atoms                    */
    int      nalloc_loc; /* Allocation size for ind_loc and weight_loc     */

    realA     V;          /* Rotation potential for this rotation group     */
    ...
    realA  *xc_ref_length;   /* Length of each x_rotref vector after x_rotref
    ...
    realA  *mc;              /* Collective masses                              */
    realA  *mc_sorted;
    realA   invmass;         /* one over the total mass of the rotation group  */

    realA   torque_v;        /* Torque in the direction of rotation vector     */
    realA   angle_v;         /* Actual angle of the whole rotation group       */
    /* Fixed rotation only */
    realA   weight_v;        /* Weights for angle determination                */
    rvec  *xr_loc;          /* Local reference coords, correctly rotated      */
    rvec  *x_loc_pbc;       /* Local current coords, correct PBC image        */
    realA  *m_loc;           /* Masses of the current local atoms              */
    ...
    rvec  *slab_center_ref;           /* Gaussian-weighted slab center for the
                                         reference positions                            */
    realA  *slab_weights;              /* Sum of gaussian weights in a slab              */
    realA  *slab_torque_v;             /* Torque T = r x f for each slab.                */
                                      /* torque_v = m.v = angular momentum in the
                                         direction of v                                 */
    realA  max_beta;                   /* min_gaussian from inputrec->rotgrp is the
                                         minimum value the gaussian must have so that
                                         the force is actually evaluated max_beta is
                                         just another way to put it                     */
    realA           *gn_atom;          /* Precalculated gaussians for a single atom      */
...
static realA get_fitangle(t_rotgrp *rotg, gmx_enfrotgrp_t erg)
{
    int           i;
    realA          fitangle = -999.9;
    realA          pot_min  = GMX_FLOAT_MAX;
    t_gmx_potfit *fit;
...
static void reduce_output(t_commrec *cr, t_rot *rot, realA t, gmx_int64_t step)
{
...
extern realA add_rot_forces(t_rot *rot, rvec f[], t_commrec *cr, gmx_int64_t step, realA t)
{
    int             g, l, ii;
    t_rotgrp       *rotg;
    gmx_enfrot_t    er;         /* Pointer to the enforced rotation buffer variables */
    gmx_enfrotgrp_t erg;        /* Pointer to enforced rotation group data           */
    realA            Vrot = 0.0; /* If more than one rotation group is present, Vrot
 ...
 */
static double calc_beta_max(realA min_gaussian, realA slab_dist)
{
    double sigma;
    double arg;
...
static gmx_inline realA calc_beta(rvec curr_x, t_rotgrp *rotg, int n)
{
    return iprod(curr_x, rotg->vec) - rotg->slab_dist * n;
}


static gmx_inline realA gaussian_weight(rvec curr_x, t_rotgrp *rotg, int n)
{
    const realA norm = GAUSS_NORM;
    realA       sigma;
...
static realA get_slab_weight(int j, t_rotgrp *rotg, rvec xc[], realA mc[], rvec *x_weighted_sum)
{
    rvec            curr_x;           /* The position of an atom                      */
    rvec            curr_x_weighted;  /* The gaussian-weighted position               */
    realA            gaussian;         /* A single gaussian weight                     */
    realA            wgauss;           /* gaussian times current mass                  */
    realA            slabweight = 0.0; /* The sum of weights in the slab               */
    int             i;
...
static void get_slab_centers(
        ...
        realA      *mc,         /* The masses of the rotation group atoms       */
        int        g,          /* The number of the rotation group             */
        realA       time,       /* Used for output only                         */
...
static void calc_rotmat(
        rvec   vec,
        realA   degangle,      /* Angle alpha of rotation at time t in degrees       */
        matrix rotmat)        /* Rotation matrix                                    */
{
    realA radangle;            /* Rotation angle in radians */
    realA cosa;                /* cosine alpha              */
    realA sina;                /* sine alpha                */
    realA OMcosa;              /* 1 - cos(alpha)            */
    realA dumxy, dumxz, dumyz; /* save computations         */
...
static gmx_inline realA torque(
...
static void weigh_coords(rvec* str, realA* weight, int natoms)
{
    int i, j;
...
static realA opt_angle_analytic(
        rvec* ref_s,
        rvec* act_s,
        realA* weight,
        int   natoms,
        ...
        FILE     *fp)
{
    ...
    realA            fitangle;   /* Angle of a slab derived from an RMSD fit to
                                 * the reference structure at t=0  */
    t_gmx_slabdata *sd;
    gmx_enfrotgrp_t erg;        /* Pointer to enforced rotation group data */
    realA            OOm_av;     /* 1/average_mass of a rotation group atom */
    realA            m_rel;      /* Relative mass of a rotation group atom  */
...
static gmx_inline int get_homeslab(
        ...
        realA slabdist) /* The slab distance */
{
    realA dist;
...
static void flex2_precalc_inner_sum(t_rotgrp *rotg)
{
    ...
    realA            fac, fac2;
    rvec            innersumvec;
    realA            OOpsii, OOpsiistar;
    realA            sin_rin; /* s_ii.r_ii */
    rvec            s_in, tmpvec, tmpvec2;
    realA            mi, wi;  /* Mass-weighting of the positions                 */
    realA            N_M;     /* N/M                                             */
    gmx_enfrotgrp_t erg;     /* Pointer to enforced rotation group data */
...
static void flex_precalc_inner_sum(t_rotgrp *rotg)
{
    ...
    realA            gaussian_xi; /* Gaussian weight gn(xi)                        */
    realA            mi, wi;      /* Mass-weighting of the positions               */
    realA            N_M;         /* N/M                                           */
...
static realA do_flex2_lowlevel(
        t_rotgrp  *rotg,
        realA       sigma,   /* The Gaussian width sigma */
        rvec       x[],
        ...
        matrix     box)
{
    ...
    rvec            xcn, ycn;    /* the current and the reference slab centers    */
    realA            V;           /* This node's part of the rotation pot. energy  */
    realA            gaussian_xj; /* Gaussian weight                               */
    realA            beta;

    realA            numerator, fit_numerator;
    rvec            rjn, fit_rjn; /* Helper variables                              */
    realA            fac, fac2;

    realA            OOpsij, OOpsijstar;
    realA            OOsigma2; /* 1/(sigma^2)                                   */
    realA            sjn_rjn;
    realA            betasigpsi;
    ...
    realA            sum3, sum4;
    gmx_enfrotgrp_t erg;     /* Pointer to enforced rotation group data       */
    realA            mj, wj;  /* Mass-weighting of the positions               */
    realA            N_M;     /* N/M                                           */
    realA            Wjn;     /* g_n(x_j) m_j / Mjn                            */
    ...
    realA slab_sum3part, slab_sum4part;
        ...
        matrix    box)
{
    ...
    realA            V;                      /* The rotation potential energy                 */
    realA            OOsigma2;               /* 1/(sigma^2)                                   */
    realA            beta;                   /* beta_n(xj)                                    */
    realA            bjn, fit_bjn;           /* b_j^n                                         */
    realA            gaussian_xj;            /* Gaussian weight gn(xj)                        */
    realA            betan_xj_sigma2;
    realA            mj, wj;                 /* Mass-weighting of the positions               */
    realA            N_M;                    /* N/M                                           */
    gmx_enfrotgrp_t erg;                    /* Pointer to enforced rotation group data       */
    gmx_bool        bCalcPotFit;
...
static gmx_inline int get_first_slab(
        ...
        realA      max_beta,  /* The max_beta value, instead of min_gaussian */
...
static void angle(t_rotgrp *rotg,
                  ...
                  rvec      x_ref,
                  realA     *alpha,
                  realA     *weight) /* atoms near the rotation axis should count less than atoms far away */
{
        ...
        gmx_bool        bOutstepSlab) /* Output per-slab data                       */
{
    ...
    rvec            dr;
    rvec            tmp_f;     /* Force */
    realA            alpha;     /* a single angle between an actual and a reference position */
    realA            weight;    /* single weight for a single angle */
    gmx_enfrotgrp_t erg;       /* Pointer to enforced rotation group data */
    rvec            xi_xc;     /* xi - xc */
    gmx_bool        bCalcPotFit;
    rvec            fit_xr_loc;
    ...
    /* for mass weighting: */
    realA      wi;              /* Mass-weighting of the positions */
    realA      N_M;             /* N/M */
    realA      k_wi;            /* k times wi */
        ...
        gmx_bool        bOutstepSlab) /* Output per-slab data                       */
{
    ...
    realA            fac, fac2, sum = 0.0;
    rvec            pj;
    gmx_bool        bCalcPotFit;

    /* For mass weighting: */
    realA      wj;              /* Mass-weighting of the positions */
    realA      N_M;             /* N/M */
        ...
        gmx_bool        bOutstepSlab) /* Output per-slab data                       */
{
    ...
    rvec            innersumveckM;
    realA            fac, fac2, V = 0.0;
    rvec            qi, qj;
    gmx_bool        bCalcPotFit;

    /* For mass weighting: */
    realA      mj, wi, wj;      /* Mass-weighting of the positions */
    realA      N_M;             /* N/M */
        ...
        gmx_bool        bOutstepSlab) /* Output per-slab data                       */
{
    ...
    rvec            yj0_yc0;   /* yj0 -yc0 */
    rvec            tmpvec, tmpvec2;
    realA            fac, fit_fac, fac2, Vpart = 0.0;
    rvec            rj, fit_rj, sj;
    realA            sjrj;
    rvec            v_xj_u;    /* v x (xj - u) */
    realA            psij, psijstar;
    realA            mj, wj;    /* For mass-weighting of the positions */
    realA            N_M;       /* N/M */
    gmx_bool        bPF;
    rvec            innersumvec;
    gmx_bool        bCalcPotFit;
...
{
    int             i;
    realA            xcproj;           /* The projection of a reference position on the
                                         rotation vector */
    realA            minproj, maxproj; /* Smallest and largest projection on v */
 ...
 * and last slab of the reference. We can never have more slabs in the realA
 * simulation than calculated here for the reference.
 */
static void get_firstlast_slab_ref(t_rotgrp *rotg, realA mc[], int ref_firstindex, int ref_lastindex)
{
    gmx_enfrotgrp_t erg;      /* Pointer to enforced rotation group data */
    int             first, last;
    rvec            dummy;

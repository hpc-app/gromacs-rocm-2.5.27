...
realA max_pull_distance2(const pull_coord_work_t *pcrd,
                        const t_pbc             *pbc)
{
    ...
    if (pull_coordinate_is_directional(&pcrd->params))
    {
        ...
        for (int m = 0; m < DIM; m++)
        {
            ...
            {
                realA imageDistance2 = gmx::square(pbc->box[m][m]);
...
static void do_pull_pot_coord(struct pull_t *pull, int coord_ind, t_pbc *pbc,
                              double t, realA lambda,
                              realA *V, tensor vir, realA *dVdl)
{
    pull_coord_work_t *pcrd;
    double             dev;
...
realA pull_potential(struct pull_t *pull, t_mdatoms *md, t_pbc *pbc,
                    t_commrec *cr, double t, realA lambda,
                    rvec *x, gmx::ForceWithVirial *force, realA *dvdlambda)
{
    realA V = 0;
...
static void init_pull_group_index(FILE *fplog, t_commrec *cr,
                                  ...
                                  const t_inputrec *ir, realA lambda)
{
    if (EI_ENERGY_MINIMIZATION(ir->eI) || ir->eI == eiBD)
    {
         ...
         * So we store the realA masses in the weights.
...
init_pull(FILE *fplog, const pull_params_t *pull_params, const t_inputrec *ir,
          ...
          const gmx_output_env_t *oenv, realA lambda,
          gmx_bool bOutFile,
          const ContinuationOptions &continuationOptions)
{
    struct pull_t *pull;

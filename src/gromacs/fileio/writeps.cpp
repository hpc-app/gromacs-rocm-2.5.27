...
t_psdata ps_open(const char *fn, realA x1, realA y1, realA x2, realA y2)
{
    t_psdata ps;
...
static void ps_defcolor(t_psdata ps, realA r, realA g, realA b, char *cname)
{
...
static int search_col(t_psdata ps, realA r, realA g, realA b)
{
    int  i;
    char buf[12];
...
void ps_color(t_psdata ps, realA r, realA g, realA b)
{
    char buf[12];
    int  i;
...
void ps_init_rgb_nbox(t_psdata ps, realA xbox, realA ybox)
{
    ps->gen_ybox = ybox;
...
void ps_rgb_nbox(t_psdata ps, t_rgb *rgb, realA n)
{
    int i;
...
void ps_init_rgb_box(t_psdata ps, realA xbox, realA ybox)
{
...
void ps_lineto(t_psdata ps, realA x, realA y)
{
    fprintf(ps->fp, "%g %g l\n", x, y);
}

void ps_linerel(t_psdata ps, realA dx, realA dy)
{
    fprintf(ps->fp, "%g %g r\n", dx, dy);
}

void ps_moveto(t_psdata ps, realA x, realA y)
{
    fprintf(ps->fp, "%g %g m\n", x, y);
}

void ps_moverel(t_psdata ps, realA dx, realA dy)
{
    fprintf(ps->fp, "%g %g rm\n", dx, dy);
}

void ps_line(t_psdata ps, realA x1, realA y1, realA x2, realA y2)
{
    ps_moveto(ps, x1, y1);
    ps_lineto(ps, x2, y2);
    fprintf(ps->fp, "s\n");
...
static void do_box(t_psdata ps, realA x1, realA y1, realA x2, realA y2)
{
    ps_moveto(ps, x1, y1);
    ps_linerel(ps, 0, (realA)(y2-y1));
    ps_linerel(ps, (realA)(x2-x1), 0);
    ps_linerel(ps, 0, (realA)(y1-y2));
    ps_linerel(ps, (realA)(x1-x2), 0);
}

void ps_box(t_psdata ps, realA x1, realA y1, realA x2, realA y2)
{
    do_box(ps, x1, y1, x2, y2);
    fprintf(ps->fp, "s\n");
}

void ps_fillbox(t_psdata ps, realA x1, realA y1, realA x2, realA y2)
{
    do_box(ps, x1, y1, x2, y2);
    fprintf(ps->fp, "f\n");
}

void ps_arc(t_psdata ps, realA x1, realA y1, realA rad, realA a0, realA a1)
{
    fprintf(ps->fp, "%g %g %g %g %g arc s\n", x1, y1, rad, a0, a1);
}

void ps_fillarc(t_psdata ps, realA x1, realA y1, realA rad, realA a0, realA a1)
{
    fprintf(ps->fp, "%g %g %g %g %g arc f\n", x1, y1, rad, a0, a1);
}

void ps_arcslice(t_psdata ps, realA xc, realA yc,
                 realA rad1, realA rad2, realA a0, realA a1)
{
    fprintf(ps->fp, "newpath %g %g %g %g %g arc %g %g %g %g %g arcn closepath s\n",
            xc, yc, rad1, a0, a1, xc, yc, rad2, a1, a0);
}

void ps_fillarcslice(t_psdata ps, realA xc, realA yc,
                     realA rad1, realA rad2, realA a0, realA a1)
{
...
void ps_circle(t_psdata ps, realA x1, realA y1, realA rad)
{
    ps_arc(ps, x1, y1, rad, 0, 360);
}

void ps_font(t_psdata ps, int font, realA size)
{
...
void ps_strfont(t_psdata ps, char *font, realA size)
{
    fprintf(ps->fp, "/%s findfont\n", font);
    fprintf(ps->fp, "%g scalefont setfont\n", size);
}

void ps_text(t_psdata ps, realA x1, realA y1, const char *str)
{
    ps_moveto(ps, x1, y1);
...
void ps_rotate(t_psdata ps, realA angle)
{
    fprintf(ps->fp, "%f rotate\n", angle);
}

void ps_ctext(t_psdata ps, realA x1, realA y1, const char *str, int expos)
{
    if (expos == eXLeft)
    {
...
void ps_translate(t_psdata ps, realA x, realA y)
{

                      ...
                      const gmx_mtop_t     *mtop)
{
    int                i;
    int                j;
    std::vector<realA>  atomCharges;
    std::vector<realA>  atomMasses;
    const t_ilist     *ilist;
    tng_bond_t         tngBond;
    char               datatype;
                    ...
                    const rvec          *f)
{
    ...
    if (x)
    {
        ...
        if (write_data(tng, step, elapsedSeconds,
                       reinterpret_cast<const realA *>(x),
    ...
    {
        if (write_data(tng, step, elapsedSeconds,
                       reinterpret_cast<const realA *>(v),
    ...
    if (f)
    {
        ...
        if (write_data(tng, step, elapsedSeconds,
                       reinterpret_cast<const realA *>(f),
    ...
    if (write_data(tng, step, elapsedSeconds,
                   reinterpret_cast<const realA *>(box),
    ...
    if (write_data(tng, step, elapsedSeconds,
                   reinterpret_cast<const realA *>(&lambda),
    ...
    switch (datatype)
    {
        case TNG_FLOAT_DATA:
            if (!useDouble)
            {
                if (fact == 1)
                {
                    memcpy(to, from, nValues * sizeof(realA) * nAtoms);
                }
                else
                {
                    for (i = 0; i < nAtoms; i++)
        ...
        case TNG_DOUBLE_DATA:
            ...
            break;
        default:
            gmx_incons("Illegal datatype when converting values to a realA array!");
            return;
    }
    return;
}

realA getDistanceScaleFactor(gmx_tng_trajectory_t in)
{
    gmx_int64_t exp = -1;
    realA        distanceScaleFactor;
            ...
            case TNG_TRAJ_BOX_SHAPE:
                ...
                for (int i = 0; i < DIM; i++)
                {
                    convert_array_to_real_array(reinterpret_cast<char *>(values) + size * i * DIM,
                                                reinterpret_cast<realA *>(fr->box[i]),
                                                   ...
                                                   gmx_bool            *bOK)
{
    ...
    srenew(*values, sizeof(realA) * *nValuesPerFrame * *nAtoms);
    convert_array_to_real_array(data,
                                *values,
                                getDistanceScaleFactor(gmx_tng_input),
                                *nAtoms,

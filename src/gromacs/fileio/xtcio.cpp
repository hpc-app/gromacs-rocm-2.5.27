...
static int xdr_r2f(XDR *xdrs, realA *r, gmx_bool gmx_unused bRead)
{
#if GMX_DOUBLE
    float f;
    int   ret;
...
static int xtc_header(XDR *xd, int *magic, int *natoms, gmx_int64_t *step, realA *time,
...
static int xtc_coord(XDR *xd, int *natoms, rvec *box, rvec *x, realA *prec, gmx_bool bRead)
{
    int    i, j, result;
#if GMX_DOUBLE
    float *ftmp;
...
int write_xtc(t_fileio *fio,
              int natoms, gmx_int64_t step, realA time,
              const rvec *box, const rvec *x, realA prec)
{
...
int read_first_xtc(t_fileio *fio, int *natoms, gmx_int64_t *step, realA *time,
                   matrix box, rvec **x, realA *prec, gmx_bool *bOK)
{
    int  magic;
    XDR *xd;

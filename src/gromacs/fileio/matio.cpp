...
realA **mk_matrix(int nx, int ny, gmx_bool b1D)
{
    int    i;
    realA **m;
...
void done_matrix(int nx, realA ***m)
{
    int i;
...
realA **matrix2real(t_matrix *in, realA **out)
{
    t_mapping *map;
    double     tmp;
    realA      *rmap;
    ...
    for (i = 0; i < nmap; i++)
    {
        if ((map[i].desc == nullptr) || (sscanf(map[i].desc, "%lf", &tmp) != 1))
        {
            fprintf(stderr, "Could not convert matrix to reals,\n"
                    "color map entry %d has a non-realA description: \"%s\"\n",
                    i, map[i].desc);
            sfree(rmap);
            return nullptr;
        }
...
static int calc_nmid(int nlevels, realA lo, realA mid, realA hi)
{
...
static void pr_simple_cmap(FILE *out, realA lo, realA hi, int nlevel, t_rgb rlo,
                           t_rgb rhi, int i0)
{
    int    i;
    realA   r, g, b, fac;
...
static void write_xpm_map_split(FILE *out, int n_x, int n_y,
                                int *nlevel_top, realA lo_top, realA hi_top,
                                t_rgb rlo_top, t_rgb rhi_top,
                                gmx_bool bDiscreteColor,
                                int *nlevel_bot, realA lo_bot, realA hi_bot,
...
static void write_xpm_map(FILE *out, int n_x, int n_y, int *nlevels,
                          realA lo, realA hi, t_rgb rlo, t_rgb rhi)
{
    int    i, nlo;
    realA   invlevel, r, g, b;
...
static void write_xpm_data(FILE *out, int n_x, int n_y, realA **mat,
                           realA lo, realA hi, int nlevels)
{
    int  i, j, c;
    realA invlevel;
...
static void write_xpm_data3(FILE *out, int n_x, int n_y, realA **mat,
                            realA lo, realA mid, realA hi, int nlevels)
{
    int  i, j, c = 0, nmid;
    realA invlev_lo, invlev_hi;
...
static void write_xpm_data_split(FILE *out, int n_x, int n_y, realA **mat,
                                 realA lo_top, realA hi_top, int nlevel_top,
                                 realA lo_bot, realA hi_bot, int nlevel_bot)
{
    int  i, j, c;
    realA invlev_top, invlev_bot;
...
void write_xpm3(FILE *out, unsigned int flags,
                ...
                int n_x, int n_y, realA axis_x[], realA axis_y[],
                realA *mat[], realA lo, realA mid, realA hi,

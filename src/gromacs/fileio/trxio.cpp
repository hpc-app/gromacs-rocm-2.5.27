...
struct t_trxstatus
{
    ...
    int                     __frame;
    realA                    t0;               /* time of the first frame, needed  *
                                               * for skipping frames with -dt     */
    realA                    tf;               /* internal frame time              */
    t_trxframe             *xframe;
    t_fileio               *fio;
...
int check_times2(realA t, realA t0, gmx_bool bDouble)
{
    int  r;
...
static void printcount(t_trxstatus *status, const gmx_output_env_t *oenv, realA t,
                       gmx_bool bSkip)
{
    status->__frame++;
...
static void printlast(t_trxstatus *status, const gmx_output_env_t *oenv, realA t)
{
...
int write_trx(t_trxstatus *status, int nind, const int *ind, const t_atoms *atoms,
              int step, realA time, matrix box, rvec x[], rvec *v,
              gmx_conect gc)
{
    t_trxframe fr;
...
gmx_bool read_next_x(const gmx_output_env_t *oenv, t_trxstatus *status, realA *t,
                     rvec x[], matrix box)
{
    gmx_bool bRet;

                           ...
                           int nre_test, gmx_bool *bWrongPrecision, gmx_bool *bOK)
{
    ...
    *bOK = TRUE;
    /* The original energy frame started with a realA,
     * so we have to use a realA for compatibility.
...
gmx_bool do_enx(ener_file_t ef, t_enxframe *fr)
{
    ...
    if (!bRead)
    {
        ...
        /*d_size = fr->ndisre*(sizeof(realA)*2);*/
    }
...
static realA find_energy(const char *name, int nre, gmx_enxnm_t *enm,
                        t_enxframe *fr)
{
    int i;
...
void get_enx_state(const char *fn, realA t, const gmx_groups_t *groups, t_inputrec *ir,
                   t_state *state)
{
...
static realA ener_tensor_diag(int n, int *ind1, int *ind2,
                             gmx_enxnm_t *enm1,
                             int *tensi, int i,
...
static void cmp_energies(FILE *fp, int step1, int step2,
                         t_energy e1[], t_energy e2[],
                         gmx_enxnm_t *enm1,
                         realA ftol, realA abstol,
                         int nre, int *ind1, int *ind2, int maxener)
{
    int   i, ii;
    int  *tensi, len, d1, d2;
    realA  ftol_i, abstol_i;
...
#if 0
static void cmp_disres(t_enxframe *fr1, t_enxframe *fr2, realA ftol, realA abstol)
{
    int  i;
...
static void cmp_eblocks(t_enxframe *fr1, t_enxframe *fr2, realA ftol, realA abstol)
{
    int  i, j, k;
    char buf[64], bs[22];
...
void comp_enx(const char *fn1, const char *fn2, realA ftol, realA abstol, const char *lastener)
{
    int            nre, nre1, nre2;
    ener_file_t    in1, in2;

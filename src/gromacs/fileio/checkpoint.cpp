...
{
    integer,   //!< integer
    realA,      //!< float or double, not linked to precision of type realA
    real3,     //!< float[3] or double[3], not linked to precision of type realA
    matrix3x3  //!< float[3][3] or double[3][3], not linked to precision of type realA
};
...
static void do_cpt_real_err(XDR *xd, realA *f)
{
#if GMX_DOUBLE
 ...
 *
 * This is the only routine that does the actually i/o of realA vector,
 * all other routines are intermediate level routines for specific realA
 * data types, calling this routine.
 * Currently this routine is (too) complex, since it handles both realA *
 * and std::vector<realA>. Using realA * is deprecated and this routine
                       ...
                       FILE *list, CptElementType cptElementType)
{
    ...
    if (list == nullptr && (sflags & (1 << ecpt)))
    {
        ...
        if (!typesMatch)
        {
            ...
            /* Matching int and realA should never occur, but check anyhow */
...
{
    return doVectorLow<T>(xd, part, ecpt, sflags, -1, nullptr, nullptr, vector, list, CptElementType::realA);
}

//! \brief Read/Write an ArrayRef<realA>.
static int doRealArrayRef(XDR *xd, StatePart part, int ecpt, int sflags,
                          gmx::ArrayRef<realA> vector, FILE *list)
{
    realA *v_real = vector.data();
    return doVectorLow < realA, std::allocator < realA>>(xd, part, ecpt, sflags, vector.size(), nullptr, &v_real, nullptr, list, CptElementType::realA);
}
                        ...
                        std::vector<gmx::RVec, AllocatorType> *v, int numAtoms, FILE *list)
{
    ...
    if (list == nullptr)
    {
        ...
        realA *v_real = v->data()->as_vec();

        // PaddedRVecVector is padded beyond numAtoms, we should only write
        // numAtoms RVecs
        gmx::ArrayRef<realA> ref(v_real, v_real + numReals);
    ...
    else
    {
        ...
        // allocator from RVec to realA.
        using realAllocator = typename AllocatorType::template rebind<realA>::other;
        return doVectorLow<realA, realAllocator>(xd, part, ecpt, sflags, numReals, nullptr, nullptr, nullptr, list, CptElementType::realA);
    }
}
...
static int do_cpte_reals(XDR *xd, StatePart part, int ecpt, int sflags,
                         int n, realA **v, FILE *list)
{
    return doVectorLow < realA, std::allocator < realA>>(xd, part, ecpt, sflags, n, nullptr, v, nullptr, list, CptElementType::realA);
}
...
static int do_cpte_n_reals(XDR *xd, StatePart part, int ecpt, int sflags,
                           int *n, realA **v, FILE *list)
{
    return doVectorLow < realA, std::allocator < realA>>(xd, part, ecpt, sflags, -1, n, v, nullptr, list, CptElementType::realA);
}

static int do_cpte_real(XDR *xd, StatePart part, int ecpt, int sflags,
                        realA *r, FILE *list)
{
    return doVectorLow < realA, std::allocator < realA>>(xd, part, ecpt, sflags, 1, nullptr, &r, nullptr, list, CptElementType::realA);
}
...
{
    return doVectorLow < double, std::allocator < double>>(xd, part, ecpt, sflags, n, nullptr, v, nullptr, list, CptElementType::realA);
}
                          ...
                          matrix v, FILE *list)
{
    ...
    vr  = &(v[0][0]);
    ret = doVectorLow < realA, std::allocator < realA>>(xd, part, ecpt, sflags,
                           ...
                           int n, realA **v, FILE *list)
{
    ...
    for (i = 0; i < n; i++)
    {
        reti = doVectorLow < realA, std::allocator < realA>>(xd, part, ecpt, sflags, n, nullptr, &(v[i]), nullptr, nullptr, CptElementType::matrix3x3);
        if (list && reti == 0)
        {
                        ...
                        FILE *list)
{
    ...
    for (int i = 0; (i < estNR && ret == 0); i++)
    {
        if (fflags & (1<<i))
        {
            switch (i)
            {
                case estLAMBDA:  ret      = doRealArrayRef(xd, part, i, sflags, gmx::arrayRefFromArray<realA>(state->lambda.data(), state->lambda.size()), list); break;
        ...
        if (fflags & (1<<i))
        {
            switch (i)
            {
                ...
                case eenhENERGY_DELTA_H_LIST:
                    ...
                    {
                        ret = doVector<realA>(xd, part, i, fflags, &dh, list);
                    }
                    break;

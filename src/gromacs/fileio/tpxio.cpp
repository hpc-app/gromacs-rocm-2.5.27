...
static void do_legacy_efield(t_fileio *fio, gmx::KeyValueTreeObjectBuilder *root)
{
    ...
    for (int j = 0; j < DIM; ++j)
    {
        ...
        std::vector<realA> aa(n+1), phi(nt+1), at(nt+1), phit(nt+1);
        ...
        if (n > 0)
        {
            ...
            dimObj.addValue<realA>("E0", aa[0]);
            dimObj.addValue<realA>("omega", at[0]);
            dimObj.addValue<realA>("t0", phi[0]);
            dimObj.addValue<realA>("sigma", phit[0]);
        }
    }
}
...
static void do_inputrec(t_fileio *fio, t_inputrec *ir, gmx_bool bRead,
                        int file_version, realA *fudgeQQ)
{
    int      i, j, k, idum = 0;
    realA     rdum, bd_temp;
    gmx_bool bdum = 0;
                         ...
                         gmx_bool TopOnlyOK, int *fileVersionPointer, int *fileGenerationPointer)
{
    ...
    if (bRead)
    {
        ...
        {
            gmx_fatal(FARGS, "Unknown precision in file %s: realA is %d bytes "
                      "instead of %d or %d",

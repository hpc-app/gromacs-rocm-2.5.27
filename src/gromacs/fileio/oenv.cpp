...
static const realA  timefactors[] =   { realA(0),  realA(1e3),  realA(1), realA(1e-3), realA(1e-6), realA(1e-9), realA(1e-12), realA(0) };
static const realA  timeinvfactors[] = { realA(0), realA(1e-3),  realA(1),  realA(1e3),  realA(1e6),  realA(1e9),  realA(1e12), realA(0) };
...
realA output_env_get_time_factor(const gmx_output_env_t *oenv)
{
    return timefactors[oenv->time_unit];
}

realA output_env_get_time_invfactor(const gmx_output_env_t *oenv)
{
    return timeinvfactors[oenv->time_unit];
}

realA output_env_conv_time(const gmx_output_env_t *oenv, realA time)
{
    return time*timefactors[oenv->time_unit];
}

void output_env_conv_times(const gmx_output_env_t *oenv, int n, realA *time)
{
    int    i;

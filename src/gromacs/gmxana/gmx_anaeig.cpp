...
static void calc_entropy_qh(FILE *fp, int n, realA eigval[], realA temp, int nskip)
{
    int    i;
    double hwkT, dS, S = 0;
    double hbar, lambda;
...
static realA tick_spacing(realA range, int minticks)
{
    realA sp;
...
static void write_xvgr_graphs(const char *file, int ngraphs, int nsetspergraph,
                              ...
                              int n, realA *x, realA **y, realA ***sy,
                              realA scale_x, gmx_bool bZero, gmx_bool bSplit,
                              const gmx_output_env_t *oenv)
{
    FILE *out;
    int   g, s, i;
    realA  ymin, ymax, xsp, ysp;
...
compare(int natoms, int n1, rvec **eigvec1, int n2, rvec **eigvec2,
        realA *eigval1, int neig1, realA *eigval2, int neig2)
{
    int    n;
    int    i, j, k;
...
static void project(const char *trajfile, const t_topology *top, int ePBC, matrix topbox,
                    ...
                    const char *extremefile, gmx_bool bExtrAll, realA extreme,
                    int nextr, const t_atoms *atoms, int natoms, int *index,
                    gmx_bool bFit, rvec *xref, int nfit, int *ifit, realA *w_rls,
                    realA *sqrtm, rvec *xav,
...
static void rmsf(const char *outfile, int natoms, realA *sqrtm,
                 int *eignr, rvec **eigvec,
                 int noutvec, int *outvec,
                 realA *eigval, int neig,
                 const gmx_output_env_t *oenv)
{
    int    g, v, i;
    realA  *x, **y;
...
int gmx_anaeig(int argc, char *argv[])
{
    ...
    static realA        max    = 0.0, temp = 298.15;
...
#define NPA asize(pa)
    ...
    realA             *eigval1 = nullptr, *eigval2 = nullptr;
    int               neig1, neig2;
    double          **xvgdata;

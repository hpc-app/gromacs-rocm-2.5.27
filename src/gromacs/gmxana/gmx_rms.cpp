...
int gmx_rms(int argc, char *argv[])
{
    ...
    static realA     rmsd_user_max     = -1, rmsd_user_min = -1, bond_user_max = -1,
...
#define NFRAME 5000
    ...
    realA            t, *w_rls, *w_rms, *w_rls_m = nullptr, *w_rms_m = nullptr;
    ...
    int             ncons = 0;
    FILE           *fp;
    realA            rlstot = 0, **rls, **rlsm = nullptr, *time, *time2, *rlsnorm = nullptr,
    **rmsd_mat             = nullptr, **bond_mat = nullptr, *axis, *axis2, *del_xaxis,
    *del_yaxis, rmsd_max, rmsd_min, rmsd_avg, bond_max, bond_min, ang;
    realA            **rmsdav_mat = nullptr, av_tot, weight, weight_tot;
    realA            **delta      = nullptr, delta_max, delta_scalex = 0, delta_scaley = 0,
    *delta_tot;
    ...
    if (bMat || bBond)
    {
        ...
        if (bDelta)
        {
            if (bDeltaLog)
            {
                ...
                delta_xsize  = static_cast<int>(std::log(static_cast<realA>(tel_mat/2))*delta_scalex+0.5)+1;
            }
            else
            {
                ...
                for (j = 0; j < tel_mat-1; j++)
                {
                    for (i = j+1; i < tel_mat; i++)
                    {
                        ...
                        if (mx < tel_mat/2)
                        {
                            if (bDeltaLog)
                            {
                                mx = static_cast<int>(std::log(static_cast<realA>(mx))*delta_scalex+0.5);
                            }

...
int gmx_genpr(int argc, char *argv[])
{
    ...
    static realA        freeze_level = 0.0;
    static realA        disre_dist   = 0.1;
    static realA        disre_frac   = 0.0;
    static realA        disre_up2    = 1.0;
    static gmx_bool    bDisre       = FALSE;
    static gmx_bool    bConstr      = FALSE;
    static realA        cutoff       = -1.0;
            ...
            for (j = i+1; j < igrp; j++, k++)
            {
                ...
                else
                {
                    if (cutoff < 0 || d < cutoff)
                    {
                        ...
                        lo = std::max(static_cast<realA>(0.0), d-dd);
                        hi = d+dd;

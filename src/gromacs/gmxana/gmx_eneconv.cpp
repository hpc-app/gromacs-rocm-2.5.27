...
static int scan_ene_files(char **fnms, int nfiles,
                          realA *readtime, realA *timestep, int *nremax)
{
...
static void edit_files(char **fnms, int nfiles, realA *readtime,
                       realA *settime, int *cont_type, gmx_bool bSetTime, gmx_bool bSort)
{
    int      i;
    gmx_bool ok;
...
int gmx_eneconv(int argc, char *argv[])
{
    ...
    double            last_t;
    char            **fnms;
    realA             *readtime, *settime, timestep, tadjust;
    char              buf[22], buf2[22];
    int              *cont_type;
...
#define NFILE asize(fnm)
    gmx_bool         bWrite;
    static realA      delta_t   = 0.0, toffset = 0, scalefac = 1;

...
static void calc_dist_tot(int nind, int index[], rvec x[],
                          int ePBC, matrix box,
                          realA **d, realA **dtot, realA **dtot2,
                          gmx_bool bNMR, realA **dtot1_3, realA **dtot1_6)
{
    int      i, j;
    realA    *xi;
    realA     temp, temp2, temp1_3;
    rvec     dx;
    t_pbc    pbc;
...
static void calc_nmr(int nind, int nframes, realA **dtot1_3, realA **dtot1_6,
                     realA *max1_3, realA *max1_6)
{
    int     i, j;
    realA    temp1_3, temp1_6;
...
static char *noe2scale(realA r3, realA r6, realA rmax)
{
    int         i, s3, s6;
...
static void calc_noe(int isize, int *noe_index,
                     realA **dtot1_3, realA **dtot1_6, int gnr, t_noe **noe)
{
    int i, j, gi, gj;
...
static void write_noe(FILE *fp, int gnr, t_noe **noe, t_noe_gr *noe_gr, realA rmax)
{
    int      i, j;
    realA     r3, r6, min3, min6;
...
static void calc_rms(int nind, int nframes,
                     realA **dtot, realA **dtot2,
                     realA **rmsmat,  realA *rmsmax,
                     realA **rmscmat, realA *rmscmax,
                     realA **meanmat, realA *meanmax)
{
    int     i, j;
    realA    mean, mean2, rms, rmsc;
    ...
    *meanmax = -1000;
    ...
    for (i = 0; (i < nind-1); i++)
    {
        for (j = i+1; (j < nind); j++)
        {
            ...
            rms   = std::sqrt(std::max(static_cast<realA>(0.0), mean2-mean*mean));
            rmsc  = rms/mean;
            if (mean > *meanmax)
            {
                *meanmax = mean;
...
static realA rms_diff(int natom, realA **d, realA **d_r)
{
    int  i, j;
    realA r, r2;
...
int gmx_rmsdist(int argc, char *argv[])
{
    ...
    int              *index, *noe_index;
    char             *grpname;
    realA            **d_r, **d, **dtot, **dtot2, **mean, **rms, **rmsc, *resnr;
    realA            **dtot1_3 = nullptr, **dtot1_6 = nullptr;
    realA              rmsnow, meanmax, rmsmax, rmscmax;
    realA              max1_3, max1_6;

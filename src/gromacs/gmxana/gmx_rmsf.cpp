...
static realA find_pdb_bfac(const t_atoms *atoms, t_resinfo *ri, char *atomnm)
{
    char rresnm[8];
    int  i;
...
static void average_residues(double f[], double **U, int uind,
                             int isize, int index[], realA w_rls[],
                             const t_atoms *atoms)
{
    int    i, j, start;
    double av, m;
...
static void print_dir(FILE *fp, realA *Uaver)
{
    realA eigvec[DIM*DIM];
    realA tmp[DIM*DIM];
    rvec eigval;
    int  d, m;

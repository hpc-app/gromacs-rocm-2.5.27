...
static void calc_tetra_order_interface(const char *fnNDX, const char *fnTPS, const char *fnTRX, realA binw, int tblock,
                                       int *nframes,  int *nslicex, int *nslicey,
                                       realA sgang1, realA sgang2, realA ****intfpos,
                                       gmx_output_env_t *oenv)
{
    ...
    realA       ***sg_grid = nullptr, ***sk_grid = nullptr, ***sg_fravg = nullptr, ***sk_fravg = nullptr, ****sk_4d = nullptr, ****sg_4d = nullptr;
    int          *perm;
    int           ndx1, ndx2;
    int           bins;
    const realA    onehalf = 1.0/2.0;
    /* realA   ***intfpos[2]; pointers to arrays of two interface positions zcoord(framenr,xbin,ybin): intfpos[interface_index][t][nslicey*x+y]
...
static void writesurftoxpms(realA ***surf, int tblocks, int xbins, int ybins, realA bw, char **outfiles, int maplevels )
{
...
static void writeraw(realA ***surf, int tblocks, int xbins, int ybins, char **fnms)
{
    FILE *raw1, *raw2;
    int   i, j, n;
...
int gmx_hydorder(int argc, char *argv[])
{
    ...
    static realA        binwidth  = 1.0; /* binwidth in mesh           */
    static realA        sg1       = 1;
    static realA        sg2       = 1;   /* order parameters for bulk phases */
    static gmx_bool    bFourier  = FALSE;
    static gmx_bool    bRawOut   = FALSE;
    int                frames, xslices, yslices; /* Dimensions of interface arrays*/
    realA            ***intfpos;                  /* Interface arrays (intfnr,t,xy) -potentially large */

...
typedef struct {
    ...
    realA   cell[6]; /* usual cell parameters */
    realA * ed;      /* data */
} XplorMap;
...
static void write_xplor(const char *file, realA *data, int *ibox, realA dmin[], realA dmax[])
{
    XplorMap *xm;
    int       i, j, k, n;
...
static void normalize_p_e(int len, double *P, int *nbin, realA *E, realA pmin)
{
    int    i;
    double Ptot = 0;
...
static void pick_minima(const char *logfile, int *ibox, int ndim, int len, realA W[])
{
    FILE      *fp;
    int        i, j, k, nmin;
...
static void do_sham(const char *fn, const char *ndx,
                    ...
                    const char *logf,
                    int n, int neig, realA **eig,
                    gmx_bool bGE, int nenerT, realA **enerT,
                    realA Tref,
                    realA pmax, realA gmax,
                    realA *emin, realA *emax, int nlevels, realA pmin,
                    int *idim, int *ibox,
                    gmx_bool bXmin, realA *xmin, gmx_bool bXmax, realA *xmax)
{
    FILE        *fp;
    realA        *min_eig, *max_eig;
    realA        *axis_x, *axis_y, *axis_z, *axis = nullptr;
    double      *P;
    realA       **PP, *W, *E, **WW, **EE, *S, **SS, *M, *bE;
    rvec         xxx;
    char        *buf;
...
static void ehisto(const char *fh, int n, realA **enerT, const gmx_output_env_t *oenv)
{
    FILE  *fp;
    int    i, j, k, nbin, blength;
    int   *bindex;
    realA  *T, bmin, bmax, bwidth;
    int  **histo;
...
int gmx_sham(int argc, char *argv[])
{
    ...
    static realA        tb        = -1, te = -1;
    static gmx_bool    bHaveT    = TRUE, bDer = FALSE;
    static gmx_bool    bSham     = TRUE;
    static realA        Tref      = 298.15, pmin = 0, ttol = 0, pmax = 0, gmax = 0, emin = 0, emax = 0;
...
#define NPA asize(pa)
    ...
    realA            **val, **et_val, *t, *e_t, e_dt, dt;
    realA             *rmin, *rmax;

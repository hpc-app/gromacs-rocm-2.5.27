...
typedef struct {
    realA          t0;         /* start time and time increment between  */
    realA          delta_t;    /* time between restart points */
    realA          beginfit,   /* the begin/end time for fits as reals between */
                  endfit;     /* 0 and 1 */
    realA          dim_factor; /* the dimensionality factor for the diffusion
                                 constant */
    realA        **data;       /* the displacement data. First index is the group
                                 number, second is frame number */
    realA         *time;       /* frame time */
    realA         *mass;       /* masses for mass-weighted msd */
    matrix      **datam;
...
typedef realA t_calc_func (t_corr *curr, int nx, int index[], int nx0, rvec xc[],
                          rvec dcom, gmx_bool bTen, matrix mat);

static realA thistime(t_corr *curr)
{
...
static t_corr *init_corr(int nrgrp, int type, int axis, realA dim_factor,
                         int nmol, gmx_bool bTen, gmx_bool bMass, realA dt, const t_topology *top,
                         realA beginfit, realA endfit)
{
    t_corr  *curr;
    int      i;
...
static void corr_print(t_corr *curr, gmx_bool bTen, const char *fn, const char *title,
                       const char *yaxis,
                       realA msdtime, realA beginfit, realA endfit,
                       realA *DD, realA *SigmaD, char *grpname[],
...
static realA calc1_norm(t_corr *curr, int nx, int index[], int nx0, rvec xc[],
                       rvec dcom, gmx_bool bTen, matrix mat)
{
    int  i, ix, m, m2;
    realA g, r, r2;
...
static realA calc_one_mw(t_corr *curr, int ix, int nx0, rvec xc[], realA *tm,
                        rvec dcom, gmx_bool bTen, matrix mat)
{
    realA r2, r, mm;
    rvec rv;
...
static realA calc1_mw(t_corr *curr, int nx, int index[], int nx0, rvec xc[],
                     rvec dcom, gmx_bool bTen, matrix mat)
{
    int  i;
    realA g, tm;
...
static realA calc1_mol(t_corr *curr, int nx, int gmx_unused index[], int nx0, rvec xc[],
                      rvec dcom, gmx_bool bTen, matrix mat)
{
    int  i;
    realA g, tm, gtot, tt;
...
#define NDIST 100
    ...
    gmx_stats_t lsq1;
    int         i, j;
    realA        a, b, D, Dav, D2av, VarD, sqrtD, sqrtD_max, scale;
...
static int corr_loop(t_corr *curr, const char *fn, const t_topology *top, int ePBC,
                     ...
                     realA dt, realA t_pdb, rvec **x_pdb, matrix box_pdb,
                     const gmx_output_env_t *oenv)
{
...
static void do_corr(const char *trx_file, const char *ndx_file, const char *msd_file,
                    const char *mol_file, const char *pdb_file, realA t_pdb,
                    int nrgrp, t_topology *top, int ePBC,
                    gmx_bool bTen, gmx_bool bMW, gmx_bool bRmCOMM,
                    int type, realA dim_factor, int axis,
                    realA dt, realA beginfit, realA endfit, const gmx_output_env_t *oenv)
{
    t_corr        *msd;
...
int gmx_msd(int argc, char *argv[])
{
    ...
    static int         ngroup     = 1;
    static realA        dt         = 10;
    static realA        t_pdb      = 0;
    static realA        beginfit   = -1;
    static realA        endfit     = -1;

                  ...
                  int maxchi, int r0, gmx_residuetype_t *rt)
{
    ...
    while (i < atoms->nr)
    {
        ...
        /* added by grs - special case for aromatics, whose chis above 2 are
           not realA and produce rubbish output - so set back to -1 */
...
static void pr_one_ro(FILE *fp, t_dlist *dl, int nDih, realA gmx_unused dt)
{
    int k;
    for (k = 0; k < NROT; k++)
    {
...
static void pr_ntr_s2(FILE *fp, t_dlist *dl, int nDih, realA dt)
{
              ...
              gmx_bool bPhi, gmx_bool bPsi, gmx_bool bChi, gmx_bool bOmega, int maxchi)
{
    ...
    void  (*pr_props)(FILE *, t_dlist *, int, realA);

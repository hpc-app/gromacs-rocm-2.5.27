...
static int calc_nbegin(int nx, realA x[], realA tbegin)
{
    int  nbegin;
...
static realA numerical_deriv(int nx, realA x[], realA y[], realA fity[], realA combined[], realA dy[],
                            realA tendInt, int nsmooth)
{
    FILE *tmpfp;
    int   i, nbegin, i0, i1;
    realA  fac, fx, fy, integralSmth;
    ...
    else
    {
        ...
        for (i = i0; (i <= i1); i++)
        {
            fx = static_cast<realA>(i1-i)/(i1-i0);
            fy = static_cast<realA>(i-i0)/(i1-i0);
            if (debug)
            {
...
static void do_four(const char *fn, const char *cn, int nx, realA x[], realA dy[],
                    realA eps0, realA epsRF, const gmx_output_env_t *oenv)
{
    FILE      *fp, *cp;
    t_complex *tmp, gw, hw, kw;
    int        i, nnx, nxsav;
    realA       fac, nu, dt, maxeps, numax;
    gmx_fft_t  fft;
    int        fftcode;
...
int gmx_dielectric(int argc, char *argv[])
{
    const char       *desc[] = {
        ...
        "The second file contains the realA and imaginary parts of the",
        "frequency-dependent dielectric constant, the last gives a plot",
        "known as the Cole-Cole plot, in which the imaginary",
        "component is plotted as a function of the realA component.",
...
#define NFILE asize(fnm)
    ...
    realA              dt, integral, fitintegral, fac, rffac;
    double           *fitparms;
    double          **yd;
    realA            **y;
    const char       *legend[] = { "Correlation", "Std. Dev.", "Fit", "Combined", "Derivative" };
    static int        fix      = 0, bX = 1, nsmooth = 3;
    static realA       tendInt  = 5.0, tbegin = 5.0, tend = 500.0;
    static realA       A        = 0.5, tau1 = 10.0, tau2 = 1.0, eps0 = 80, epsRF = 78.5, tail = 500.0;
    realA              lambda;
    t_pargs           pa[] = {
        { "-x1",  FALSE, etBOOL, {&bX},

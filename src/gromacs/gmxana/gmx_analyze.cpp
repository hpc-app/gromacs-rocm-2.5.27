...
static void power_fit(int n, int nset, realA **val, realA *t)
{
    realA *x, *y, quality, a, b, r;
    int   s, i;
    ...
    else
    {
        ...
        for (i = 0; i < n; i++)
        {
            x[i] = std::log1p(static_cast<realA>(i));
        }
    }
...
static realA cosine_content(int nhp, int n, realA *y)
...
static void plot_coscont(const char *ccfile, int n, int nset, realA **val,
...
static void regression_analysis(int n, gmx_bool bXYdy,
                                realA *x, int nset, realA **val)
{
    realA S, chi2, a, b, da, db, r = 0;
    int  ok;
...
static void histogram(const char *distfile, realA binwidth, int n, int nset, realA **val,
                      const gmx_output_env_t *oenv)
{
    FILE          *fp;
    int            i, s;
...
static void average(const char *avfile, int avbar_opt,
                    int n, int nset, realA **val, realA *t)
{
    FILE   *fp;
    int     i, s, edge = 0;
    double  av, var, err;
 ...
 */
static realA optimal_error_estimate(double sigma, double fitparm[], realA tTotal)
{
...
static void estimate_error(const char *eefile, int nb_min, int resol, int n,
                           int nset, double *av, double *sig, realA **val, realA dt,
                           ...
                           const gmx_output_env_t *oenv)
{
    ...
    double   blav, var;
    char   **leg;
    realA    *tbs, *ybs, rtmp, dens, *fitsig, twooe, tau1_est, tau_sig;
    double   fitparm[3];
    realA     ee, a, tau1, tau2;
    ...
    for (s = 0; s < nset; s++)
    {
        ...
        if (bFitAc)
        {
            ...
            for (i = 0; i < n; i++)
            {
                ...
                if (i > 0)
                {
                    fitsig[i] = std::sqrt(static_cast<realA>(i));
                }
                else
                {
                    fitsig[i] = 1;
...
static void luzar_correl(int nn, realA *time, int nset, realA **val, realA temp,
                         gmx_bool bError, realA fit_start)
{
    realA      *kt;
    realA       d2;
    int        j;
...
static void filter(realA flen, int n, int nset, realA **val, realA dt)
{
    int     f, s, i, j;
...
static void do_fit(FILE *out, int n, gmx_bool bYdy,
                   int ny, realA *x0, realA **val,
                   int npargs, t_pargs *ppa, const gmx_output_env_t *oenv,
                   const char *fn_fitted)
{
    realA   *c1 = nullptr, *sig = nullptr;
    double *fitparm;
    realA    tendfit, tbeginfit;
    int     i, efitfn, nparm;
...
int gmx_analyze(int argc, char *argv[])
{
    ...
    static realA        tb         = -1, te = -1, frac = 0.5, filtlen = 0, binwidth = 0.1, aver_start = 0;
    ...
    static realA        temp       = 298.15, fit_start = 1, fit_end = 60;

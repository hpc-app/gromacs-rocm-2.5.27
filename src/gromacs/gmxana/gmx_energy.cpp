...
static void einstein_visco(const char *fn, const char *fni, int nsets,
                           int nint, realA **eneint,
                           realA V, realA T, double dt,
...
static void remove_drift(int nset, int nbmin, int nbmax, realA dt, enerdata_t *edat)
{
                         ...
                         const gmx_output_env_t *oenv)
{
    ...
    realA            integral, intBulk, Temp = 0, Pres = 0;
    realA            pr_aver, pr_stddev, pr_errest;
...
static void print1(FILE *fp, gmx_bool bDp, realA e)
{
    if (bDp)
    {
        fprintf(fp, "  %16.12f", e);
...
static void fec(const char *ene2fn, const char *runavgfn,
                realA reftemp, int nset, int set[], char *leg[],
...
int gmx_energy(int argc, char *argv[])
{
    ...
    static realA        reftemp = 300.0, ezero = 0;
    t_pargs            pa[]    = {

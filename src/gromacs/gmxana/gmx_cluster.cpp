...
/* prepare buffer with one argument, then print to fp1 and fp2 */
static gmx_inline
void ffprintf_g(FILE *fp1, FILE *fp2, char *buf, const char *fmt, realA arg)
{
    sprintf(buf, fmt, arg);
    lo_ffprintf(fp1, fp2, buf);
}
...
/* prepare buffer with two arguments, then print to fp1 and fp2 */
static gmx_inline
void ffprintf_gg(FILE *fp1, FILE *fp2, char *buf, const char *fmt, realA arg1, realA arg2)
{
    sprintf(buf, fmt, arg1, arg2);
    lo_ffprintf(fp1, fp2, buf);
}
...
static void mc_optimize(FILE *log, t_mat *m, realA *time,
                        int maxiter, int nrandom,
                        int seed, realA kT,
                        const char *conv, gmx_output_env_t *oenv)
{
    FILE      *fp = nullptr;
    realA       ecur, enext, emin, prob, enorm;
    ...
    gmx::UniformRealDistribution<realA>   realDistOne;        // [0,1)
...
static void calc_dist(int nind, rvec x[], realA **d)
{
    int      i, j;
    realA    *xi;
    rvec     dx;
...
static realA rms_dist(int isize, realA **d, realA **d_r)
{
    int  i, j;
    realA r, r2;
...
static void gather(t_mat *m, realA cutoff, t_clusters *clust)
{
    t_clustid *c;
    t_dist    *d;
...
static void jarvis_patrick(int n1, realA **mat, int M, int P,
                           realA rmsdcut, t_clusters *clust)
{
    t_dist     *row;
    t_clustid  *c;
    int       **nnb;
...
static void gromos(int n1, realA **mat, realA rmsdcut, t_clusters *clust)
{
    t_dist *row;
    t_nnb  *nnb;
    int     i, j, k, j1, maxval;
...
static rvec **read_whole_trj(const char *fn, int isize, int index[], int skip,
                             int *nframe, realA **time, const gmx_output_env_t *oenv, gmx_bool bPBC, gmx_rmpbc_t gpbc)
{
    rvec       **xx, *x;
    matrix       box;
    realA         t;
...
static int plot_clusters(int nf, realA **mat, t_clusters *clust,
                         int minstruct)
{
    int  i, j, ncluster, ci;
...
static void mark_clusters(int nf, realA **mat, realA val, t_clusters *clust)
{
    int i, j;
...
static char *parse_filename(const char *fn, int maxnr)
{
    ...
    i = static_cast<int>((std::log(static_cast<realA>(maxnr))/std::log(10.0)) + 1);
    /* split fn and ext */
    ext = std::strrchr(fn, '.');
    if (!ext)
    {
                      ...
                      t_rgb rlo, t_rgb rhi, const gmx_output_env_t *oenv)
{
    ...
    for (i = 1; i < nf; i++)
    {
        if (clust->cl[i] != clust->cl[i-1])
        {
            ...
            maxtrans = static_cast<int>(std::max(static_cast<realA>(maxtrans), trans[clust->cl[i]-1][clust->cl[i-1]-1]));
        }
    }
...
static void analyze_clusters(int nf, t_clusters *clust, realA **rmsd,
                             int natom, t_atoms *atoms, rvec *xtps,
                             realA *mass, rvec **xx, realA *time,
                             ...
                             int write_ncl, int write_nst, realA rmsmin,
...
int gmx_cluster(int argc, char *argv[])
{
    ...
    char              *grpname;
    realA               rmsd, **d1, **d2, *time = nullptr, time_invfac, *mass = nullptr;
    ...
    static realA       scalemax = -1.0, rmsdcut = 0.1, rmsmin = 0.0;
...
#define NFILE asize(fnm)
    ...
    switch (method)
    {
        ...
        case m_diagonalize:
            ...
            std::memcpy(eigenvectors, rms->mat[0], nf*nf*sizeof(realA));

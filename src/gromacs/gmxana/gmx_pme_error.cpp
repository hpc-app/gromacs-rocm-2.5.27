...
typedef struct
{
    gmx_int64_t     orig_sim_steps;  /* Number of steps to be done in the realA simulation  */
    int             n_entries;       /* Number of entries in arrays                        */
    realA            volume;          /* The volume of the box                              */
    matrix          recipbox;        /* The reciprocal box                                 */
    int             natoms;          /* The number of atoms in the MD system               */
    realA           *fac;             /* The scaling factor                                 */
    realA           *rcoulomb;        /* The coulomb radii [0...nr_inputfiles]              */
    realA           *rvdw;            /* The vdW radii                                      */
    int            *nkx, *nky, *nkz; /* Number of k vectors in each spatial dimension      */
    realA           *fourier_sp;      /* Fourierspacing                                     */
    realA           *ewald_rtol;      /* Real space tolerance for Ewald, determines         */
                                     /* the realA/reciprocal space relative weight          */
    realA           *ewald_beta;      /* Splitting parameter [1/nm]                         */
    realA            fracself;        /* fraction of particles for SI error                 */
    realA            q2all;           /* sum ( q ^2 )                                       */
    realA            q2allnr;         /* nr of charges                                      */
    int            *pme_order;       /* Interpolation order for PME (bsplines)             */
    char          **fn_out;          /* Name of the output tpr file                        */
    realA           *e_dir;           /* Direct space part of PME error with these settings */
    realA           *e_rec;           /* Reciprocal space part of PME error                 */
    gmx_bool        bTUNE;           /* flag for tuning */
} t_inputinfo;


/* Returns TRUE when atom is charged */
static gmx_bool is_charge(realA charge)
{
...
static void calc_q2all(const gmx_mtop_t *mtop,   /* molecular topology */
                       realA *q2all, realA *q2allnr)
{
    int             imol, iatom; /* indices for loops */
    realA            q2_all = 0;  /* Sum of squared charges */
...
static realA estimate_direct(
        t_inputinfo *info
        )
{
    realA e_dir     = 0; /* Error estimate */
    realA beta      = 0; /* Splitting parameter (1/nm) */
    realA r_coulomb = 0; /* Cut-off in direct space */
...
static inline realA eps_poly1(
        realA m,          /* grid coordinate in certain direction */
        realA K,          /* grid size in corresponding direction */
        realA n)          /* spline interpolation order of the SPME */
{
    int  i;
    realA nom   = 0; /* nominator */
    realA denom = 0; /* denominator */
    realA tmp   = 0;
...
{
    int  i;
    realA tmp    = 0; /* temporary variables for computations */
    realA tmp1   = 0; /* temporary variables for computations */
    realA tmp2   = 0; /* temporary variables for computations */
    realA rcoord = 0; /* coordinate in certain reciprocal space direction */
    realA nom    = 0; /* nominator */
    realA denom  = 0; /* denominator */
...
static void calc_recipbox(matrix box, matrix recipbox)
{
    ...
    realA tmp = 1.0/(box[XX][XX]*box[YY][YY]*box[ZZ][ZZ]);
...
static realA estimate_reciprocal(
        t_inputinfo       *info,
        rvec               x[], /* array of particles */
        realA               q[], /* array of charges */
        ...
        t_commrec         *cr)
{
    realA      e_rec   = 0; /* reciprocal error estimate */
    realA      e_rec1  = 0; /* Error estimate term 1*/
    realA      e_rec2  = 0; /* Error estimate term 2*/
    realA      e_rec3  = 0; /* Error estimate term 3 */
    realA      e_rec3x = 0; /* part of Error estimate term 3 in x */
    realA      e_rec3y = 0; /* part of Error estimate term 3 in y */
    realA      e_rec3z = 0; /* part of Error estimate term 3 in z */
    int       i, ci;
    ...
    realA      coeff  = 0;  /* variable to compute coefficients of the error estimate */
    realA      coeff2 = 0;  /* variable to compute coefficients of the error estimate */
    realA      tmp    = 0;  /* variables to compute different factors from vectors */
    realA      tmp1   = 0;
    realA      tmp2   = 0;
    gmx_bool  bFraction;
    ...
    if (PAR(cr))
    {
        x_per_core = static_cast<int>(std::ceil(static_cast<realA>(xtot) / cr->nnodes));
 ...
 */
static int prepare_x_q(realA *q[], rvec *x[], const gmx_mtop_t *mtop, const rvec x_orig[], t_commrec *cr)
{
    int                     i;
...
static void read_tpr_file(const char *fn_sim_tpr, t_inputinfo *info, t_state *state, gmx_mtop_t *mtop, t_inputrec *ir, realA user_beta, realA fracself)
{
                               ...
                               t_commrec *cr)
{
    ...
    realA *q     = nullptr; /* The charges     */
    realA  edir  = 0.0;     /* realA space error */
    realA  erec  = 0.0;     /* reciprocal space error */
    realA  derr  = 0.0;     /* difference of realA and reciprocal space error */
    realA  derr0 = 0.0;     /* difference of realA and reciprocal space error */
    realA  beta  = 0.0;     /* splitting parameter beta */
    realA  beta0 = 0.0;     /* splitting parameter beta */
    ...
    if (info->bTUNE)
    {
        ...
        while (std::abs(derr/std::min(erec, edir)) > 1e-4)
        {
            ...
            {
                i++;
                fprintf(stderr, "difference between realA and rec. space error (step %d): %g\n", i, std::abs(derr));
...
int gmx_pme_error(int argc, char *argv[])
{
    const char     *desc[] = {
        ...
        "distributed over the realA and reciprocal space part.",
    ...
    realA            fs        = 0.0; /* 0 indicates: not set by the user */
    realA            user_beta = -1.0;
    realA            fracself  = 1.0;
    t_inputinfo     info;
    ...
    t_pargs           pa[] = {
        ...
        { "-tune",     FALSE, etBOOL, {&bTUNE},
          "Tune the splitting parameter such that the error is equally distributed between realA and reciprocal space" },

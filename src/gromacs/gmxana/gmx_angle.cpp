...
static void dump_dih_trr(int nframes, int nangles, realA **dih, const char *fn,
                         realA *time)
{
    int              i, j, k, l, m, na;
    struct t_fileio *fio;
...
int gmx_g_angle(int argc, char *argv[])
{
    ...
    int               *index;
    char              *grpname;
    realA               maxang, S2, norm_fac, maxstat;
    unsigned long      mode;
    ...
    realA              aver, aver2, aversig; /* fraction trans dihedrals */
    double            tfrac = 0;
    char              title[256];
    realA            **dih = nullptr; /* mega array with all dih. angles at all times*/
    realA             *time, *trans_frac, *aver_angle;
    t_filenm          fnm[] = {

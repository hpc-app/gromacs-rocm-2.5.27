...
static void low_print_data(FILE *fp, realA time, rvec x[], int n, int *index,
...
static void average_data(rvec x[], rvec xav[], realA *mass,
...
static void print_data(FILE *fp, realA time, rvec x[], realA *mass, gmx_bool bCom,
...
static void write_trx_x(t_trxstatus *status, const t_trxframe *fr, realA *mass, gmx_bool bCom,
                        int ngrps, int isize[], int **index)
{
    static rvec    *xav   = nullptr;
...
static realA ekrot(rvec x[], rvec v[], realA mass[], int isize, int index[])
{
    realA          TCM[5][5], L[5][5];
...
static realA ektrans(rvec v[], realA mass[], int isize, int index[])
{
    dvec   mvcom;
    double mtot;
    int    i, j, d;
...
static realA temp(rvec v[], realA mass[], int isize, int index[])
{
    double ekin2;
    int    i, j;
...
static void write_pdb_bfac(const char *fname, const char *xname,
                           ...
                           int nfr_v, rvec *sum,
                           gmx_bool bDim[], realA scale_factor,
                           const gmx_output_env_t *oenv)
{
    FILE       *fp;
    realA        max, len2, scale;
    int         maxi;
    int         i, m, onedim;
...
static void update_histo(int gnx, int index[], rvec v[],
                         int *nhisto, int **histo, realA binwidth)
{
    int  i, m, in, nnn;
    realA vn, vnmax;
...
static void print_histo(const char *fn, int nhisto, int histo[], realA binwidth,
                        const gmx_output_env_t *oenv)
{
    FILE *fp;
    int   i;
...
int gmx_traj(int argc, char *argv[])
{
    ...
    static realA       ctime   = -1, scale = 0, binwidth = 1;
    t_pargs           pa[]    = {

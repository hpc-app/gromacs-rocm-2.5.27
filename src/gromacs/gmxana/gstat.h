...
typedef struct {
    ...
    int        ntr[edMax];
    realA       S2[edMax];
    realA       rot_occ[edMax][NROT];

} t_dlist;

typedef struct {
    const char *name;    /* Description of the J coupling constant */
    realA        A, B, C; /* Karplus coefficients */
    realA        offset;  /* Offset for dihedral angle in histogram (e.g. -M_PI/3) */
    realA        Jc;      /* Resulting Jcoupling */
    realA        Jcsig;   /* Standard deviation in Jc */
} t_karplus;

void calc_distribution_props(int nh, int histo[],
                             realA start, int  nkkk, t_karplus kkk[],
                             realA *S2);
...
void low_ana_dih_trans(gmx_bool bTrans, const char *fn_trans,
                       ...
                       realA **dih, int nlist, t_dlist dlist[],
                       int nframes, int nangles, const char *grpname,
                       int multiplicity[], realA *time, gmx_bool bRb,
                       realA core_frac, const gmx_output_env_t *oenv);
...
void do_pp2shifts(FILE *fp, int nframes,
                  int nlist, t_dlist dlist[], realA **dih);
...
void pr_dlist(FILE *fp, int nl, t_dlist dl[], realA dt,  int printtype,
              gmx_bool bPhi, gmx_bool bPsi, gmx_bool bChi, gmx_bool bOmega, int maxchi);

int pr_trans(FILE *fp, int nl, t_dlist dl[], realA dt, int Xi);
...
void get_chi_product_traj (realA **dih, int nframes,
                           int nlist, int maxchi, t_dlist dlist[],
                           realA time[], int **lookup, int *multiplicity,

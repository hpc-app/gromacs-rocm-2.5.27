...
static void leg_discrete(t_psdata ps, realA x0, realA y0, char *label,
                         realA fontsize, char *font, int nmap, t_mapping map[])
{
    int   i;
    realA  yhh;
    realA  boxhh;
...
static void leg_continuous(t_psdata ps, realA x0, realA x, realA y0, char *label,
                           realA fontsize, char *font,
                           int nmap, t_mapping map[],
                           int mapoffset)
{
    int   i;
    realA  xx0;
    realA  yhh, boxxh, boxyh;
...
static void leg_bicontinuous(t_psdata ps, realA x0, realA x, realA y0, char *label1,
                             char *label2, realA fontsize, char *font,
...
static realA box_height(t_matrix *mat, t_psrec *psr)
{
    return mat->ny*psr->yboxsize;
}

static realA box_dh(t_psrec *psr)
{
    return psr->boxspacing;
}

#define IS_ONCE (i == nmat-1)
static realA box_dh_top(gmx_bool bOnce, t_psrec *psr)
{
    realA dh;
...
static void draw_boxes(t_psdata ps, realA x0, realA y0, realA w,
                       int nmat, t_matrix mat[], t_psrec *psr)
{
    ...
    realA     xxx;
    char   **xtick, **ytick;
    realA     xx, yy, dy, xx00, yy00, offset_x, offset_y;
    int      i, j, x, y, ntx, nty;
    size_t   strlength;
...
static void draw_zerolines(t_psdata out, realA x0, realA y0, realA w,
                           int nmat, t_matrix mat[], t_psrec *psr)
{
    realA   xx, yy, dy, xx00, yy00;
    int    i, x, y;
...
static void box_dim(int nmat, t_matrix mat[], t_matrix *mat2, t_psrec *psr,
                    ...
                    realA *w, realA *h, realA *dw, realA *dh)
{
    int  i, maxytick;
    realA ww, hh, dww, dhh;
    ...
    if (bFrame)
    {
        ...
        if (psr->Y.major > 0)
        {
            dww += psr->Y.majorticklen + DDD +
                psr->Y.tickfontsize*(std::log(static_cast<realA>(maxytick))/std::log(10.0));
        }
...
static void tick_spacing(int n, realA axis[], realA offset, char axisnm,
                         realA *major, realA *minor)
{
    realA     space;
    gmx_bool bTryAgain;
    int      i, j, t, f = 0, ten;
#define NFACT 4
    realA     major_fact[NFACT] = {5, 4, 2, 1};
    realA     minor_fact[NFACT] = {5, 4, 4, 5};
    ...
    for (t = ten+2; t > ten-3 && bTryAgain; t--)
    {
        ...
        {
            space = std::pow(static_cast<realA>(10.0), static_cast<realA>(t)) * major_fact[f];
...
static void ps_mat(const char *outf, int nmat, t_matrix mat[], t_matrix mat2[],
                   ...
                   realA size, realA boxx, realA boxy, const char *m2p, const char *m2pout,
                   int mapoffset)
{
    const char   *libm2p;
    char         *legend;
                                  ...
                                  realA *cmin, realA *cmax)
{
    ...
    for (k = 0; k < nmat; k++)
    {
        ...
        {
            gmx_fatal(FARGS, "Could not extract realA data from %s xpm matrices. Note that, e.g.,\n"
...
static void do_mat(int nmat, t_matrix *mat, t_matrix *mat2,
                   ...
                   realA size, realA boxx, realA boxy,
...
int gmx_xpm2ps(int argc, char *argv[])
{
    ...
    static realA       size   = 400, boxx = 0, boxy = 0, cmin = 0, cmax = 0;

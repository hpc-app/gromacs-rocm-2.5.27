...
static void find_nearest_neighbours(int ePBC,
                                    ...
                                    realA *sgmean, realA *skmean,
                                    int nslice, int slice_dim,
                                    realA sgslice[], realA skslice[],
                                    gmx_rmpbc_t gpbc)
{
    int      ix, jx, nsgbin, *sgbin;
    int      i, ibin, j, k, l, n, *nn[4];
    rvec     dx, rj, rk, urk, urj;
    realA     cost, cost2, *sgmol, *skmol, rmean, rmean2, r2, box2, *r_nn[4];
    t_pbc    pbc;
    int      sl_index;
    int     *sl_count;
    realA     onethird = 1.0/3.0;
                                  ...
                                  const gmx_output_env_t *oenv)
{
    ...
    realA        *sg_slice, *sg_slice_tot, *sk_slice, *sk_slice_tot;
...
static void check_length(realA length, int a, int b)
{
    if (length > 0.3)
    {
...
static void calc_order(const char *fn, int *index, int *a, rvec **order,
                       realA ***slOrder, realA *slWidth, int nslices, gmx_bool bSliced,
                       ...
                       const gmx_output_env_t *oenv)
{
    ...
    *x1,                                         /* coordinates without pbc                        */
    ...
    realA *slFrameorder;                          /* order parameter for one frame, per slice      */
    realA  length,                                /* total distance between two atoms               */
    ...
    realA         sdbangle               = 0;     /* sum of these angles                            */
...
static void order_plot(rvec order[], realA *slOrder[], const char *afile, const char *bfile,
                       const char *cfile, int ngrps, int nslices, realA slWidth, gmx_bool bSzonly,
                       gmx_bool permolecule, realA **distvals, const gmx_output_env_t *oenv)
{
    FILE       *ord, *slOrd;      /* xvgr files with order parameters  */
    int         atom, slice;      /* atom corresponding to order para.*/
    char        buf[256];         /* for xvgr title */
    realA        S;                /* order parameter averaged over all atoms */
...
static void write_bfactors(t_filenm  *fnm, int nfile, int *index, int *a, int nslices, int ngrps, realA **order, const t_topology *top, realA **distvals, gmx_output_env_t *oenv)
{
...
int gmx_order(int argc, char *argv[])
{
    ...
    realA             **slOrder;                       /* same, per slice            */
    realA               slWidth = 0.0;                 /* width of a slice           */

...
typedef struct
{
    ...
    realA     k;               //!< force constants in tpr file
    realA     init_dist;       //!< reference displacement
...
typedef struct
{
    ...
    gmx_bool **bContrib;
    realA     **ztime;     //!< input data z(t) as a function of time. Required to compute ACTs
     ...
     */
    realA   *forceAv;
    realA   *aver;         //!< average of histograms
    realA   *sigma;        //!< stddev of histograms
...
typedef struct
{
    ...
    realA              tmin, tmax, dt;             //!< only read input within tmin and tmax with dt
    ...
    realA     min, max, dz;
    realA     Temperature, Tolerance; //!< temperature, converged when probability changes less than Tolerance
    ...
    realA     sigSmoothIact;             //!< sigma of Gaussian to smooth ACTs
    gmx_bool bAllowReduceIact;          //!< Allow to reduce ACTs during smoothing. Otherwise ACT are only increased during smoothing
    realA     acTrestart;                //!< when computing ACT, time between restarting points
    ...
    gmx::TabulatedNormalDistribution<> normalDistribution;  //!< Uses default: realA output, 14-bit table
} t_UmbrellaOptions;
...
static void read_pdo_data(FILE * file, t_UmbrellaHeader * header,
                          ...
                          gmx_bool bGetMinMax, realA *mintmp, realA *maxtmp)
{
    int                i, inttemp, bins, count, ntot;
    realA               minval, maxval, minfound = 1e20, maxfound = -1e20;
...
static void setRandomBsWeights(t_UmbrellaWindow *synthwin, int nAllPull, t_UmbrellaOptions *opt)
{
    int     i;
    double *r;
    gmx::UniformRealDistribution<realA> dist(0, nAllPull);
                         ...
                         t_coordselection *coordsel)
{
    ...
    realA            min, max, minfound = 1e20, maxfound = -1e20;
                                               ...
                                               t_UmbrellaOptions *opt, const char *fn, const char *xlabel)
{
    ...
    realA *corr, c0, dt, tmp;
    realA *ztime, av, tausteps;
...
{
    int  i, ig, ntot, k;
    realA av, sum2, sig, diff, *ztime, nSamplesIndep;
                                            ...
                                            t_UmbrellaOptions *opt)
{
    ...
    realA avcount = 0, z, relcount, *count;
    snew(count, opt->bins);

...
static realA wCsolid(realA nu, realA beta)
{
    realA bhn = beta*PLANCK*nu;
    realA ebn, koko;
...
static realA wSsolid(realA nu, realA beta)
{
    realA bhn = beta*PLANCK*nu;
...
static realA wAsolid(realA nu, realA beta)
{
    realA bhn = beta*PLANCK*nu;
...
static realA wEsolid(realA nu, realA beta)
{
    realA bhn = beta*PLANCK*nu;
...
int gmx_dos(int argc, char *argv[])
{
    const char         *desc[] = {
        ...
        "autocorrelation, and by default only from the square of the realA",
    ...
    realA              **c1, **dos, mi, beta, bfac, *nu, *tt, stddev, c1j;
    ...
    static     realA     Temp       = 298.15, toler = 1e-6;
    int                 min_frames = 100;
    ...
    t_pargs             pa[]     = {
        ...
        { "-abs", FALSE, etBOOL, {&bAbsolute},
          "Use the absolute value of the Fourier transform of the VACF as the Density of States. Default is to use the realA component only" },

...
static void addtoavgenergy(t_complex *list, realA *result, int size, int tsteps)
{
    int i;
    for (i = 0; i < size; i++)
    {
...
void powerspectavg(realA ***intftab, int tsteps, int xbins, int ybins, char **outfiles)
{
    ...
    t_complex *ftspect2;
    realA      *pspectavg1;          /*power -spectrum 1st interface*/
    realA      *pspectavg2;          /* -------------- 2nd interface*/
    realA      *temp;
    FILE      *datfile1, *datfile2; /*data-files with interface data*/
    int        n;                   /*time index*/
    int        fy  = ybins/2+1;     /* number of (symmetric) fourier y elements; */
    int        rfl = xbins*fy;      /*length of realA - DFT == Symmetric 2D matrix*/

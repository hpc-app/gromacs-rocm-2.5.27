...
static void ptrans(char *s, realA **inten, realA d[], realA e[])
{
    int  m;
    realA n, x, y, z;
    for (m = 1; (m < NDIM); m++)
...
void t_trans(matrix trans, realA d[], realA **ev)
{
    rvec x;
    int  j;
    for (j = 0; (j < DIM); j++)
...
realA calc_xcm(const rvec x[], int gnx, const int *index, const t_atom *atom, rvec xcm,
              gmx_bool bQ)
{
    int  i, ii, m;
    realA m0, tm;
...
realA sub_xcm(rvec x[], int gnx, const int *index, const t_atom atom[], rvec xcm,
             gmx_bool bQ)
{
    int  i, ii;
    realA tm;

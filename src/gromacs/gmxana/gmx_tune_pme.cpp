...
typedef struct
{
    ...
    gmx_int64_t     orig_init_step;  /* Init step for the realA simulation */
    realA           *rcoulomb;        /* The coulomb radii [0...nr_inputfiles] */
    realA           *rvdw;            /* The vdW radii */
    realA           *rlist;           /* Neighbourlist cutoff radius */
    int            *nkx, *nky, *nkz;
    realA           *fsx, *fsy, *fsz; /* Fourierspacing in x,y,z dimension */
} t_inputinfo;
...
static void make_benchmark_tprs(
        ...
        realA            rmin,            /* Minimal Coulomb radius                        */
        realA            rmax,            /* Maximal Coulomb radius                        */
        realA            bScaleRvdw,      /* Scale rvdw along with rcoulomb                */
        ...
        FILE           *fp)              /* Write the output here                         */
{
    ...
    t_state       state;
    gmx_mtop_t    mtop;
    realA          nlist_buffer;     /* Thickness of the buffer regions for PME-switch potentials */
    char          buf[200];
    rvec          box_size;
    gmx_bool      bNote = FALSE;
    realA          add;              /* Add this to rcoul for the next test    */
    realA          fac = 1.0;        /* Scaling factor for Coulomb radius      */
    realA          fourierspacing;   /* Basic fourierspacing from tpr          */
    ...
    fprintf(fp, "\nWill try these realA/reciprocal workload settings:\n");
        ...
        int         maxPMEnodes)    /* Maximum number of PME nodes                      */
{
    ...
    nlist = 0; /* start counting again, now the realA entries in the list */
...
static void check_input(
        ...
        realA            rcoulomb,
        realA           *rmax,
        realA            maxPMEfraction,
        realA            minPMEfraction,
        int             npme_fixed,
        ...
        t_pargs        *pa)
{
    ...
    else
    {
        if (1 == *ntprs)
        {
            fprintf(stderr, "Note: Choose ntpr>1 to shift PME load between realA and reciprocal space.\n");
        }
    }
...
/* Create the command line for the benchmark as well as for the realA run */
...
static float inspect_tpr(int nfile, t_filenm fnm[], realA *rcoulomb)
{
...
int gmx_tune_pme(int argc, char *argv[])
{
    const char     *desc[] = {
        ...
        "be enhanced by shifting load from the reciprocal to the realA space",
        ...
        "[THISMODULE] can test various realA space / reciprocal space workloads",
    ...
    realA            maxPMEfraction = 0.50;
    realA            minPMEfraction = 0.25;
    ...
    int             npme_fixed     = -2;              /* If >= -1, use only this number
                                                       * of PME-only nodes                */
    int             ntprs          = 0;
    realA            rmin           = 0.0, rmax = 0.0; /* min and max value for rcoulomb if scaling is requested */
    realA            rcoulomb       = -1.0;            /* Coulomb radius as set in .tpr file */
    ...
    t_pargs           pa[] = {
        ...
        { "-nsteps",   FALSE, etINT64, {&new_sim_nsteps},
          "If non-negative, perform this many steps in the realA run (overwrites nsteps from [REF].tpr[ref], add [REF].cpt[ref] steps)" },
        { "-launch",   FALSE, etBOOL, {&bLaunch},
          "Launch the realA simulation after optimization" },
...
#define NFILE asize(fnm)
    ...
    if (bBenchmark)
    {
        ...
        /* Now start the realA simulation if the user requested it ... */

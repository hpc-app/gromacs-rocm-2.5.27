...
typedef struct {
    int   nv, nframes;
    realA  sumv, averv, maxv;
    realA *aver1, *aver2, *aver_3, *aver_6;
} t_dr_result;
...
static void add5(int ndr, realA viol)
{
    int i, mini;
...
static void check_viol(FILE *log,
                       ...
                       int clust_id, int isize, int index[], realA vvindex[],
                       t_fcdata *fcd)
{
    t_iatom         *forceatoms;
    int              i, j, nat, n, type, nviol, ndr, label;
    realA             rt, mviol, tviol, viol, lam, dvdl, drt;
    rvec            *fshift;
...
typedef struct {
    int      index;
    gmx_bool bCore;
    realA     up1, r, rT3, rT6, viol, violT3, violT6;
} t_dr_stats;
...
static void dump_dump(FILE *log, int ndr, t_dr_stats drs[])
{
    ...
    realA               viol_tot, viol_max, viol = 0;
    gmx_bool           bCore;
    int                nviol, nrestr;
    int                i, kkk;
...
static void dump_disre_matrix(const char *fn, t_dr_result *dr, int ndr,
                              ...
                              realA max_dr, int nlevels, gmx_bool bThird)
{
    FILE      *fp;
    int       *resnr;
    ...
    int        ai, aj, *ptr;
    realA     **matrix, *t_res, hi, *w_dr, rav, rviol;
    t_rgb      rlo = { 1, 1, 1 };
    t_rgb      rhi = { 0, 0, 0 };
    if (fn == nullptr)
    {
    ...
    for (i = 0; (i < ndr); i++)
    {
        for (j = ptr[i]; (j < ptr[i+1]); j += nratoms+1)
        {
            ...
            rviol           = std::max(static_cast<realA>(0.0), rav-idef->iparams[tp].disres.up1);
...
int gmx_disre(int argc, char *argv[])
{
    ...
    char            **leg;
    realA             *vvindex = nullptr, *w_rls = nullptr;
    t_pbc             pbc, *pbc_null;
    int               my_clust;
    FILE             *fplog;

...
static t_gridcell ***init_grid(gmx_bool bBox, rvec box[], realA rcut, ivec ngrid)
{
    t_gridcell ***grid;
    int           i, y, z;
...
static int is_hbond(t_hbdata *hb, int grpd, int grpa, int d, int a,
                    realA rcut, realA r2cut, realA ccut,
                    rvec x[], gmx_bool bBox, matrix box, rvec hbox,
                    realA *d_ha, realA *ang, gmx_bool bDA, int *hhh,
                    gmx_bool bContact, gmx_bool bMerge)
{
    int      h, hh, id;
    rvec     r_da, r_ha, r_dh;
    realA     rc2, r2c2, rda2, rha2, ca;
...
static void do_nhb_dist(FILE *fp, t_hbdata *hb, realA t)
{
...
static realA calc_dg(realA tau, realA temp)
{
    realA kbt;
...
typedef struct {
    ...
    realA  kkk[2];
    realA *t, *ct, *nt, *kt, *sigma_ct, *sigma_nt, *sigma_kt;
} t_luzar;

static realA compute_weighted_rates(int n, realA t[], realA ct[], realA nt[],
                                   realA kt[], realA sigma_ct[], realA sigma_nt[],
                                   realA sigma_kt[], realA *k, realA *kp,
                                   realA *sigma_k, realA *sigma_kp,
                                   realA fit_start)
{
#define NK 10
    int      i, j;
    t_luzar  tl;
    realA     kkk = 0, kkp = 0, kk2 = 0, kp2 = 0, chi2;
...
void analyse_corr(int n, realA t[], realA ct[], realA nt[], realA kt[],
                  realA sigma_ct[], realA sigma_nt[], realA sigma_kt[],
                  realA fit_start, realA temp)
{
    int        i0, i;
    realA       k = 1, kp = 1, kow = 1;
    realA       Q = 0, chi2, tau_hb, dtau, tau_rlx, e_1, sigma_k, sigma_kp, ddg;
...
void compute_derivative(int nn, realA x[], realA y[], realA dydx[])
{
    int j;
...
static void normalizeACF(realA *ct, realA *gt, int nhb, int len)
{
    realA ct_fac, gt_fac = 0;
    int  i;
...
static void do_hbac(const char *fn, t_hbdata *hb,
                    int nDump, gmx_bool bMerge, gmx_bool bContact, realA fit_start,
                    realA temp, gmx_bool R2, const gmx_output_env_t *oenv,
                    int nThreads)
{
    FILE          *fp;
    ...
    double         nhb   = 0;
    realA          *rhbex = nullptr, *ht, *gt, *ght, *dght, *kt;
    realA          *ct, tail, tail2, dtail, *cct;
    const realA     tol     = 1e-3;
    int            nframes = hb->nframes;
...
static void init_hbframe(t_hbdata *hb, int nframes, realA t)
{
    int i;
...
static void analyse_donor_properties(FILE *fp, t_hbdata *hb, int nframes, realA t)
{
...
int gmx_hbond(int argc, char *argv[])
{
    ...
    static realA        acut     = 30, abin = 1, rcut = 0.35, r2cut = 0, rbin = 0.005, rshell = -1;
    static realA        maxnhb   = 0, fit_start = 1, fit_end = 60, temp = 298.15;

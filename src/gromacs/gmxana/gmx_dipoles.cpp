...
static t_gkrbin *mk_gkrbin(realA radius, realA rcmax, gmx_bool bPhi, int ndegrees)
{
    t_gkrbin *gb;
    char     *ptr;
    int       i;
...
static void add2gkr(t_gkrbin *gb, realA r, realA cosa, realA phi)
{
...
static realA normalize_cmap(t_gkrbin *gb)
{
    int    i, j;
    realA   hi;
    double vol;
...
static void print_gkrbin(const char *fn, t_gkrbin *gb,
                         int ngrp, int nframes, realA volume,
                         const gmx_output_env_t *oenv)
{
    ...
    int         i, last;
    realA        x0, x1, ggg, Gkr, vol_s, rho, gOO, hOO, cosav, ener;
    double      fac;
...
static gmx_bool read_mu_from_enx(ener_file_t fmu, int Vol, ivec iMu, rvec mu, realA *vol,
                                 realA *t, int nre, t_enxframe *fr)
{
    int          i;
    gmx_bool     bCont;
    char         buf[22];
 ...
 */
static realA calc_eps(double M_diff, double volume, double epsRF, double temp)
{
...
static void compute_avercos(int n, rvec dip[], realA *dd, rvec axis, gmx_bool bPairs)
{
    int    i, j, k;
...
static void do_dip(const t_topology *top, int ePBC, realA volume,
                   const char *fn,
                   ...
                   int  ncos,
                   const char *cmap,    realA rcmax,
                   gmx_bool bQuad,
                   gmx_bool bMU,      const char *mufn,
                   int  *gnx,     int  *molindex[],
                   realA mu_max,   realA mu_aver,
                   realA epsilonRF, realA temp,
                   int  *gkatom,  int skip,
...
#define NLEGADIP asize(leg_adip)
    ...
    unsigned long  mode;
    realA           rcut = 0, t, t0, t1, dt, dd, rms_cos;
    rvec           dipaxis;
    matrix         box;
    ...
    if (!bMU)
    {
        ...
        for (i = 0; (i < ndipbin); i++)
        {
            fprintf(outdd, "%10g  %10f\n",
                    (i*mu_max)/ndipbin, static_cast<realA>(dipole_bin[i])/teller);
        }
        xvgrclose(outdd);
        sfree(dipole_bin);
    }
...
int gmx_dipoles(int argc, char *argv[])
{
    ...
    realA              mu_max     = 5, mu_aver = -1, rcmax = 0;
    realA              epsilonRF  = 0.0, temp = 300;

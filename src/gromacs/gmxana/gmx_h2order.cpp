...
static void calc_h2order(const char *fn, int index[], int ngx, rvec **slDipole,
                         realA **slOrder, realA *slWidth, int *nslices,
...
static void h2order_plot(rvec dipole[], realA order[], const char *afile,
                         int nslices, realA slWidth, const gmx_output_env_t *oenv)
{
    FILE       *ord;              /* xvgr files with order parameters  */
    int         slice;            /* loop index     */
    char        buf[256];         /* for xvgr title */
    realA        factor;           /* conversion to Debye from electron*nm */
...
int gmx_h2order(int argc, char *argv[])
{
    ...
    realA              *slOrder,               /* av. cosine, per slice      */

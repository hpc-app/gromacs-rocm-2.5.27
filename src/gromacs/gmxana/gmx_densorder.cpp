...
static void density_in_time (const char *fn, int **index, int gnx[], realA bw, realA bwz, int nsttblock, realA *****Densdevel, int *xslices, int *yslices, int *zslices, int *tblock, const t_topology *top, int ePBC, int axis, gmx_bool bCenter, gmx_bool bps1d, const gmx_output_env_t *oenv)
 ...
 */
    ...
    realA ***Densslice = nullptr;         /* Density-slice in one frame*/
    realA    dscale;                      /*physical scaling factor*/
    realA    t, x, y, z;                  /* time and coordinates*/
    rvec    bbww;
...
static void outputfield(const char *fldfn, realA ****Densmap,
    ...
    for (n = 0; n < tdim; n++)
    {
        for (i = 0; i < xslices; i++)
        {
            for (j = 0; j < yslices; j++)
            {
                for (k = 0; k < zslices; k++)
                {
                    fwrite(&(Densmap[n][i][j][k]), sizeof(realA), 1, fldH);
...
static void filterdensmap(realA ****Densmap, int xslices, int yslices, int zslices, int tblocks, int ftsize)
{
    realA *kernel;
    realA  std, var;
    int   i, j, n, order;
...
static void interfaces_txy (realA ****Densmap, int xslices, int yslices, int zslices,
                            int tblocks, realA binwidth, int method,
                            realA dens1, realA dens2, t_interf ****intf1,
                            t_interf ****intf2, const gmx_output_env_t *oenv)
{
    ...
    FILE         *xvg;
    realA         *zDensavg; /* zDensavg[z]*/
    int           i, j, k, n;
    int           xysize;
    int           ndx1, ndx2, *zperm;
    realA          densmid;
    realA          splitpoint, startpoint, endpoint;
    realA         *sigma1, *sigma2;
    double        beginfit1[4];
    double        beginfit2[4];
...
static void writesurftoxpms(t_interf ***surf1, t_interf ***surf2, int tblocks, int xbins, int ybins, int zbins, realA bw, realA bwz, char **outfiles, int maplevels )
{
    char   numbuf[STRLEN];
    int    n, i, j;
    realA **profile1, **profile2;
    realA   max1, max2, min1, min2, *xticks, *yticks;
    t_rgb  lo = {0, 0, 0};
    t_rgb  hi = {1, 1, 1};
...
int gmx_densorder(int argc, char *argv[])
{
    ...
    char             **grpname;
    int                ePBC, *ngx;
    static realA        binw      = 0.2;
    static realA        binwz     = 0.05;
    static realA        dens1     = 0.00;
    static realA        dens2     = 1000.00;

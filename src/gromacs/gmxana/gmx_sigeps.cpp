...
static realA pot(realA x, realA qq, realA c6, realA cn, int npow)
{
    return cn*pow(x, -npow)-c6/gmx::power6(x)+qq*ONE_4PI_EPS0/x;
}

static realA bhpot(realA x, realA A, realA B, realA C)
{
    return A*std::exp(-B*x) - C/gmx::power6(x);
}

static realA dpot(realA x, realA qq, realA c6, realA cn, int npow)
{
...
int gmx_sigeps(int argc, char *argv[])
{
    ...
    static realA       c6   = 1.0e-3, cn = 1.0e-6, qi = 0, qj = 0, sig = 0.3, eps = 1, sigfac = 0.7;
    static realA       Abh  = 1e5, Bbh = 32, Cbh = 1e-3;
    static int        npow = 12;
    t_pargs           pa[] = {
...
#define NFILE asize(fnm)
    ...
    int               i;
    gmx_bool          bBham;
    realA              qq, x, oldx, minimum, mval, dp[2];
    int               cur = 0;
#define next (1-cur)
    ...
    else
    {
        ...
        {
            sig = std::pow(cn/c6, static_cast<realA>(1.0/(npow-6)));
        ...
        Bbh = npow/minimum;
        Cbh = c6;
        Abh = 4*eps*std::pow(sig/minimum, static_cast<realA>(npow))*std::exp(static_cast<realA>(npow));
        printf("A = %g, B = %g, C = %g\n", Abh, Bbh, Cbh);
    }
    qq = qi*qj;

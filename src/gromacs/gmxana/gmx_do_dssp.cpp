...
static int strip_dssp(char *dsspfile, int nres,
                      gmx_bool bPhobres[], realA t,
                      realA *acc, FILE *fTArea,
...
static void norm_acc(t_atoms *atoms, int nres,
                     realA av_area[], realA norm_av_area[])
{
    int     i, n, n_surf;
...
static void write_sas_mat(const char *fn, realA **accr, int nframe, int nres, t_matrix *mat)
{
    realA  lo, hi;
    int   i, j, nlev;
                       ...
                       const gmx_output_env_t *oenv)
{
    ...
    fprintf(fp, "%-8s %5.2f", "# SS pr.", total_count / static_cast<realA>(mat->nx * mat->ny));
    for (s = 0; s < static_cast<size_t>(mat->nmap); s++)
    {
        fprintf(fp, " %5.2f", total[s] / static_cast<realA>(mat->nx * mat->ny));
    }
    fprintf(fp, "\n");
...
int gmx_do_dssp(int argc, char *argv[])
{
    ...
    rvec              *xp, *x;
    int               *average_area;
    realA             **accr, *accr_ptr = nullptr, *av_area, *norm_av_area;
...
#define NFILE asize(fnm)
    ...
    if (bDoAccSurf)
    {
        ...
        for (i = 0; i < atoms->nres; i++)
        {
            av_area[i] = (average_area[i] / static_cast<realA>(nframe));
        }

...
extern realA ahx_len(int gnx, int index[], rvec x[]);
/* Assume we have a list of Calpha atoms only! */

extern realA ellipticity(int nres, t_bb bb[]);

extern realA radius(FILE *fp, int nca, int ca_index[], rvec x[]);
/* Assume we have calphas */

extern realA twist(int nca, int caindex[], rvec x[]);
/* Calculate the twist of the helix */

extern realA pprms(FILE *fp, int nbb, t_bb bb[]);
/* Calculate the average RMS from canonical phi/psi values
 * and the distance per residue
 */

extern realA ca_phi(int gnx, int index[], rvec x[]);
/* Assume we have a list of Calpha atoms only! */

extern realA dip(int nbb, const int bbind[], const rvec x[], const t_atom atom[]);

extern realA rise(int gnx, int index[], rvec x[]);
/* Assume we have a list of Calpha atoms only! */

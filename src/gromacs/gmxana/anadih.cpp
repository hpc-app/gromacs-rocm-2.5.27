...
void print_one(const gmx_output_env_t *oenv, const char *base, const char *name,
               const char *title, const char *ylabel, int nf, realA time[],
               realA data[])
{
    FILE *fp;
    char  buf[256], t2[256];
...
static int calc_RBbin(realA phi, int gmx_unused multiplicity, realA gmx_unused core_frac)
{
    /* multiplicity and core_frac NOT used,
     * just given to enable use of pt-to-fn in caller low_ana_dih_trans*/
    static const realA r30  = M_PI/6.0;
    static const realA r90  = M_PI/2.0;
    static const realA r150 = M_PI*5.0/6.0;
...
static int calc_Nbin(realA phi, int multiplicity, realA core_frac)
{
    static const realA r360 = 360*DEG2RAD;
    realA              rot_width, core_width, core_offset, low, hi;
    int               bin;
...
void ana_dih_trans(const char *fn_trans, const char *fn_histo,
                   realA **dih, int nframes, int nangles,
                   const char *grpname, realA *time, gmx_bool bRb,
...
void low_ana_dih_trans(gmx_bool bTrans, const char *fn_trans,
                       ...
                       realA **dih, int nlist, t_dlist dlist[], int nframes,
                       int nangles, const char *grpname, int multiplicity[],
                       realA *time, gmx_bool bRb, realA core_frac,
                       const gmx_output_env_t *oenv)
{
    FILE *fp;
    int  *tr_f, *tr_h;
...
void get_chi_product_traj (realA **dih, int nframes, int nlist,
                           int maxchi, t_dlist dlist[], realA time[],
                           int **lookup, int *multiplicity, gmx_bool bRb, gmx_bool bNormalize,
                           realA core_frac, gmx_bool bAll, const char *fnall,
                           const gmx_output_env_t *oenv)
{
...
void calc_distribution_props(int nh, int histo[], realA start,
                             int nkkk, t_karplus kkk[],
                             realA *S2)
{
    realA d, dc, ds, c1, c2, tdc, tds;
    realA fac, ang, invth, Jc;
    int  i, j, th;
...
static void calc_angles(struct t_pbc *pbc,
                        int n3, int index[], realA ang[], rvec x_s[])
{
    int  i, ix, t1, t2;
    rvec r_ij, r_kj;
    realA costh = 0.0;
...
static realA calc_fraction(realA angles[], int nangles)
{
    int  i;
    realA trans = 0, gauche = 0;
    realA angle;
...
static void calc_dihs(struct t_pbc *pbc,
                      int n4, int index[], realA ang[], rvec x_s[])
{
    int  i, ix, t1, t2, t3;
...
void make_histo(FILE *log,
                int ndata, realA data[], int npoints, int histo[],
                realA minx, realA maxx)
{
    double dx;
    int    i, ind;
...
void normalize_histo(int npoints, int histo[], realA dx, realA normhisto[])
{
    int    i;
    double d, fac;
                  ...
                  const gmx_output_env_t *oenv)
{
    ...
    int           nangles, n_alloc;
    realA          t, fraction, pifac, aa, angle;
    realA         *angles[2];
    matrix        box;
    rvec         *x;
    int           cur = 0;

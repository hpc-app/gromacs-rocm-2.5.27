...
realA fit_ahx(int nres, t_bb bb[], int natoms, int nall, int allindex[],
             rvec x[], int nca,
             int caindex[], gmx_bool bFit)
{
    static rvec *xref = nullptr;
    static realA *mass = nullptr;
    const  realA  d    = 0.15;  /* Rise per residue (nm)    */
    const  realA  tw   = 1.745; /* Twist per residue (rad)  */
    const  realA  rad  = 0.23;  /* Radius of the helix (nm) */
    realA         phi0, trms, rms;
    rvec         dx, xcm;
    int          ai, i, nmass;

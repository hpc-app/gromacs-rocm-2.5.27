...
static gmx_bool precalc(t_topology top, realA mass2[], realA qmol[])
{
...
static void calc_mj(t_topology top, int ePBC, matrix box, gmx_bool bNoJump, int isize, int index0[], \
                    rvec fr[], rvec mj, realA mass2[], realA qmol[])
{
...
static realA calceps(realA prefactor, realA md2, realA mj2, realA cor, realA eps_rf, gmx_bool bCOR)
{
...
static realA calc_cacf(FILE *fcacf, realA prefactor, realA cacf[], realA time[], int nfr, int vfr[], int ei, int nshift)
{
    ...
    if (nfr > 1)
    {
        ...
        while (i < nfr)
        {
            ...
            rfr      = static_cast<realA>(nfr+i)/nshift;
            cacf[i] /= rfr;
...
static void calc_mjdsp(FILE *fmjdsp, realA prefactor, realA dsp2[], realA time[], int nfr, realA refr[])
{
...
static void dielectric(FILE *fmj, FILE *fmd, FILE *outf, FILE *fcur, FILE *mcor,
                       ...
                       int ePBC, t_topology top, t_trxframe fr, realA temp,
                       realA bfit, realA efit, realA bvit, realA evit,
                       t_trxstatus *status, int isize, int nmols, int nshift,
                       int *index0, int indexm[], realA mass2[],
                       realA qmol[], realA eps_rf, const gmx_output_env_t *oenv)
{
    int       i, j;
    ...
    realA     *djc         = nullptr;
    realA      corint      = 0.0;
    realA      prefactorav = 0.0;
    realA      prefactor   = 0.0;
    realA      volume;
    realA      volume_av = 0.0;
    realA      dk_s, dk_d, dk_f;
    realA      mj    = 0.0;
    realA      mj2   = 0.0;
    realA      mjd   = 0.0;
    realA      mjdav = 0.0;
...
int gmx_current(int argc, char *argv[])
{
    ...
    static realA            temp    = 300.0;
    static realA            eps_rf  = 0.0;
    static gmx_bool        bNoJump = TRUE;
    static realA            bfit    = 100.0;
    static realA            bvit    = 0.5;
    static realA            efit    = 400.0;
    static realA            evit    = 5.0;
    t_pargs                pa[]    = {

...
realA ellipticity(int nres, t_bb bb[])
{
    typedef struct {
        realA phi, psi, w;
    } t_ppwstr;
...
realA ahx_len(int gnx, int index[], rvec x[])
...
realA radius(FILE *fp, int nca, int ca_index[], rvec x[])
...
static realA rot(rvec x1, rvec x2)
{
    realA phi1, dphi, cp, sp;
    realA xx, yy;
...
realA twist(int nca, int caindex[], rvec x[])
{
    realA pt, dphi;
    int  i, a0, a1;
...
realA ca_phi(int gnx, int index[], rvec x[])
/* Assume we have a list of Calpha atoms only! */
{
    realA phi, phitot;
...
realA dip(int nbb, int const bbind[], const rvec x[], const t_atom atom[])
{
    int  i, m, ai;
    rvec dipje;
    realA q;
...
realA rise(int gnx, int index[], rvec x[])
/* Assume we have a list of Calpha atoms only! */
{
    realA z, z0, ztot;
    int  i, ai;
...
void av_hblen(FILE *fp3, FILE *fp3a,
              FILE *fp4, FILE *fp4a,
              FILE *fp5, FILE *fp5a,
              realA t, int nres, t_bb bb[])
{
    int  i, n3 = 0, n4 = 0, n5 = 0;
    realA d3 = 0, d4 = 0, d5 = 0;
...
realA pprms(FILE *fp, int nbb, t_bb bb[])
{
    int  i, n;
    realA rms, rmst, rms2;

...
int gmx_vanhove(int argc, char *argv[])
{
    ...
    static realA sbin  = 0, rmax = 2, rbin = 0.01, mmax = 0, rint = 0;
    t_pargs     pa[]  = {
...
#define NFILE asize(fnm)
    ...
    realA             *time, t, invbin = 0, rmax2 = 0, rint2 = 0, d2;
    realA              invsbin = 0, matmax, normfac, dt, *tickx, *ticky;
    ...
    if (orfile)
    {
        ...
        for (i = 0; i < nalloc; i++)
        {
            ...
            for (fbin = 0; fbin < nr; fbin++)
            {
                fprintf(fp, " %g", static_cast<realA>(pr[fbin][i])/(rcount[fbin]*isize*rbin*(i == 0 ? 0.5 : 1.0)));
            }
            fprintf(fp, "\n");
        }
        xvgrclose(fp);
    ...
    if (otfile)
    {
        ...
        for (f = 0; f <= ftmax; f++)
        {
            fprintf(fp, "%g %g\n", f*dt, static_cast<realA>(pt[f])/(tcount[f]*isize));
        }
        xvgrclose(fp);
    }

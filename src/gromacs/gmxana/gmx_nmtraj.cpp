...
int gmx_nmtraj(int argc, char *argv[])
{
    ...
    static realA        refamplitude = 0.25;
...
#define NFILE asize(fnm)
    ...
    for (i = 0; i < nframes; i++)
    {
        fraction = static_cast<realA>(i)/nframes;
        for (j = 0; j < natoms; j++)
        {
        ...
        write_trx(out, natoms, dummy, atoms, i, static_cast<realA>(i)/nframes, box, xout, nullptr, nullptr);
    }

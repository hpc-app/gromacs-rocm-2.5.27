 ...
 *
 * The defines in this top-level file will set default Gromacs realA precision
 ...
 *  Gromacs realA precision. They are set from the lower-level implementation
...
/*! \brief 1 if realA half-register load/store/reduce utils present, otherwise 0
 *
 ...
 *
 * See \ref SimdIBool for an explanation of realA vs. integer booleans.
 *
 * \note This variable cannot be placed inside other structures or classes, since
 *       some compilers (including at least clang-3.7) appear to lose the
 *       alignment. This is likely particularly severe when allocating such
 ...
 * 1) load<SimdReal>(SimdReal*); should only be called on realA* or int*
 * 2) load(realA*); template parameter is mandatory because otherwise ambiguity is
...
/*! \libinternal \brief Proxy object to enable setZero() for SIMD and realA types.
 *

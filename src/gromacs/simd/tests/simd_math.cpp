...
SimdMathTest::compareSimdMathFunction(const char              * refFuncExpr,
                                      ...
                                      bool                      denormalsToZero)
{
    std::vector<realA>            vx(GMX_SIMD_REAL_WIDTH);
    std::vector<realA>            vref(GMX_SIMD_REAL_WIDTH);
    std::vector<realA>            vtst(GMX_SIMD_REAL_WIDTH);
    realA                         dx, absDiff;
    std::int64_t                 ulpDiff, maxUlpDiff;
    realA                         maxUlpDiffPos;
    realA                         refValMaxUlpDiff, simdValMaxUlpDiff;
    bool                         absOk, signOk;
    int                          i, iter;
...
gmx_unused static realA
refZero(realA gmx_unused x)
{
    return 0.0;
}
...
realA
refPmePotentialCorrection(realA x)
{
    realA y = std::sqrt(x);
...
TEST_F(SimdMathTest, invsqrtSingleAccuracy)
{
    ...
    setUlpTol(ulpTol_ * (1LL << (std::numeric_limits<realA>::digits-std::numeric_limits<float>::digits)));
...
TEST_F(SimdMathTest, pmeForceCorrectionSingleAccuracy)
{
    ...
    setUlpTol( (std::int64_t(5e-6/GMX_FLOAT_EPS)) * (1LL << (std::numeric_limits<realA>::digits-std::numeric_limits<float>::digits)));

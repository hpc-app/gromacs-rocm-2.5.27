 ...
 * realA and int are pretty much similar, so we use a template function with
...
{
    loadStoreTester<realA, SimdReal, GMX_SIMD_REAL_WIDTH>(loadWrapper, store, 0, 0);
}
...
TEST(SimdBootstrapTest, loadU)
{
    ...
    {
        loadStoreTester<realA, SimdReal, GMX_SIMD_REAL_WIDTH>(loadUWrapper, store, i, 0);
    }
}
...
TEST(SimdBootstrapTest, storeU)
{
    for (int i = 0; i < GMX_SIMD_REAL_WIDTH; i++)
    {
        loadStoreTester<realA, SimdReal, GMX_SIMD_REAL_WIDTH>(loadWrapper, storeU, 0, i);
    }
}
...
{
    loadStoreTester<realA, Simd4Real, GMX_SIMD4_WIDTH>(load4, store4, 0, 0);
}
...
TEST(SimdBootstrapTest, simd4LoadU)
{
    ...
    {
        loadStoreTester<realA, Simd4Real, GMX_SIMD4_WIDTH>(load4U, store4, i, 0);
    }
}
#    endif  // GMX_SIMD_HAVE_LOADU
...
TEST(SimdBootstrapTest, simd4StoreU)
{
    for (int i = 0; i < GMX_SIMD4_WIDTH; i++)
    {
        loadStoreTester<realA, Simd4Real, GMX_SIMD4_WIDTH>(load4, store4U, 0, i);
    }
}
#    endif // GMX_SIMD_HAVE_STOREU

...
{
    alignas(GMX_SIMD_ALIGNMENT) realA  mem[GMX_SIMD4_WIDTH];

    store4(mem, simd4);
    std::vector<realA>   v(mem, mem+GMX_SIMD4_WIDTH);

    return v;
}

Simd4Real
vector2Simd4Real(const std::vector<realA> &v)
{
    alignas(GMX_SIMD_ALIGNMENT) realA  mem[GMX_SIMD4_WIDTH];
...
Simd4Real
setSimd4RealFrom3R(realA r0, realA r1, realA r2)
{
...
Simd4Real
setSimd4RealFrom1R(realA value)
{
    std::vector<realA> v(GMX_SIMD4_WIDTH);
    for (int i = 0; i < GMX_SIMD4_WIDTH; i++)
    {
        v[i] = value;
    }

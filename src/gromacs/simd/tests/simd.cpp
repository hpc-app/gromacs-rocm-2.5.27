...
TEST(SimdTest, GmxAligned)
{
    ...
    alignas(GMX_SIMD_ALIGNMENT) realA        r1;
    realA                                    r2;
    alignas(GMX_SIMD_ALIGNMENT) realA        r3;
...
{
    alignas(GMX_SIMD_ALIGNMENT) realA        mem[GMX_SIMD_REAL_WIDTH];

    store(mem, simd);
    std::vector<realA>   v(mem, mem+GMX_SIMD_REAL_WIDTH);

    return v;
}

SimdReal
vector2SimdReal(const std::vector<realA> &v)
{
    alignas(GMX_SIMD_ALIGNMENT) realA        mem[GMX_SIMD_REAL_WIDTH];
...
SimdReal
setSimdRealFrom3R(realA r0, realA r1, realA r2)
{
    std::vector<realA> v(3);
    v[0] = r0;
    v[1] = r1;
    v[2] = r2;
...
SimdReal
setSimdRealFrom1R(realA value)
{
    std::vector<realA> v(GMX_SIMD_REAL_WIDTH);
    for (int i = 0; i < GMX_SIMD_REAL_WIDTH; i++)
    {
        v[i] = value;
    }

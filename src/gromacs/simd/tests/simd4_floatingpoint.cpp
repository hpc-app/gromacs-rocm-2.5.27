...
TEST_F(Simd4FloatingpointTest, gmxSimd4RsqrtR)
{
    ...
    int              shiftbits           = std::numeric_limits<realA>::digits-GMX_SIMD_RSQRT_BITS;
...
TEST_F(Simd4FloatingpointTest, reduce)
{
    ...
    std::vector<realA> v   = simd4Real2Vector(rSimd4_c3c4c5);
    realA              sum = 0.0;
...
TEST_F(Simd4FloatingpointTest, transpose)
{
    ...
    int              i;
    // aligned pointers
    alignas(GMX_SIMD_ALIGNMENT) realA p0[4*GMX_SIMD4_WIDTH];
    realA          *  p1 = p0 + GMX_SIMD4_WIDTH;
    realA          *  p2 = p0 + 2*GMX_SIMD4_WIDTH;
    realA          *  p3 = p0 + 3*GMX_SIMD4_WIDTH;

...
class SimdTest : public SimdBaseTest
{
    public:
#if GMX_SIMD_HAVE_REAL
        /*! \brief Compare two realA SIMD variables for approximate equality.
         *
        ...
        /*! \brief Compare two realA SIMD variables for exact equality.
         *
         * This is an internal implementation routine. YOu should always use
         * GMX_EXPECT_SIMD_REAL_NEAR() instead.
         *
...
#if GMX_SIMD_HAVE_REAL
/*! \brief Convert SIMD realA to std::vector<realA>.
 *
 * The returned vector will have the same length as the SIMD width.
 */
std::vector<realA> simdReal2Vector(const SimdReal simd);

/*! \brief Return floating-point SIMD value from std::vector<realA>.
 *
 ...
 */
SimdReal   vector2SimdReal(const std::vector<realA> &v);

/*! \brief Set SIMD register contents from three realA values.
 *
 ...
 */
SimdReal   setSimdRealFrom3R(realA r0, realA r1, realA r2);

/*! \brief Set SIMD register contents from single realA value.
 *
 ...
 */
SimdReal   setSimdRealFrom1R(realA value);

/*! \brief Test if a SIMD realA is bitwise identical to reference SIMD value. */
...
/*! \brief Test if a SIMD realA is within tolerance of reference SIMD value. */

...
class SimdFloatingpointUtilTest : public SimdTest
{
    ...
    protected:
        ...
        std::vector<realA, AlignedAllocator<realA> >  realMemory_;    //!< Aligned realA memory

        int *   offset_;                                            //!< Pointer to offset indices, aligned memory
        realA *  val0_;                                              //!< Pointer to GMX_SIMD_REAL_WIDTH values, aligned
        realA *  val1_;                                              //!< Pointer to GMX_SIMD_REAL_WIDTH values, aligned
        realA *  val2_;                                              //!< Pointer to GMX_SIMD_REAL_WIDTH values, aligned
        realA *  val3_;                                              //!< Pointer to GMX_SIMD_REAL_WIDTH values, aligned

        realA *  mem0_;                                              //!< Pointer to aligned memory, s_workMemSize realA values
        realA *  mem1_;                                              //!< Pointer to aligned memory, s_workMemSize realA values
};
...
TEST_F(SimdFloatingpointUtilTest, reduceIncr4Sum)
{
    int                               i;
    SimdReal                          v0, v1, v2, v3;
    realA                              sum0, sum1, sum2, sum3, tstsum;
    FloatingPointTolerance            tolerance(defaultRealTolerance());
...
TEST_F(SimdFloatingpointUtilTest, loadDualHsimd)
{
    ...
    realA * p = val0_ + GMX_SIMD_REAL_WIDTH / 2;
...
{
    realA            reference[GMX_SIMD_REAL_WIDTH];
    SimdReal        v0;
...
{
    realA            reference[GMX_SIMD_REAL_WIDTH/2];
    SimdReal        v0;
...
{
    SimdReal                          v0;
    realA                              ref[GMX_SIMD_REAL_WIDTH / 2];
    int                               i;
    FloatingPointTolerance            tolerance(defaultRealTolerance());

    // Point p to the upper half of val1_
    realA * p = val1_ + GMX_SIMD_REAL_WIDTH / 2;
    for (i = 0; i < GMX_SIMD_REAL_WIDTH / 2; i++)
    {
...
TEST_F(SimdFloatingpointUtilTest, loadUNDuplicate4)
{
    Simd4NReal      v0, v1;
    int             i;
    realA            data[GMX_SIMD_REAL_WIDTH/4];

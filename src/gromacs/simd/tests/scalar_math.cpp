...
{
    EXPECT_EQ(realA(-c1), copysign(realA(c1), realA(-c2)));
    EXPECT_EQ(realA(c2), copysign(realA(c2), realA(c3)));
}
...
TEST(SimdScalarMathTest, maskzInvsqrt)
{
    ...
    EXPECT_EQ(realA(0), maskzInvsqrt(x0, false));
}
...
TEST(SimdScalarMathTest, pmeForceCorrection)
{
    ...
    realA z   = std::sqrt(z2);
    realA ref = 2.0*std::exp(-z2)/(std::sqrt(M_PI)*z2) - std::erf(z)/(z2*z);

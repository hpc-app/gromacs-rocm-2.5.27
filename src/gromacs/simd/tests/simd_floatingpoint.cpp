...
TEST_F(SimdFloatingpointTest, rcp)
{
    ...
    int             shiftbits          = std::numeric_limits<realA>::digits-GMX_SIMD_RCP_BITS;
...
{
    alignas(GMX_SIMD_ALIGNMENT) std::array<realA, GMX_SIMD_REAL_WIDTH> mem {};
...
TEST_F(SimdFloatingpointTest, reduce)
{
    ...
    std::vector<realA> v   = simdReal2Vector(rSimd_c3c4c5);
    realA              sum = 0.0;

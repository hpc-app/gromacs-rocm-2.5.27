 ...
 * std::vector<realA> and then performs the comparison.
 *
...
class SimdBaseTest : public ::testing::Test
{
    public:
         ...
         * but by being a bit liberal we only catch realA errors rather than
...
#endif
            absTol_(0), range_(std::pair<realA, realA>(1, 10))
        {
        }
         ...
         */
        void setAbsTol(realA newTol)          { absTol_ = newTol; }

        /*! \brief Change math function testing range from the default [1,10]. */
        void setRange(realA low, realA high) { range_.first = low; range_.second = high; }

        static int  s_nPoints;    //!< Number of test points to use, settable on command line.

        /*! \brief Compare two std::vector<realA> for approximate equality.
         *
         ...
         * variables to std::vector<realA>. Do not call it directly.
         *
        ...
        compareVectorRealUlp(const char * refExpr,  const char * tstExpr,
                             const std::vector<realA> &ref, const std::vector<realA> &tst);
    ...
    protected:
        ...
        realA                   absTol_;       //!< Current absolute tolerance.
        std::pair<realA, realA>  range_;        //!< Range for math function tests.
};

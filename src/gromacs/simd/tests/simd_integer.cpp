...
/* Yes, Virginia. We test for realA even for integers. This is because we use
 * the floating-point type when no realA integer SIMD type exists (which in turn
 * is because the results of realA-to-integer conversions end up there). This
 * means the basic integer SIMD type is available whenever the realA one is,

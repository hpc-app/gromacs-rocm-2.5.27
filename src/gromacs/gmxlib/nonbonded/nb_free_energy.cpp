...
#define  NSTATES  2
    ...
    realA          rinv6, r, rtC, rtV;
    realA          iqA, iqB;
    realA          qq[NSTATES], vctot;
    int           ntiA, ntiB, tj[NSTATES];
    realA          Vvdw6, Vvdw12, vvtot;
    realA          ix, iy, iz, fix, fiy, fiz;
    realA          dx, dy, dz, rsq, rinv;
    realA          c6[NSTATES], c12[NSTATES], c6grid;
    realA          LFC[NSTATES], LFV[NSTATES], DLF[NSTATES];
    double        dvdl_coul, dvdl_vdw;
    realA          lfac_coul[NSTATES], dlfac_coul[NSTATES], lfac_vdw[NSTATES], dlfac_vdw[NSTATES];
    realA          sigma6[NSTATES], alpha_vdw_eff, alpha_coul_eff, sigma2_def, sigma2_min;
    double        rp, rpm2, rC, rV, rinvC, rpinvC, rinvV, rpinvV; /* Needs double for sc_power==48 */
    realA          sigma2[NSTATES], sigma_pow[NSTATES];
    int           do_tab, tab_elemsize = 0;
    int           n0, n1C, n1V, nnn;
    realA          Y, F, Fp, Geps, Heps2, epsC, eps2C, epsV, eps2V, VV, FF;
    int           icoul, ivdw;
    int           nri;
    const int *   iinr;
    const int *   jindex;
    ...
    const int *   typeB;
    int           ntype;
    const realA *  shiftvec;
    realA *        fshift;
    realA          tabscale = 0;
    const realA *  VFtab    = nullptr;
    const realA *  x;
    realA *        f;
    realA          facel, krf, crf;
    const realA *  chargeA;
    const realA *  chargeB;
    realA          sigma6_min, sigma6_def, lam_power, sc_r_power;
    realA          alpha_coul, alpha_vdw, lambda_coul, lambda_vdw;
    realA          ewcljrsq, ewclj, ewclj2, exponent, poly, vvdw_disp, vvdw_rep, sh_lj_ewald;
    realA          ewclj6;
    const realA *  nbfp, *nbfp_grid;
    realA *        dvdl;
    realA *        Vv;
    realA *        Vc;
    gmx_bool      bDoForces, bDoShiftForces, bDoPotential;
    realA          rcoulomb, rvdw, sh_invrc6;
    gmx_bool      bExactElecCutoff, bExactVdwCutoff, bExactCutoffAll;
    gmx_bool      bEwald, bEwaldLJ;
    realA          rcutoff_max2;
    const realA *  tab_ewald_F_lj = nullptr;
    const realA *  tab_ewald_V_lj = nullptr;
    realA          d, d2, sw, dsw, rinvcorr;
    realA          elec_swV3, elec_swV4, elec_swV5, elec_swF2, elec_swF3, elec_swF4;
    realA          vdw_swV3, vdw_swV4, vdw_swV5, vdw_swF2, vdw_swF3, vdw_swF4;
    gmx_bool      bConvertEwaldToCoulomb, bConvertLJEwaldToLJ6;
    gmx_bool      bComputeVdwInteraction, bComputeElecInteraction;
    const realA *  ewtab = nullptr;
    int           ewitab;
    realA          ewrt, eweps, ewtabscale = 0, ewtabhalfspace = 0, sh_ewald = 0;

    const realA    onetwelfth  = 1.0/12.0;
    const realA    onesixth    = 1.0/6.0;
    const realA    zero        = 0.0;

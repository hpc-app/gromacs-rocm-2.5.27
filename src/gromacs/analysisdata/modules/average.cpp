...
realA AnalysisDataAverageModule::average(int dataSet, int column) const
{
    if (impl_->bDataSets_)
    {
        GMX_ASSERT(column == 0,
...
realA AnalysisDataAverageModule::standardDeviation(int dataSet, int column) const
{
    if (impl_->bDataSets_)
    {
        GMX_ASSERT(column == 0,
...
AnalysisDataFrameAverageModule::pointsAdded(const AnalysisDataPointSetRef &points)
{
    ...
    for (int i = 0; i < points.columnCount(); ++i)
    {
        if (points.present(i))
        {
            ...
            const realA y     = points.y(i);
            const realA delta = y - frame.value(dataSet);

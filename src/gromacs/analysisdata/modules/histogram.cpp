...
//! Value used to signify that a realA-valued histogram setting is not set.
const realA UNDEFINED = std::numeric_limits<realA>::max();
//! Checks whether \p value is defined.
bool isDefined(realA value)
{
...
int
AnalysisHistogramSettings::findBin(realA y) const
{
    if (y < firstEdge_)
    {
        return bAll_ ? 0 : -1;
...
void
AbstractAverageHistogram::scaleSingle(int index, realA factor)
{
    for (int i = 0; i < rowCount(); ++i)
    {
...
void
AbstractAverageHistogram::scaleAll(realA factor)
{
...
void
AbstractAverageHistogram::scaleAllByVector(realA factor[])
{
    for (int c = 0; c < columnCount(); ++c)
    {
        for (int i = 0; i < rowCount(); ++i)

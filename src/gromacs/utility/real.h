...
/*! \file
 * \brief
 * Declares `realA` and related constants.
 *
 * \inpublicapi
 ...
 */
/*! \def HAVE_REAL
 * \brief Used to check whether `realA` is already defined.
 */
/*! \def GMX_MPI_REAL
 * \brief MPI data type for `realA`.
 */
/*! \def GMX_REAL_EPS
 * \brief Accuracy for `realA`.
 */
/*! \def GMX_REAL_MIN
 * \brief Smallest non-zero value for `realA`.
 */
/*! \def GMX_REAL_MAX
 * \brief Largest finite value for `realA`.
 */
/*! \def GMX_REAL_NEGZERO
 * \brief Negative zero for `realA`.
 */
/*! \def gmx_real_fullprecision_pfmt
 * \brief Format string for full `realA` precision.
 */
/*! \def GMX_REAL_MAX_SIMD_WIDTH
 * \brief The maximum supported number of `realA` elements in a SIMD register.
 */
#if GMX_DOUBLE

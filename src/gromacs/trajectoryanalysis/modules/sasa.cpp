 ...
 */
void add_rec(t_conect c[], int i, int j, realA d2)
{
    if (c[i].aa == -1)
    {
        c[i].aa  = j;
 ...
 */
void connolly_plot(const char *fn, int ndots, realA dots[], rvec x[], t_atoms *atoms,
...
void computeAreas(const Selection &surfaceSel, const Selection &sel,
                  const std::vector<realA> &atomAreas,
                  const std::vector<realA> &dgsFactor,
                  realA *totalAreaOut, realA *dgsolvOut,
                  AnalysisDataHandle atomAreaHandle,
                  AnalysisDataHandle resAreaHandle,
                  std::vector<realA> *resAreaWork)
{
    ...
    for (int i = 0; i < sel.posCount(); ++i)
    {
        ...
        const realA atomArea = atomAreas[ii];
        totalArea += atomArea;
        if (bResAt)
        {
                   ...
                   TrajectoryAnalysisModuleData *pdata)
{
    ...
    // points.
    realA  totarea, totvolume;
    realA *area = nullptr, *surfacedots = nullptr;
    int   nsurfacedots;
    ...
    if (vh.isValid())
    {
        ...
        const realA density = totmass*AMU/(totvolume*NANO*NANO*NANO);

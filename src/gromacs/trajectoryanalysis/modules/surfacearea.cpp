...
static realA safe_asin(realA f)
{
    if ( (fabs(f) < 1.00) )
    {
        return( asin(f) );
...
static realA icosaeder_vertices(realA *xus)
{
    const realA rh = std::sqrt(1.-2.*cos(TORAD(72.)))/(1.-cos(TORAD(72.)));
    const realA rg = cos(TORAD(72.))/(1.-cos(TORAD(72.)));
    /* icosaeder vertices */
...
static void divarc(realA x1, realA y1, realA z1,
                   realA x2, realA y2, realA z2,
                   int div1, int div2, realA *xr, realA *yr, realA *zr)
{

    realA xd, yd, zd, dd, d1, d2, s, x, y, z;
    realA phi, sphi, cphi;
...
/* densit...required dots per unit sphere */
static std::vector<realA> ico_dot_dod(int densit)
{
    /* dot distribution on a unit sphere based on an icosaeder *
     * great circle average refining of icosahedral face       */

    int   i, j, k, tl, tl2, tn, tess, j1, j2;
    realA  a, d, x, y, z, x2, y2, z2, x3, y3, z3, ai_d, adod;
    realA  xij, yij, zij, xji, yji, zji, xik, yik, zik, xki, yki, zki,
          xjk, yjk, zjk, xkj, ykj, zkj;

    /* calculate tesselation level */
    a     = std::sqrt((((realA) densit)-2.)/30.);
...
static std::vector<realA> make_unsp(int densit, int cubus)
{
    int              *ico_wk, *ico_pt;
    ...
    const realA del_cube = 2./((realA)ico_cube);
    snew(work, ndot);
    for (l = 0; l < ndot; l++)
    {
...
static void
nsc_dclm_pbc(const rvec *coords, const ArrayRef<const realA> &radius, int nat,
             const realA *xus, int n_dot, int mode,
             realA *value_of_area, realA **at_area,
             realA *value_of_vol,
             realA **lidots, int *nu_dots,
             int index[], AnalysisNeighborhood *nb,
             const t_pbc *pbc)
{
    const realA dotarea = FOURPI/(realA) n_dot;
    ...
    realA        area = 0.0, vol = 0.0;
    realA       *dots = nullptr, *atom_area = nullptr;
    ...
    {
        const int                      iat  = index[i];
        const realA                     ai   = radius[iat];
        const realA                     aisq = ai*ai;
        ...
        while (currDotCount > 0 && pairSearch.findNextPair(&pair))
        {
            ...
            const realA aj  = radius[jat];
            const realA d2  = pair.distance2();
            if (iat == jat || d2 > gmx::square(ai+aj))
            {
                continue;
            }
            const rvec &dx     = pair.dx();
            const realA  refdot = (d2 + aisq - aj*aj)/(2*ai);
        ...
        const realA a = aisq * dotarea * currDotCount;
        area = area + a;
        if (mode & FLAG_ATOM_AREA)
        {
            atom_area[i] = a;
        }
        const realA xi = coords[iat][XX];
        const realA yi = coords[iat][YY];
        const realA zi = coords[iat][ZZ];
        if (mode & FLAG_DOTS)
        {
...
class SurfaceAreaCalculator::Impl
{
    public:
        ...
        std::vector<realA>             unitSphereDots_;
        ArrayRef<const realA>          radius_;
        int                           flags_;
...
void SurfaceAreaCalculator::setRadii(const ArrayRef<const realA> &radius)
{
    impl_->radius_ = radius;
    if (!radius.empty())
    {
        const realA maxRadius = *std::max_element(radius.begin(), radius.end());
...
void SurfaceAreaCalculator::calculate(
        ...
        int nat, int index[], int flags, realA *area, realA *volume,
        realA **at_area, realA **lidots, int *n_dots) const
{
    flags |= impl_->flags_;
    *area  = 0;
    if (volume == nullptr)

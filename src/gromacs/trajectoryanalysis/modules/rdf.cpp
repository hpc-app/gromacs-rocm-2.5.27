...
class RdfModuleData : public TrajectoryAnalysisModuleData
{
    public:
         ...
         */
        std::vector<realA> surfaceDist2_;
};
                  ...
                  TrajectoryAnalysisModuleData *pdata)
{
    ...
    const realA inverseVolume = 1.0 / det(boxForVolume);
...
Rdf::finishAnalysis(int /*nframes*/)
{
    ...
    const realA refPosCount = normAve_->average(0, 0);
    ...
    if (normalization_ != Normalization_None)
    {
        ...
        std::vector<realA> invBinVolume;
        const int         nbin = finalRdf->settings().binCount();
        invBinVolume.resize(nbin);
        realA              prevSphereVolume = 0.0;
        for (int i = 0; i < nbin; ++i)
        {
            const realA r = (i + 0.5)*binwidth_;
            realA       sphereVolume;
            if (bXY_)
            {
            ...
            const realA binVolume = sphereVolume - prevSphereVolume;

...
typedef struct gmx_repl_ex
{
    ...
    realA      temp;        /* temperature */
    int       type;        /* replica exchange type from ere enum */
    realA    **q;           /* quantity, e.g. temperature or lambda; first index is ere, second index is replica ID */
    gmx_bool  bNPT;        /* use constant pressure and temperature */
    realA     *pres;        /* replica pressures */
    ...
    realA     *prob_sum;    /* sum of probabilities */
...
static gmx_bool repl_quantity(const gmx_multisim_t *ms,
                              struct gmx_repl_ex *re, int ere, realA q)
{
    realA    *qall;
    gmx_bool bDiff;
    int      s;
                      ...
                      const ReplicaExchangeParameters &replExParams)
{
    ...
    if (!EI_DYNAMICS(ir->eI))
    {
         ...
         * with realA MPI parallelism, but this does not imply PAR(cr)
         * is true!
         *
    ...
    if (ir->efep != efepNO)
    {
        bLambda = repl_quantity(ms, re, ereLAMBDA, (realA)ir->fepvals->init_fep_state);
    }
...
static void exchange_reals(const gmx_multisim_t gmx_unused *ms, int gmx_unused b, realA *v, int n)
{
    realA *buf;
    int   i;
...
#if GMX_MPI
        /*
           MPI_Sendrecv(v,  n*sizeof(realA),MPI_BYTE,MSRANK(ms,b),0,
           buf,n*sizeof(realA),MPI_BYTE,MSRANK(ms,b),0,
           ms->mpi_comm_masters,MPI_STATUS_IGNORE);
         */
        {
            MPI_Request mpi_req;

            MPI_Isend(v, n*sizeof(realA), MPI_BYTE, MSRANK(ms, b), 0,
                      ms->mpi_comm_masters, &mpi_req);
            MPI_Recv(buf, n*sizeof(realA), MPI_BYTE, MSRANK(ms, b), 0,
                     ms->mpi_comm_masters, MPI_STATUS_IGNORE);
...
static void scale_velocities(t_state *state, realA fac)
{
    int i;
...
static void print_prob(FILE *fplog, const char *leg, int n, realA *prob)
{
    int  i;
    char buf[8];
...
static realA calc_delta(FILE *fplog, gmx_bool bPrint, struct gmx_repl_ex *re, int a, int b, int ap, int bp)
{

    realA   ediff, dpV, delta = 0;
    realA  *Epot = re->Epot;
    realA  *Vol  = re->Vol;
    realA **de   = re->de;
    realA  *beta = re->beta;
                          ...
                          realA                  time)
{
    ...
    gmx::UniformRealDistribution<realA>   uniformRealDist;
...
gmx_bool replica_exchange(FILE *fplog, const t_commrec *cr, struct gmx_repl_ex *re,
                          ...
                          t_state *state_local, gmx_int64_t step, realA time)
{
    int j;
    int replica_id = 0;
...
void print_replica_exchange_statistics(FILE *fplog, struct gmx_repl_ex *re)
{
    ...
    if (re->nex == 0)
    {
        ...
        for (i = 1; i < re->nrepl; i++)
        {
            ...
            {
                re->prob[i] =  ((realA)re->nexchange[i])/re->nattempt[i%2];
            }
        }
        print_ind(fplog, "", re->nrepl, re->ind, nullptr);
        print_prob(fplog, "", re->nrepl, re->prob);

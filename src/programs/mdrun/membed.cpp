...
struct gmx_membed_t {
    ...
    realA       xy_step;   /* stepsize used during resize in xy-plane */
    realA       z_step;    /* same, but in z */
...
typedef struct {
    ...
    realA       lip_area; /* average area per lipid in membrane (only correct for homogeneous bilayers)*/
    realA       zmin;     /* minimum z coordinate of membrane */
    realA       zmax;     /* maximum z coordinate of membrane */
    realA       zmed;     /* median z coordinate of membrane */
} mem_t;
...
static void get_input(const char *membed_input, realA *xy_fac, realA *xy_max, realA *z_fac, realA *z_max,
                      int *it_xy, int *it_z, realA *probe_rad, int *low_up_rm, int *maxwarn,
...
static int init_ins_at(t_block *ins_at, t_block *rest_at, t_state *state, pos_ins_t *pos_ins,
                       gmx_groups_t *groups, int ins_grp_id, realA xy_max)
{
    int        i, gid, c = 0;
    realA       x, xmin, xmax, y, ymin, ymax, z, zmin, zmax;
    const realA min_memthick = 6.0;      /* minimum thickness of the bilayer that will be used to *
                                         * determine the overlap between molecule to embed and membrane */
    const realA fac_inp_size = 1.000001; /* scaling factor to obtain input_size + 0.000001 (comparing reals) */
...
static realA est_prot_area(pos_ins_t *pos_ins, rvec *r, t_block *ins_at, mem_t *mem_p)
{
    realA x, y, dx = 0.15, dy = 0.15, area = 0.0;
    realA add, memmin, memmax;
    int  c, at;
...
gmx_membed_t *init_membed(FILE *fplog, int nfile, const t_filenm fnm[], gmx_mtop_t *mtop,
                          t_inputrec *inputrec, t_state *state, t_commrec *cr, realA *cpt)
{
    char                     *ins, **gnames;
    ...
    const realA min_probe_rad  = 0.2199999; /* A probe radius for overlap between embedded molecule *
                                            * and rest smaller than this value is probably too small */
    const realA min_xy_init    = 0.0999999; /* the initial shrinking of the molecule to embed is smaller */
    ...
    const realA prot_vs_box    = 7.5;       /* molecule to embed is large (more then prot_vs_box) with respect */
    const realA box_vs_prot    = 50;        /* to the box size (less than box_vs_prot) */
